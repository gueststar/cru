...
{
  if ((! new_edge) ? 1 : new_edge->next_edge ? IER(867) : old_edges ? 0 : IER(868))
	 return;
...
{
  if ((! new_edge) ? 1 : (! front) ? IER(869) : (! back) ? IER(870) : ((! *front) != ! *back) ? IER(871) : 0)
	 return;
  if ((! *back) ? 0 : (*back)->next_edge ? IER(872) : 0)
	 return;
  if (*front)
	  ...
	  // the list.
{
  ...
  r = NULL;
  e = &n;
  if ((! o) ? IER(873) : (! (o->hash)) ? IER(874) : (! *e) ? 1 : ! ((*e)->next_edge))
	 return NULL;
	  ...
	  // Remove and return the first edge from a list.
{
  ...
  if ((! edges) ? IER(875) : *edges ? 0 : IER(876))
	 return NULL;
	  ...
	  // order relational predicate.
{
  ...
  m = NULL;
  if ((! t) ? IER(879) : (! edges) ? IER(880) : *edges ? 0 : IER(881))
	 return NULL;
  for (e = *edges; *err ? NULL : e; e = e->next_edge)
	 if (! (e->remote.node ? 0 : IER(882)))
	  ...
	  // Remove edges matching the given terminus and carrier.
{
  ...
  if (terminus ? (carrier ? (edges ? (carrier->next_edge ? IER(883) : 0) : IER(884)) : 1) : 1)
	 return NULL;
  result = NULL;
  for (e = edges; *e;)
...
{
#define DANGLING(l,x) (((x = (by_class ? CLASS_OF(l->remote.node) : l->remote.node))) ? 0 : IER(885))
#define VERTEX(x) (by_accumulator ? x->accumulator : with_locks ? _cru_read (&(x->vertex), err) : x->vertex)
  ...
  else if ((m->m_free != m->r_free) ? IER(888) : DANGLING(e,n))
	 return NULL;
  else
	 {
