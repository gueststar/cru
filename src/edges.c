...
{
  if ((! new_edge) ? 1 : new_edge->next_edge ? IER(868) : old_edges ? 0 : IER(869))
	 return;
...
{
  if ((! new_edge) ? 1 : (! front) ? IER(870) : (! back) ? IER(871) : ((! *front) != ! *back) ? IER(872) : 0)
	 return;
  if ((! *back) ? 0 : (*back)->next_edge ? IER(873) : 0)
	 return;
  if (*front)
	  ...
	  // the list.
{
  ...
  r = NULL;
  e = &n;
  if ((! o) ? IER(874) : (! (o->hash)) ? IER(875) : (! *e) ? 1 : ! ((*e)->next_edge))
	 return NULL;
	  ...
	  // Remove and return the first edge from a list.
{
  ...
  if ((! edges) ? IER(876) : *edges ? 0 : IER(877))
	 return NULL;
	  ...
	  // order relational predicate.
{
  ...
  m = NULL;
  if ((! t) ? IER(880) : (! edges) ? IER(881) : *edges ? 0 : IER(882))
	 return NULL;
  for (e = *edges; *err ? NULL : e; e = e->next_edge)
	 if (! (e->remote.node ? 0 : IER(883)))
	  ...
	  // Remove edges matching the given terminus and carrier.
{
  ...
  if (terminus ? (carrier ? (edges ? (carrier->next_edge ? IER(884) : 0) : IER(885)) : 1) : 1)
	 return NULL;
  result = NULL;
  for (e = edges; *e;)
...
{
#define DANGLING(l,x) (((x = (by_class ? CLASS_OF(l->remote.node) : l->remote.node))) ? 0 : IER(886))
#define VERTEX(x) (by_accumulator ? x->accumulator : with_locks ? _cru_read (&(x->vertex), err) : x->vertex)
  ...
  else if ((m->m_free != m->r_free) ? IER(889) : DANGLING(e,n))
	 return NULL;
  else
	 {
