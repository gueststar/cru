	  ...
	  // Put a new node into a node queue at the end.
{
  if ((! q) ? IER(1381) : 0)
	 return 0;
  if ((! new_node) ? 1 : new_node->next_node ? IER(1382) : new_node->previous ? IER(1383) : 0)
	 return 0;
  if (*q ? NULL : (*q = (node_queue) _cru_malloc (sizeof (**q))))
	 memset (*q, 0, sizeof (**q));
  else if (*q ? 0 : RAISE(ENOMEM))
	 return 0;
  if (((!((*q)->front)) != !((*q)->back)) ? IER(1384) : (!((*q)->back)) ? 0 : (*q)->back->next_node ? IER(1385) : 0)
	 return 0;
  if ((*q)->back)
	 {
	  ...
	  // Append one node queue to another.
{
  ...
  if ((! r) ? 1 : *q ? 0 : ! ! (*q = r))
	 return 1;
  if (((!(r->front)) != ! (r->back)) ? IER(1387) : ((!((*q)->front)) != !((*q)->back)) ? IER(1388) : 0)
	 return 0;
  ...
  if (r->back->next_node ? IER(1389) : (*q)->back->next_node ? IER(1390) : (r->front->previous != &(r->front)) ? IER(1391) : 0)
	 return 0;
