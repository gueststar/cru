...
{
  return ! (pthread_rwlock_init (&packet_lock, NULL) ? IER(1239) : 0);
}
  ...
  for (err = 0; (t = reserve_packets); _cru_free (t))
	 {
		...
		err = (packet_count-- ? err : THE_IER(1240));
	 }
	  ...
	  // dip into the reserves regardless of whether they're exhausted.
{
  ...
  if ((l = (packet_list) _cru_malloc (sizeof (*l))) ? 1 : pthread_rwlock_wrlock (&packet_lock) ? IER(1243) : 0)
	 goto a;
  l = (((! reserve_packets) ? 1 : packet_count-- ? 0 : IER(1244)) ? NULL : _cru_popped_packet (&reserve_packets, err));
  if (pthread_rwlock_unlock (&packet_lock) ? IER(1245) : 1)
	 RAISE(ENOMEM);
 a: return l;
}
  ...
  while ((t = p))
	 {
		...
		p = p->next_packet;
		if (pthread_rwlock_rdlock (&packet_lock) ? IER(1246) : 0)
		  goto a;
		ok = (packet_count >= RESERVE_PACKET_LIMIT);
		if (pthread_rwlock_unlock (&packet_lock) ? IER(1247) : ok ? 1 : pthread_rwlock_wrlock (&packet_lock) ? IER(1248) : 0)
		  goto a;
		memset (t, 0, sizeof (*t));
		t->next_packet = reserve_packets;
		reserve_packets = t;
		packet_count++;
		if (pthread_rwlock_unlock (&packet_lock) ? IER(1249) : 1)
		  continue;
	 a: _cru_free (t);
	 }
}
	  ...
	  // Return the current number of packets held in reserve.
{
  ...
  if (pthread_rwlock_rdlock (&packet_lock) ? IER(1250) : 0)
	 return 0;
  result = packet_count;
	  ...
	  // Create more reserve packets if necessary.
{
  ...
  if (pthread_rwlock_wrlock (&packet_lock) ? IER(1252) : 0)
	 return;
...
{
  if ((! h) ? 1 : h->next_packet ? IER(1254) : (! t) ? IER(1255) : 0)
	 return 0;
  h->next_packet = *t;
  *t = h;
  return 1;
...
{
  if ((! b) ? IER(1258) : (! i) ? IER(1259) : b->payload ? 1 : ! ! (b->receiver))
	 return 0;
  b->payload = i->payload;
...
{
  if ((! i) ? IER(1261) : (! b) ? IER(1262) : (b != *i) ? 1 : _cru_popped_packet (i, err) ? 0 : IER(1263))
	 return 0;
  memset (b, 0, sizeof (*b));
  return 1;
}
