	  ...
	  // possible.
{
  ...
  if (a ? 0 : IER(1292))
	 return NULL;
  pod_size = a * sizeof (*d);
  if ((pod_size < a) ? IER(1293) : (pod_size < sizeof (*d)) ? IER(1294) : 0)
	 return NULL;
  ...
  if ((p = (packet_pod) _cru_malloc (sizeof (*p))) ? 0 : RAISE(ENOMEM))
	 {
#ifdef WRAP
		_cru_free_wrapper (__LINE__, __FILE__, pod_size, d);
#else
		free (d);
#endif
		return NULL;
	  ...
	  // Harvest the packets from a particular list in a pod.
{
  ...
  if ((! s) ? 1 : (! (s->pod)) ? IER(1296) : (i >= s->arity))
	 return NULL;
  r = s->pod[i];
  s->pod[i] = NULL;
  return r;
