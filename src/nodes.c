...
{
  if ((! n) ? IER(1224) : (! nodes) ? IER(1225) : n->previous ? IER(1226) : n->next_node ? IER(1227) : 0)
	 return 0;
  if (*(n->previous = nodes))
...
{
  if ((! n) ? IER(1228) : (!(n->previous)) ? IER(1229) : 0)
	 return NULL;
  if (*(n->previous) == n)
	  ...
	  // at each call site to the test code.
{
  ...
  if ((! n) ? IER(1234) : (! p) ? IER(1235) : (! (p->vertex.map)) ? IER(1236) : *err)
	 return NULL;
	  ...
	  // code.
{
  ...
  else if ((p->vertex.m_free != p->vertex.r_free) ? IER(1238) : 0)
	 return NULL;
  else
	 {
  ...
  if ((! n) ? 0 : p->vertex.reduction ? 1 : ! IER(1239))
