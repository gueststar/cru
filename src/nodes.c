...
{
  if ((! n) ? IER(1229) : (! nodes) ? IER(1230) : n->previous ? IER(1231) : n->next_node ? IER(1232) : 0)
	 return 0;
  if (*(n->previous = nodes))
...
{
  if ((! n) ? IER(1233) : (!(n->previous)) ? IER(1234) : 0)
	 return NULL;
  if (*(n->previous) == n)
	  ...
	  // at each call site to the test code.
{
  ...
  if ((! n) ? IER(1239) : (! p) ? IER(1240) : (! (p->vertex.map)) ? IER(1241) : *err)
	 return NULL;
	  ...
	  // code.
{
  ...
  else if ((p->vertex.m_free != p->vertex.r_free) ? IER(1243) : 0)
	 return NULL;
  else
	 {
  ...
  if ((! n) ? 0 : p->vertex.reduction ? 1 : ! IER(1244))
