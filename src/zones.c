	  ...
	  // switch during testing to simulate other possible errors.
{
  ...
  result = NULL;
  killed = found = dblx = 0;
  if ((! source) ? IER(1736) : (source->gruntled != PORT_MAGIC) ? IER(1737) : 0)
	 return NULL;
  if ((!(r = source->local)) ? IER(1738) : (r->valid != ROUTER_MAGIC) ? IER(1739) : 0)
	 return NULL;
  z = &(r->ro_plan.zone);
  if ((! (d = source->peers)) ? IER(1740) : (e = r->ro_sig.orders.v_order.equal) ? *err : IER(1741))
	 {
  ...
  for (incoming = NULL; incoming ? incoming : (incoming = _cru_exchanged (source, d, err));)
	 {
		...
		if (killed ? 1 : found)
		  goto a;
		if ((n = (node_list) incoming->payload) ? (_cru_test_and_set_membership (n, &seen, err) ? 1 : *err) : IER(1742))
		  goto a;
	  ...
	  // co-workers by adding every incoming packet's payload to it.
{
  ...
  sample = 0;
  killed = 0;
  if ((! source) ? IER(1743) : (source->gruntled != PORT_MAGIC) ? IER(1744) : 0)
	 return NULL;
  if ((! (r = source->local)) ? IER(1745) : (r->valid != ROUTER_MAGIC) ? IER(1746) : 0)
	 return NULL;
  if (! (destinations = source->peers) ? IER(1747) : *err)
  ...
  for (incoming = NULL; incoming ? incoming : (incoming = _cru_exchanged (source, destinations, err));)
	 {
		KILL_SITE(33);
		killed = (killed ? 1 : KILLED);
		if (killed ? 1 : (n = (node_list) incoming->payload) ? 0 : IER(1748))
		  goto a;
	  ...
	  // filtering and mutation.
{
  ...
  if ((! r) ? 1 : (r->valid != ROUTER_MAGIC) ? IER(1749) : 0)
	 return NULL;
  z = &((w = &(r->ro_plan))->zone);
	  ...
	  // unreachable nodes.
{
  ...
  if ((! r) ? IER(1750) : (r->valid != ROUTER_MAGIC) ? IER(1751) : 0)
	 return 0;
  if (((! r->ports) ? IER(1752) : (! (r->lanes)) ? IER(1753) : 0) ? (r->valid = MUGGLE(75)) : 0)
	 return 0;
  if ((! unvisitable) ? IER(1754) : *unvisitable ? IER(1755) : 0)
	 return 0;
  for (visited = 1; (! e) ? 0 : *unvisitable ? 0 : visited; e = e->next_edge)
	 {
		if (((p = r->ports[_cru_scalar_hash (o = e->remote.node) % r->lanes]) ? 0 : IER(1756)) ? (r->valid = MUGGLE(76)) : 0)
		  return 0;
		if ((p->gruntled != PORT_MAGIC) ? IER(1757) : 0)
		  return 0;
		if (p->reachable ? (! _cru_member (o, p->reachable)) : 0)
		  continue;
		if ((p == s) ? 0 : (pthread_rwlock_rdlock (&(p->p_lock)) ? IER(1758) : 0) ? (p->gruntled = MUGGLE(77)) : 0)
		  return 0;
		if (! (*unvisitable = p->disabled))
		  visited = _cru_member (o, p->visited);
		if ((p == s) ? 0 : (pthread_rwlock_unlock (&(p->p_lock)) ? IER(1759) : 0) ? (p->gruntled = MUGGLE(78)) : 0)
		  return 0;
	 }
...
{
  if ((! s) ? IER(1760) : (s->gruntled != PORT_MAGIC) ? IER(1761) : ! all_visited (e, s->local, s, unvisitable, err))
	 return 0;
  if ((pthread_mutex_lock (&(s->suspension)) ? IER(1762) : 0) ? (s->gruntled = MUGGLE(79)) : 0)
	 return 1;
  if (s->backoff)
	 s->backoff = 1;
  if (pthread_mutex_unlock (&(s->suspension)) ? IER(1763) : 0)
	  ...
	  // is always implied if the traversal order is unconstrained.
{
  ...
  if ((! s) ? IER(1764) : (s->gruntled != PORT_MAGIC) ? IER(1765) : 0)
	 return 0;
  if ((! n) ? IER(1766) : (! (r = s->local)) ? IER(1767) : (r->valid != ROUTER_MAGIC) ? IER(1768) : 0)
	 return 0;
...
{
  return ((n ? 0 : IER(1769)) ? 0 : visited (n->edges_out, s, unvisitable, err));
}
