	  ...
	  // switch during testing to simulate other possible errors.
{
  ...
  result = NULL;
  killed = found = dblx = 0;
  if ((! source) ? IER(1692) : (source->gruntled != PORT_MAGIC) ? IER(1693) : 0)
	 return NULL;
  if ((!(r = source->local)) ? IER(1694) : (r->valid != ROUTER_MAGIC) ? IER(1695) : 0)
	 return NULL;
  z = &(r->ro_plan.zone);
  if ((! (d = source->peers)) ? IER(1696) : (e = r->ro_sig.orders.v_order.equal) ? *err : IER(1697))
	 {
  ...
  for (incoming = NULL; incoming ? incoming : (incoming = _cru_exchanged (source, d, err));)
	 {
		...
		if (killed ? 1 : found)
		  goto a;
		if ((n = (node_list) incoming->payload) ? (_cru_test_and_set_membership (n, &seen, err) ? 1 : *err) : IER(1698))
		  goto a;
	  ...
	  // co-workers by adding every incoming packet's payload to it.
{
  ...
  sample = 0;
  killed = 0;
  if ((! source) ? IER(1699) : (source->gruntled != PORT_MAGIC) ? IER(1700) : 0)
	 return NULL;
  if ((! (r = source->local)) ? IER(1701) : (r->valid != ROUTER_MAGIC) ? IER(1702) : 0)
	 return NULL;
  if (! (destinations = source->peers) ? IER(1703) : *err)
  ...
  for (incoming = NULL; incoming ? incoming : (incoming = _cru_exchanged (source, destinations, err));)
	 {
		KILL_SITE(31);
		killed = (killed ? 1 : KILLED);
		if (killed ? 1 : (n = (node_list) incoming->payload) ? 0 : IER(1704))
		  goto a;
	  ...
	  // filtering and mutation.
{
  ...
  if ((! r) ? 1 : (r->valid != ROUTER_MAGIC) ? IER(1705) : 0)
	 return NULL;
  z = &((w = &(r->ro_plan))->zone);
	  ...
	  // unreachable nodes.
{
  ...
  if ((! r) ? IER(1706) : (r->valid != ROUTER_MAGIC) ? IER(1707) : 0)
	 return 0;
  if (((! r->ports) ? IER(1708) : (! (r->lanes)) ? IER(1709) : 0) ? (r->valid = MUGGLE(79)) : 0)
	 return 0;
  if ((! unvisitable) ? IER(1710) : *unvisitable ? IER(1711) : 0)
	 return 0;
  for (visited = 1; (! e) ? 0 : *unvisitable ? 0 : visited; e = e->next_edge)
	 {
		if (((p = r->ports[_cru_scalar_hash (o = e->remote.node) % r->lanes]) ? 0 : IER(1712)) ? (r->valid = MUGGLE(80)) : 0)
		  return 0;
		if ((p->gruntled != PORT_MAGIC) ? IER(1713) : 0)
		  return 0;
		if (p->reachable ? (! _cru_member (o, p->reachable)) : 0)
		  continue;
		if ((p == s) ? 0 : (pthread_rwlock_rdlock (&(p->p_lock)) ? IER(1714) : 0) ? (p->gruntled = MUGGLE(81)) : 0)
		  return 0;
		if (! (*unvisitable = p->disabled))
		  visited = _cru_member (o, p->visited);
		if ((p == s) ? 0 : (pthread_rwlock_unlock (&(p->p_lock)) ? IER(1715) : 0) ? (p->gruntled = MUGGLE(82)) : 0)
		  return 0;
	 }
...
{
  if ((! s) ? IER(1716) : (s->gruntled != PORT_MAGIC) ? IER(1717) : ! all_visited (e, s->local, s, unvisitable, err))
	 return 0;
  if ((pthread_mutex_lock (&(s->suspension)) ? IER(1718) : 0) ? (s->gruntled = MUGGLE(83)) : 0)
	 return 1;
  if (s->backoff)
	 s->backoff = 1;
  if (pthread_mutex_unlock (&(s->suspension)) ? IER(1719) : 0)
	 s->gruntled = MUGGLE(84);
  return 1;
}
	  ...
	  // is always implied if the traversal order is unconstrained.
{
  ...
  if ((! s) ? IER(1720) : (s->gruntled != PORT_MAGIC) ? IER(1721) : 0)
	 return 0;
  if ((! n) ? IER(1722) : (! (r = s->local)) ? IER(1723) : (r->valid != ROUTER_MAGIC) ? IER(1724) : 0)
	 return 0;
...
{
  return ((n ? 0 : IER(1725)) ? 0 : visited (n->edges_out, s, unvisitable, err));
}
