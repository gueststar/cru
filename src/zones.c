	  ...
	  // switch during testing to simulate other possible errors.
{
  ...
  result = NULL;
  killed = found = dblx = 0;
  if ((! source) ? IER(1680) : (source->gruntled != PORT_MAGIC) ? IER(1681) : 0)
	 return NULL;
  if ((!(r = source->local)) ? IER(1682) : (r->valid != ROUTER_MAGIC) ? IER(1683) : 0)
	 return NULL;
  z = &(r->ro_plan.zone);
  if ((! (d = source->peers)) ? IER(1684) : (e = r->ro_sig.orders.v_order.equal) ? *err : IER(1685))
	 {
  ...
  for (incoming = NULL; incoming ? incoming : (incoming = _cru_exchanged (source, d, err));)
	 {
		...
		if (killed ? 1 : found)
		  goto a;
		if ((n = (node_list) incoming->payload) ? (_cru_test_and_set_membership (n, &seen, err) ? 1 : *err) : IER(1686))
		  goto a;
	  ...
	  // co-workers by adding every incoming packet's payload to it.
{
  ...
  sample = 0;
  killed = 0;
  if ((! source) ? IER(1687) : (source->gruntled != PORT_MAGIC) ? IER(1688) : 0)
	 return NULL;
  if ((! (r = source->local)) ? IER(1689) : (r->valid != ROUTER_MAGIC) ? IER(1690) : 0)
	 return NULL;
  if (! (destinations = source->peers) ? IER(1691) : *err)
  ...
  for (incoming = NULL; incoming ? incoming : (incoming = _cru_exchanged (source, destinations, err));)
	 {
		KILL_SITE(31);
		killed = (killed ? 1 : KILLED);
		if (killed ? 1 : (n = (node_list) incoming->payload) ? 0 : IER(1692))
		  goto a;
	  ...
	  // filtering and mutation.
{
  ...
  if ((! r) ? 1 : (r->valid != ROUTER_MAGIC) ? IER(1693) : 0)
	 return NULL;
  z = &((w = &(r->ro_plan))->zone);
	  ...
	  // unreachable nodes.
{
  ...
  if ((! r) ? IER(1694) : (r->valid != ROUTER_MAGIC) ? IER(1695) : 0)
	 return 0;
  if (((! r->ports) ? IER(1696) : (! (r->lanes)) ? IER(1697) : 0) ? (r->valid = MUGGLE(75)) : 0)
	 return 0;
  if ((! unvisitable) ? IER(1698) : *unvisitable ? IER(1699) : 0)
	 return 0;
  for (visited = 1; (! e) ? 0 : *unvisitable ? 0 : visited; e = e->next_edge)
	 {
		if (((p = r->ports[_cru_scalar_hash (o = e->remote.node) % r->lanes]) ? 0 : IER(1700)) ? (r->valid = MUGGLE(76)) : 0)
		  return 0;
		if ((p->gruntled != PORT_MAGIC) ? IER(1701) : 0)
		  return 0;
		if (p->reachable ? (! _cru_member (o, p->reachable)) : 0)
		  continue;
		if ((p == s) ? 0 : (pthread_rwlock_rdlock (&(p->p_lock)) ? IER(1702) : 0) ? (p->gruntled = MUGGLE(77)) : 0)
		  return 0;
		if (! (*unvisitable = p->disabled))
		  visited = _cru_member (o, p->visited);
		if ((p == s) ? 0 : (pthread_rwlock_unlock (&(p->p_lock)) ? IER(1703) : 0) ? (p->gruntled = MUGGLE(78)) : 0)
		  return 0;
	 }
...
{
  if ((! s) ? IER(1704) : (s->gruntled != PORT_MAGIC) ? IER(1705) : ! all_visited (e, s->local, s, unvisitable, err))
	 return 0;
  if ((pthread_mutex_lock (&(s->suspension)) ? IER(1706) : 0) ? (s->gruntled = MUGGLE(79)) : 0)
	 return 1;
  if (s->backoff)
	 s->backoff = 1;
  if (pthread_mutex_unlock (&(s->suspension)) ? IER(1707) : 0)
	  ...
	  // is always implied if the traversal order is unconstrained.
{
  ...
  if ((! s) ? IER(1708) : (s->gruntled != PORT_MAGIC) ? IER(1709) : 0)
	 return 0;
  if ((! n) ? IER(1710) : (! (r = s->local)) ? IER(1711) : (r->valid != ROUTER_MAGIC) ? IER(1712) : 0)
	 return 0;
...
{
  return ((n ? 0 : IER(1713)) ? 0 : visited (n->edges_out, s, unvisitable, err));
}
