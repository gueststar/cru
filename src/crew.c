	  ...
	  // Initialize pthread resources.
{
  ...
  if (! _cru_error_checking_mutex_type (&a, err))
	 goto a;
  if (pthread_mutex_init (&crew_lock, &a) ? IER(696) : 0)
	 {
		pthread_mutexattr_destroy (&a);
		goto a;
	 }
  if (! (pthread_mutexattr_destroy (&a) ? IER(697) : 0))
	 return 1;
	  ...
	  // Release pthread resources.
{
  ...
  for (c = reserve_crews; (t = ((c == NULL_CREW) ? NULL : c)); _cru_free (t))
	 {
		if (crew_count-- ? 0 : (err = THE_IER(698)))
		  break;
		c = c->c_held;
	 }
  if (err ? 0 : crew_count)
	  ...
	  // Create a crew or borrow it from the reserves if possible.
{
  ...
  if ((c = crew_of (err)) ? 1 : pthread_mutex_lock (&(crew_lock)) ? IER(701) : 0)
	 goto a;
  if ((reserve_crews == NULL_CREW) ? 1 : crew_count ? 0 : IER(702))
	 goto b;
  c = reserve_crews;
	  ...
	  // reserve.
{
  ...
  if (! c)
	 return;
  if ((! (c->c_held)) ? 1 : pthread_mutex_lock (&crew_lock) ? IER(704) : 0)
	 goto a;
  if ((crew_count < RESERVE_CREW_LIMIT) ? 0 : pthread_mutex_unlock (&crew_lock) ? IER(705) : 1)
	 goto a;
  for (r = &reserve_crews; *r != NULL_CREW; r = &((*r)->c_held))
	 if ((c == *r) ? IER(706) : *r ? 0 : IER(707))                  // check for a double-free
		goto b;
  crew_count++;
  *r = c;
 b: if (pthread_mutex_unlock (&crew_lock))
	  ...
	  // Return the current number of crews held in reserve.
{
  ...
  if (pthread_mutex_lock (&crew_lock) ? IER(709) : 0)
	 return 0;
  result = crew_count;
	  ...
	  // Create more reserve crews if necessary.
{
  ...
  if (pthread_mutex_lock (&crew_lock) ? IER(711) : 0)
	 return;
	  ...
	  // instructed to exit and wait only for synchronization.
{
  ...
  dblx = 0;
  if ((! r) ? IER(713) : (! c) ? IER(714) : (r->valid != ROUTER_MAGIC) ? IER(715) : (! (r->threads)) ? IER(716) : 0)
	 goto a;
  if ((r->ports ? 0 : IER(717)) ? (r->valid = MUGGLE(10)) : (r->threads ? 0 : IER(718)) ? (r->valid = MUGGLE(11)) : 0)
	 goto a;
  if ((pthread_mutex_lock (&(r->lock)) ? IER(719) : 0) ? (r->valid = MUGGLE(12)) : 0)
	 goto a;
  c->bays = r->lanes;
  c->ids = r->threads;
  for (c->created = 0; dblx ? 0 : (c->created < c->bays); (c->created)++)
	 if (_cru_create (&(c->ids[c->created]), m, r->ports[c->created], &dblx))
		break;
  r->running = c->created;
  if ((pthread_mutex_unlock (&(r->lock)) ? IER(720) : 0) ? (r->valid = MUGGLE(13)) : ! (c->created))
	 goto a;
	  ...
	  // assign at most one result, which will be an error code.
{
  ...
  disjunction = 0;
  if ((! c) ? 1 : (c->bays < c->created) ? IER(721) : (! (c->ids)) ? IER(722) : 0)
	 return *err;
  for (i = 0; i < c->created; i++)
	 if (pthread_join (c->ids[i], (void **) &status) ? (! IER(723)) : 1)
		{
	  ...
	  // Join with the crew and return the sum of their return values.
{
  ...
  if ((! c) ? 1 : (c->bays < c->created) ? IER(724) : (! (c->ids)) ? IER(725) : 0)
	 return 0;
  for (sum = i = 0; i < c->created; i++)
	 if (pthread_join (c->ids[i], (void **) &n) ? (! IER(726)) : 1)
		sum += n;
  free_crew (c, err);
  return sum;
}
	  ...
	  // base_bay.
{
  ...
  if (base)
	 *base = NULL;
  if ((! c) ? 1 : (c->bays < c->created) ? IER(727) : c->ids ? 0 : IER(728))
	 goto a;
  hit = 0;
  for (i = 0; i < c->created; i++)
	 {
		if (pthread_join (c->ids[i], (void **) &r) ? IER(729) : ! r)
		  continue;
...
#define NO_DESTRUCTOR NULL
  ...
  if (result)
	 *result = NULL;
  if ((! c) ? 1 : (c->bays < c->created) ? IER(731) : (! (c->ids)) ? IER(732) : (any = 0))
	 return;
  x = NULL;
  for (i = 0; i < c->created; i++)
	 if (pthread_join (c->ids[i], (void **) &x) ? (! IER(733)) : ! ! x)
		{
		  if (x->extant ? (result ? (any ? (! IER(734)) : ++any) : ! IER(735)) : 0)
			 *result = x->value;
		  RAISE(x->ma_status);
	  ...
	  // reduce their results.
{
  ...
  if (result)
	 *result = NULL;
  if ((! m) ? IER(736) : (! c) ? 1 : (c->bays < c->created) ? IER(737) : c->ids ? 0 : IER(738))
	 return;
  if (((d = m->m_free) != m->r_free) ? IER(739) : 0)
	 return;
  ...
  if (*err ? 1 : (! (x ? x->extant : 0)) ? 1 : result ? 0 : IER(740))
	 goto b;
  *err = x->ma_status;
  *result = x->value;
  x->value = NULL;
