	  ...
	  // once for each node received with a non-stretch class mark.
{
  ...
  if ((! n) ? IER(1540) : n->edges_in ? IER(1541) : 0)
	 return 0;
  l = NULL;
  if ((! s) ? IER(1542) : 0)
	 return 0;
  if (! _cru_scattered_by_hashes (n, s->orders.v_order.hash, d, BY_TERMINUS, err))
	 return 0;
  if ((! x) ? IER(1543) : (! (x->expander)) ? IER(1544) : 0)
	 return 0;
  if ((new_edge_map = _cru_get_edge_maps ()) ? 0 : IER(1545))
	 return 0;
  for (e = &(n->edges_out); *err ? NULL : *e;)
	 {
		if (((m = (*e)->remote.node)) ? 0 : IER(1546))
		  return 0;
		*new_edge_map = NULL;
	  ...
	  // previously detected.
{
  ...
  if (count ? (limit ? ((*count)++ >= limit) : 0) : ! IER(1547))
	 RAISE(CRU_INTOVF);
  if ((c = _cru_popped_packet (i, err)) ? 0 : IER(1548))
	 return NULL;
...
{
  if ((! i) ? IER(1549) : (! (i->carrier)) ? IER(1550) : (! (i->receiver)) ? IER(1551) : 0)
	 return;
  if ((! c) ? IER(1552) : (! (c->receiver)) ? IER(1553) : (i == c) ? IER(1554) : s ? 0 : IER(1555))
	 return;
...
overwrite (i, c, s, d, err)
	  ...
	  // class mark.
{
  if ((! i) ? IER(1556) : (! (i->receiver)) ? IER(1557) : (! c) ? IER(1558) : (! (c->receiver)) ? IER(1559) : s ? 0 : IER(1560))
	 return;
	  ...
	  // error.
{
  ...
  if ((! i) ? IER(1561) : (! *i) ? IER(1562) : 0)
	 return;
  if ((! b) ? IER(1563) : (! q) ? IER(1564) : (! d) ? IER(1565) : (! (n = (*i)->receiver)) ? IER(1566) : 0)
	 goto a;
...
{
#define UNEQUAL(a,b) \
((! b) ? (! IER(1567)) : (a == b->vertex) ? 0 : FAILED(r->ro_sig.orders.v_order.equal, a, b->vertex))
  ...
  if ((!(r = source->local)) ? IER(1570) : (r->valid != ROUTER_MAGIC) ? IER(1571) : 0)
	 goto b;
  if ((!(d = source->peers)) ? IER(1572) : (r->tag != EXT) ? IER(1573) : 0)
	 goto b;
  if ((r->ro_sig.orders.v_order.equal) ? 0 : IER(1574))
	 goto b;
  ...
  for (incoming = NULL; incoming ? incoming : (incoming = _cru_exchanged (source, d, err));)
	 {
		...
		if ((! (n = incoming->receiver)) ? IER(1575) : 0)
		  goto c;
	 ...
	 e: if (*c ? (! unequal) : 0)
		...
		if ((! *p) ? IER(1576) : ((*p)->receiver->class_mark != STRETCHED) ? 1 : ++changed ? 1 : IER(1577))
		  continue;
	 f: if (n->class_mark == STRETCHED)
		  graft (incoming, *c, &(r->ro_sig), err);
		else if (changed-- ? 1 : IER(1578))
	  ...
	  // port's list of survivors. Cf. pruning_task in filters.c.
{
  ...
  sample = 0;
  seen = NULL;
  if ((! source) ? IER(1579) : (source->gruntled != PORT_MAGIC) ? IER(1580) : (killed = 0))
	 return NULL;
  if ((!(r = source->local)) ? IER(1581) : (r->valid != ROUTER_MAGIC) ? IER(1582) : 0)
	 return NULL;
  s = &(r->ro_sig);
  if (((d = source->peers)) ? 0 : IER(1583))
	 return (node_queue) _cru_abort (source, d, err);
  for (incoming = NULL; incoming ? incoming : (incoming = _cru_exchanged (source, d, err));)
	 {
		KILL_SITE(29);
		killed = (killed ? 1 : KILLED);
		if (killed ? 1 : (! (n = incoming->receiver)) ? IER(1584) : _cru_test_and_set_membership (n, &seen, err) ? 1 : *err)
		  goto a;
	  ...
	  // router and block until finished.
{
  ...
  if (*err ? 1 : (! g) ? 1 : (! (b = g->base_node)) ? IER(1585) : 0)
	 goto a;
  if ((! r) ? IER(1586) : (r->valid != ROUTER_MAGIC) ? IER(1587) : (r->tag != EXT) ? IER(1588) : 0)
	 goto a;
  if ((! (r->ports)) ? IER(1589) : r->ro_sig.orders.v_order.hash ? 0 : IER(1590))
	 goto a;
