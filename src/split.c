	  ...
	  // at each call site to the test code.
{
  ...
  received = 0;
  if ((! n) ? IER(1504) : n->doppleganger ? IER(1505) : (! s) ? IER(1506) : (!(s->fissile)) ? IER(1507) : *err)
	 return;
  o = &(s->sp_ctops.outward);
  if ((! z) ? IER(1508) : ! PASSED(s->fissile, n->vertex, n->vertex_property))
	 goto a;
  if ((!(s->ana)) ? IER(1509) : (!(o->ana_labeler.tpred)) ? IER(1510) : (!(o->ana_labeler.top)) ? IER(1511) : 0)
	 return;
  if ((!(s->cata)) ? IER(1512) : (!(o->cata_labeler.tpred)) ? IER(1513) : (!(o->cata_labeler.top)) ? IER(1514) : 0)
	 return;
	  ...
	  // at each call site to the test code.
{
  ...
  if (*err ? 1 : (! n) ? IER(1516) : (! s) ? IER(1517) : (! d) ? IER(1518) : ! ((c = n->doppleganger)))
	 return;
  i = &(s->sp_ctops.inward);
  if ((! z) ? IER(1519) : (sender ? carrier : NULL) ? 0 : sender ? IER(1520) : carrier ? IER(1521) : 1)
	 return;
  if ((! (i->ana_labeler.tpred)) ? IER(1522) : (! (i->ana_labeler.top)) ? IER(1523) : 0)
	 return;
  if ((! (i->cata_labeler.tpred)) ? IER(1524) : (! (i->cata_labeler.top)) ? IER(1525) : 0)
	 return;
  l = carrier->label;
  a_label = c_label = NULL;
	  ...
	  // DISCONNECT, and CREATE instructions.
{
  ...
  sample = 0;
  seen = NULL;
  if ((! source) ? IER(1526) : (source->gruntled != PORT_MAGIC) ? IER(1527) : 0)
	 goto a;
  if ((!(r = source->local)) ? IER(1528) : (r->valid != ROUTER_MAGIC) ? IER(1529) : (killed = 0))
	 goto a;
  if ((!(d = source->peers)) ? IER(1530) : (r->tag != SPL) ? IER(1531) : 0)
  ...
  for (incoming = NULL; incoming ? incoming : (incoming = _cru_exchanged (source, d, err));)
	 {
		...
		if ((! (n = incoming->receiver)) ? IER(1532) : *err ? 1 : killed)
		  {
	  ...
	  // router and block until finished.
{
  ...
  if (*err ? 1 : (! g) ? IER(1533) : (! *g) ? 1 : (! (b = (*g)->base_node)) ? IER(1534) : 0)
	 goto a;
  if ((! r) ? IER(1535) : (r->valid != ROUTER_MAGIC) ? IER(1536) : (r->tag != SPL) ? IER(1537) : 0)
	 goto b;
  if ((! (r->ports)) ? IER(1538) : r->ro_sig.orders.v_order.hash ? 0 : IER(1539))
	 goto b;
