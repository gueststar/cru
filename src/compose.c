 	  ...
 	  // Compute the props associated with a single vertex in a graph.
{
  ...
  if (*err ? 1 : (! n) ? IER(630) : (! c) ? IER(631) : ! _cru_set_membership (n, visited, err))
	 return;
	  ...
	  // Free the guard values from one node in a graph.
{
  ...
  if ((! c) ? IER(632) : (! visited) ? IER(633) : (! n) ? IER(634) : ! _cru_member (n, *visited))
	 return;
  if (! (g = n->props))
	 return;
  n->props = NULL;
...
#define RECORDED(x) (_cru_member (x, seen) ? 1 : (*err ? 0 : ! killed) ? 0 : _cru_listed (x, source->survivors))
  ...
  if ((! source) ? IER(635) : (source->gruntled != PORT_MAGIC) ? IER(636) : 0)
	 return NULL;
  if ((! (r = source->local)) ? IER(637) : (r->valid != ROUTER_MAGIC) ? IER(638) : 0)
	 return NULL;
  if ((! (d = source->peers)) ? IER(639) : (r->tag != COM) ? IER(640) : 0)
  ...
  for (incoming = NULL; incoming ? incoming : (incoming = _cru_exchanged (source, d, err));)
	 {
		...
		if ((n = (node_list) incoming->payload) ? RECORDED(n) : IER(641))
		  goto a;
...
#define MARKED_FOR_DELETION(x,d) (_cru_member ((node_list) x, d) ? 1 : _cru_set_membership ((node_list) x, &(d), err))
  ...
  if ((! n) ? IER(642) : (! c) ? IER(643) : ! g)
	 return;
  if ((! (c->labeler.qpred)) ? IER(644) : (! (c->labeler.qop)) ? IER(645) : 0)
	 return;
	  ...
	  // them.
{
  ...
  sample = 0;
  killed = 0;
  if ((! s) ? IER(646) : (s->gruntled != PORT_MAGIC) ? IER(647) : ! _cru_pingback (s, err))
	 return NULL;
  if ((! (r = s->local)) ? IER(648) : (r->valid != ROUTER_MAGIC) ? IER(649) : (! (r->ports)) ? IER(650) : 0)
	 return NULL;
  if ((r->tag != COM) ? IER(651) : (! (r->lanes)) ? IER(652) : (s->own_index >= r->lanes) ? IER(653) : 0)
  ...
  for (; n; n = n->next_node)
	 {
		KILL_SITE(4);
		killed = (killed ? 1 : KILLED);
		if (killed ? 1 : *err ? 1 : n->edges_by ? IER(654) : 0)
		  continue;
		b = NULL;
		for (e = n->edges_out; *err ? NULL : e; e = e->next_edge)
	  ...
	  // deduplicate. Return non-zero if there are any new ones.
{
  ...
  if ((! n) ? IER(655) : (! o) ? IER(656) : ! (p = n->props))
	 return 0;
	  ...
	  // tasks, but need not communicate with them.
{
  ...
  killed = 0;
  changed = 0;
  if ((! s) ? IER(659) : (s->gruntled != PORT_MAGIC) ? IER(660) : ! _cru_pingback (s, err))
	 return NULL;
  if ((! (r = s->local)) ? IER(661) : (r->valid != ROUTER_MAGIC) ? IER(662) : 0)
	 return NULL;
  if ((r->tag != COM) ? IER(663) : 0)
	 return NULL;
	  ...
	  // block until finished.
{
  ...
  if (*err ? 1 : ! g)
	 goto a;
  if ((! r) ? IER(664) : (r->valid != ROUTER_MAGIC) ? IER(665) : (r->tag != COM) ? IER(666) : (! (r->ports)) ? IER(667) : 0)
	 goto a;
