	  ...
	  // Scatter and consume a list of packets prepared by the caller.
{
  ...
  if ((! t) ? IER(1478) : (! (t->pod)) ? IER(1479) : (! (t->arity)) ? IER(1480) : 0)
	 return;
	  ...
	  // return non-zero if successful
{
  ...
  if ((! t) ? IER(1481) : (! (t->arity)) ? IER(1482) : t->pod ? 0 : IER(1483))
	 return 0;
  for (; e; e = e->next_edge)
	 {
	  ...
	  // message, and whose carrier field stores a copy of the edge.
{
  ...
  if ((! n) ? IER(1484) : (! t) ? IER(1485) : (! (t->pod)) ? IER(1486) : (! (t->arity)) ? IER(1487) : 0)
	 return NULL;
	  ...
	  // message, and whose carrier field stores a copy of the edge.
{
  ...
  if ((! n) ? IER(1488) : (! t) ? IER(1489) : (!(t->pod)) ? IER(1490) : (! (t->arity)) ? IER(1491) : 0)
	 return;
	  ...
	  // Send a packet with a sender field to another worker.
{
  ...
  if ((! t) ? IER(1492) : (! (t->pod)) ? IER(1493) : (! (t->arity)) ? IER(1494) : 0)
	 return;
	  ...
	  // and receiver to a worker indexed by the hash of the receiver.
{
  ...
  if ((! t) ? IER(1495) : (! (t->pod)) ? IER(1496) : (! (t->arity)) ? IER(1497) : 0)
	 return 0;
...
#define MARKED(x) ((x == SEEN) ? 1 : (x == SCATTERED) ? 1 : (x == RETIRED) ? 1 : (x == STRETCHED))
  ...
  if ((! n) ? IER(1498) : (! t) ? IER(1499) : (!(t->pod)) ? IER(1500) : (! (t->arity)) ? IER(1501) : h ? 0 : IER(1502))
	 return 0;
  for (e = n->edges_out; e; e = e->next_edge)
	 if ((m = e->remote.node) ? 0 : IER(1503))
		return 0;
	  ...
	  // each outgoing edge from n and delete the edge if successful.
{
  ...
  if ((! n) ? IER(1504) : (! t) ? IER(1505) : (!(t->pod)) ? IER(1506) : (! (t->arity)) ? IER(1507) : h ? 0 : IER(1508))
	 return 1;
  for (e = &(n->edges_out); *e; _cru_free_edges_and_labels (d, _cru_popped_edge (e, err), err))
	 if ((m = (*e)->remote.node) ? 0 : IER(1509))
		return 1;
	  ...
	  // an error.
{
  ...
  if (e ? 0 : IER(1510))
	 return 0;
  if ((! t) ? IER(1511) : (! (t->arity)) ? IER(1512) : t->pod ? 0 : IER(1513))
	 goto a;
  for (; (! *e) ? 0 : (c = _cru_popped_edge (e, err)) ? 1 : ! IER(1514); _cru_free_edges_and_labels (d, c, err))
	 if (c->remote.node != n)
		{
	  ...
	  // its payload to the vertex at the other end of *e.
{
  ...
  if ((! h) ? IER(1515) : (! t) ? IER(1516) : (! (t->arity)) ? IER(1517) : (! (t->pod)) ? IER(1518) :  (! n) ? IER(1519) : 0)
	 return;
	  ...
	  // Scatter and consume the edges in a list of edge maps.
{
  ...
  if ((! h) ? IER(1520) : (! t) ? IER(1521) : (! (t->pod)) ? IER(1522) : (! (t->arity)) ? IER(1523) : 0)
	 return NULL;
  i = NULL;
  for (l = NULL; *err ? NULL : e;)
	 {
		if ((! (e->ante)) ? IER(1524) : (! (e->post)) ? IER(1525) : e->post->remote.node ? IER(1526) : *err)
		  break;
