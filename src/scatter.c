	  ...
	  // return non-zero if successful
{
  ...
  if ((! t) ? IER(1474) : (! (t->arity)) ? IER(1475) : t->pod ? 0 : IER(1476))
	 return 0;
  for (; e; e = e->next_edge)
	 {
	  ...
	  // message, and whose carrier field stores a copy of the edge.
{
  ...
  if ((! n) ? IER(1477) : (! t) ? IER(1478) : (! (t->pod)) ? IER(1479) : (! (t->arity)) ? IER(1480) : 0)
	 return NULL;
	  ...
	  // message, and whose carrier field stores a copy of the edge.
{
  ...
  if ((! n) ? IER(1481) : (! t) ? IER(1482) : (!(t->pod)) ? IER(1483) : (! (t->arity)) ? IER(1484) : 0)
	 return;
	  ...
	  // Send a packet with a sender field to another worker.
{
  ...
  if ((! t) ? IER(1485) : (! (t->pod)) ? IER(1486) : (! (t->arity)) ? IER(1487) : 0)
	 return;
	  ...
	  // and receiver to a worker indexed by the hash of the receiver.
{
  ...
  if ((! t) ? IER(1488) : (! (t->pod)) ? IER(1489) : (! (t->arity)) ? IER(1490) : 0)
	 return 0;
	  ...
	  // each outgoing edge from n and delete the edge if successful.
{
  ...
  if ((! n) ? IER(1497) : (! t) ? IER(1498) : (!(t->pod)) ? IER(1499) : (! (t->arity)) ? IER(1500) : h ? 0 : IER(1501))
	 return 1;
  for (e = &(n->edges_out); *e; _cru_free_edges_and_labels (d, _cru_popped_edge (e, err), err))
	 if ((m = (*e)->remote.node) ? 0 : IER(1502))
		return 1;
	  ...
	  // an error.
{
  ...
  if (e ? 0 : IER(1503))
	 return 0;
  if ((! t) ? IER(1504) : (! (t->arity)) ? IER(1505) : t->pod ? 0 : IER(1506))
	 goto a;
  for (; (! *e) ? 0 : (c = _cru_popped_edge (e, err)) ? 1 : ! IER(1507); _cru_free_edges_and_labels (d, c, err))
	 if (c->remote.node != n)
		{
	  ...
	  // its payload to the vertex at the other end of *e.
{
  ...
  if ((! h) ? IER(1508) : (! t) ? IER(1509) : (! (t->arity)) ? IER(1510) : (! (t->pod)) ? IER(1511) :  (! n) ? IER(1512) : 0)
	 return;
	  ...
	  // Scatter and consume the edges in a list of edge maps.
{
  ...
  if ((! h) ? IER(1513) : (! t) ? IER(1514) : (! (t->pod)) ? IER(1515) : (! (t->arity)) ? IER(1516) : 0)
	 return NULL;
  i = NULL;
  for (l = NULL; *err ? NULL : e;)
	 {
		if ((! (e->ante)) ? IER(1517) : (! (e->post)) ? IER(1518) : e->post->remote.node ? IER(1519) : *err)
		  break;
