	  ...
	  // Scatter and consume a list of packets prepared by the caller.
{
  ...
  if ((! t) ? IER(1459) : (! (t->pod)) ? IER(1460) : (! (t->arity)) ? IER(1461) : 0)
	 return;
	  ...
	  // return non-zero if successful
{
  ...
  if ((! t) ? IER(1462) : (! (t->arity)) ? IER(1463) : t->pod ? 0 : IER(1464))
	 return 0;
  for (; e; e = e->next_edge)
	 {
	  ...
	  // message, and whose carrier field stores a copy of the edge.
{
  ...
  if ((! n) ? IER(1469) : (! t) ? IER(1470) : (!(t->pod)) ? IER(1471) : (! (t->arity)) ? IER(1472) : 0)
	 return;
	  ...
	  // and receiver to a worker indexed by the hash of the receiver.
{
  ...
  if ((! t) ? IER(1476) : (! (t->pod)) ? IER(1477) : (! (t->arity)) ? IER(1478) : 0)
	 return 0;
...
#define MARKED(x) ((x == SEEN) ? 1 : (x == SCATTERED) ? 1 : (x == RETIRED) ? 1 : (x == STRETCHED))
  ...
  if ((! n) ? IER(1479) : (! t) ? IER(1480) : (!(t->pod)) ? IER(1481) : (! (t->arity)) ? IER(1482) : h ? 0 : IER(1483))
	 return 0;
  for (e = &(n->edges_out); *e; e = &((*e)->next_edge))
	 if ((m = (*e)->remote.node) ? 0 : IER(1484))
		return 0;
	  ...
	  // each outgoing edge from n and delete the edge if successful.
{
  ...
  if ((! n) ? IER(1485) : (! t) ? IER(1486) : (!(t->pod)) ? IER(1487) : (! (t->arity)) ? IER(1488) : h ? 0 : IER(1489))
	 return 1;
  for (e = &(n->edges_out); *e; _cru_free_edges_and_labels (d, _cru_popped_edge (e, err), err))
	 if ((m = (*e)->remote.node) ? 0 : IER(1490))
		return 1;
	  ...
	  // an error.
{
  ...
  if (e ? 0 : IER(1491))
	 return 0;
  if ((! t) ? IER(1492) : (! (t->arity)) ? IER(1493) : t->pod ? 0 : IER(1494))
	 goto a;
  for (; (! *e) ? 0 : (c = _cru_popped_edge (e, err)) ? 1 : ! IER(1495); _cru_free_edges_and_labels (d, c, err))
	 if (c->remote.node != n)
		{
	  ...
	  // its payload to the vertex at the other end of *e.
{
  ...
  if ((! h) ? IER(1496) : (! t) ? IER(1497) : (! (t->arity)) ? IER(1498) : (! (t->pod)) ? IER(1499) :  (! n) ? IER(1500) : 0)
	 return;
	  ...
	  // Scatter and consume the edges in a list of edge maps.
{
  ...
  if ((! h) ? IER(1501) : (! t) ? IER(1502) : (! (t->pod)) ? IER(1503) : (! (t->arity)) ? IER(1504) : 0)
	 return NULL;
  i = NULL;
  for (l = NULL; *err ? NULL : e;)
	 {
		if ((! (e->ante)) ? IER(1505) : (! (e->post)) ? IER(1506) : e->post->remote.node ? IER(1507) : *err)
		  break;
