...
int
_cru_scattered_to_unmarked (e, t, err)
	  edge_list e;
	  packet_pod t;
	  int *err;

	  // Send an empty initial packet to another worker for each edge
	  // connected to an unmarked node and return non-zero if successful.
{
  packet_list p;
  node_list n;
  uintptr_t q;
	 ...
	 if (((n = e->remote.node)) ? (! _cru_read (&(n->marked), err)) : ! IER(1459))
		{
		  if (! (p = _cru_initial_packet_of (n, q = _cru_scalar_hash (n), err)))
			 success = 0;
		  else if (! _cru_push_packet (p, &(t->pod[q % t->arity]), err))
			 success = 0;
		}
  return success;
}
