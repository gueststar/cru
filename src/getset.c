...
{
  if (pthread_key_create (&edge_storage, NULL) ? IER(963) : 0)
	 return 0;
  if (pthread_key_create (&killed_storage, NULL) ? IER(964) : 0)
	 goto a;
  if (pthread_key_create (&destructors_storage, NULL) ? IER(965) : 0)
	 goto b;
  if (pthread_key_create (&edge_map_storage, NULL) ? IER(966) : 0)
	 goto c;
  if (pthread_key_create (&context_storage, NULL) ? IER(967) : 0)
	 goto d;
  return 1;
	  ...
	  // Store a context in the thread specific storage area for contexts.
{
  if (NOMEM)
	 return *err;
  return (pthread_setspecific (context_storage, (void *) c) ? IER(973) : 0);
}
	  ...
	  // Store an edge list in the thread specific storage area for edge lists.
{
  if (NOMEM)
	 return *err;
  return (pthread_setspecific (edge_storage, (void *) edge) ? IER(974) : 0);
}
	  ...
	  // Store an edge map in the thread specific storage area for edge maps.
{
  if (NOMEM)
	 return *err;
  return (pthread_setspecific (edge_map_storage, (void *) pair) ? IER(975) : 0);
}
	  ...
	  // Store a destructors in the thread specific storage area for destructorss.
{
  if (NOMEM)
	 return *err;
  return (pthread_setspecific (destructors_storage, (void *) destructors) ? IER(976) : 0);
}
	  ...
	  // kill switches.
{
  if (NOMEM)
	 return *err;
  return (pthread_setspecific (killed_storage, (void *) killed) ? IER(977) : 0);
}
