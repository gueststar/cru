	  ...
	  // starts successfully.
{
  ...
  started = dblx = 0;
  if ((! r) ? 1 : (r->valid != ROUTER_MAGIC) ? IER(1043) : r->ports ? 0 : IER(1044))
	 goto a;
	  ...
	  // the status disjunction.
{
  ...
  started = dblx = 0;
  if ((! r) ? IER(1045) : (r->valid != ROUTER_MAGIC) ? IER(1046) : 0)
	 return 0;
  if (((! (r->ports)) ? IER(1047) : (! (r->lanes)) ? IER(1048) : 0) ? (r->valid = MUGGLE(27)) : 0)
	 return 0;
  if (! _cru_crewed (c = _cru_crew_of (&dblx), (runner) _cru_status_runner, r, &dblx))
	 goto a;
	  ...
	  // receiver of the initial packet to the initial node.
{
  ...
  started = dblx = 0;
  if ((! r) ? 1 : (r->valid != ROUTER_MAGIC) ? IER(1049) : count ? 0 : IER(1050))
	 goto a;
  if (((! (r->ports)) ? IER(1051) : (! (r->lanes)) ? IER(1052) : 0) ? (r->valid = MUGGLE(28)) : 0)
	 goto a;
  *count = 0;
	  ...
	  // the node list.
{
  ...
  started = dblx = 0;
  if ((! r) ? IER(1053) : (r->valid != ROUTER_MAGIC) ? IER(1054) : 0)
	 return 0;
  if (((! (r->ports)) ? IER(1055) : (! (r->lanes)) ? IER(1056) : 0) ? (r->valid = MUGGLE(29)) : 0)
	 return 0;
  if ((! i) ? IER(1057) : (! u) ? IER(1058) : *u ? IER(1059) : 0)
	 return 0;
	  ...
	  // maybe types.
{
  ...
  dblx = started = 0;
  if ((! result) ? IER(1060) : 0)
	 goto a;
  *result = NULL;
  if ((! r) ? IER(1061) : (r->valid != ROUTER_MAGIC) ? IER(1062) : 0)
	 goto a;
  if (((! (r->ports)) ? IER(1063) : (! (r->lanes)) ? IER(1064) : 0) ? (r->valid = MUGGLE(30)) : 0)
	 return 0;
	  ...
	  // workers return maybe types.
{
  ...
  dblx = started = 0;
  if ((! result) ? IER(1065) : 0)
	 goto a;
  *result = NULL;
  if ((! r) ? IER(1066) : (r->valid != ROUTER_MAGIC) ? IER(1067) : (r->tag != MAP) ? IER(1068) : 0)
	 goto a;
  if (((! (r->ports)) ? IER(1069) : (! (r->lanes)) ? IER(1070) : 0) ? (r->valid = MUGGLE(31)) : 0)
	 return 0;
  if (! _cru_crewed (c = _cru_crew_of (&dblx), (runner) _cru_maybe_runner, r, &dblx))
	 goto b;
  if ((p = _cru_initial_packet_of (i, q = _cru_scalar_hash (i), &dblx)))
	  ...
	  // base v. Consume v and the router.
{
  ...
  if ((! r) ? IER(1071) : (r->valid != ROUTER_MAGIC) ? IER(1072) : 0)
	 return 0;
  if (((! (r->ports)) ? IER(1073) : (! (r->lanes)) ? IER(1074) : 0) ? (r->valid = MUGGLE(32)) : 0)
	 return 0;
  if ((! g) ? IER(1075) : *g ? IER(1076) : 0)
	 return 0;
  if (*err)
	 goto a;
