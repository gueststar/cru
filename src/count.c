	  ...
	  // Count the vertices in a graph co-operatively with other workers.
{
  ...
  sample = 0;
  seen = NULL;
  if ((! s) ? IER(682) : (s->gruntled != PORT_MAGIC) ? IER(683) : 0)
	 return NULL;
  if ((! (r = s->local)) ? IER(684) : (r->valid != ROUTER_MAGIC) ? IER(685) : 0)
	 return NULL;
  if (((d = s->peers)) ? 0 : IER(686))
	 return _cru_abort (s, d, err);
  for (incoming = NULL; incoming ? incoming : (incoming = _cru_exchanged (s, d, err));)
	 {
		KILL_SITE(6);
		killed = (killed ? 1 : KILLED);
		if ((n = (node_list) incoming->payload) ? (_cru_test_and_set_membership (n, &seen, err) ? 1 : *err) : IER(687))
		  goto a;
		if (killed ? 1 : *err)
		  goto a;
	  ...
	  // Count the edges in a graph co-operatively with other workers.
{
  ...
  killed = 0;
  seen = NULL;
  if ((! s) ? IER(689) : (s->gruntled != PORT_MAGIC) ? IER(690) : 0)
	 return NULL;
  if ((! (r = s->local)) ? IER(691) : (r->valid != ROUTER_MAGIC) ? IER(692) : 0)
	 return NULL;
  if (((d = s->peers)) ? 0 : IER(693))
	 return _cru_abort (s, d, err);
  for (incoming = NULL; incoming ? incoming : (incoming = _cru_exchanged (s, d, err));)
	 {
		KILL_SITE(7);
		killed = (killed ? 1 : KILLED);
		if ((n = (node_list) incoming->payload) ? (_cru_test_and_set_membership (n, &seen, err) ? 1 : *err) : IER(694))
		  goto a;
		if (killed ? 1 : *err)
		  goto a;
		_cru_scatter_out (n, d, err);
