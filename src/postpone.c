	  ...
	  // edges.
{
  ...
  if ((! n) ? IER(1318) : (! postponable) ? IER(1319) : 0)
	 return;
	  ...
	  // at each call site to the test code.
{
  ...
  if ((! s) ? IER(1321) : (! (s->bpred)) ? IER(1322) : (! (s->bop)) ? IER(1323) : (! l) ? IER(1324) : (! z) ? IER(1325) : 0)
	 return 0;
	  ...
	  // stationary ones, and leave the rest where they are.
{
  ...
  if ((! n) ? IER(1326) : (! p) ? IER(1327) : (! z) ? IER(1328) : *err)
	 return;
	  ...
	  // to workers responding to adjacent nodes.
{
  ...
  if ((! source) ? IER(1329) : (source->gruntled != PORT_MAGIC) ? IER(1330) : 0)
	 goto a;
  if ((!(r = source->local)) ? IER(1331) : (r->valid != ROUTER_MAGIC) ? IER(1332) : (killed = 0))
	 goto a;
  if ((!(d = source->peers)) ? IER(1333) : (r->tag != POS) ? IER(1334) : 0)
  ...
  for (incoming = NULL; incoming ? incoming : (incoming = _cru_exchanged (source, d, err));)
	 {
		...
		if ((n = incoming->receiver) ? 0 : IER(1335))
		  goto b;
...
#define ALL_EDGES(x) _cru_cat_edges (x->edges_postponed, x->edges_out)
  ...
  if ((! source) ? IER(1336) : (source->gruntled != PORT_MAGIC) ? IER(1337) : 0)
	 goto a;
  if ((!(r = source->local)) ? IER(1338) : (r->valid != ROUTER_MAGIC) ? IER(1339) : (killed = 0))
	 goto a;
  if ((!(d = source->peers)) ? IER(1340) : (r->tag != POS) ? IER(1341) : 0)
	 goto b;
  for (incoming = NULL; incoming ? incoming : (incoming = _cru_exchanged (source, d, err));)
	 {
		KILL_SITE(25);
		killed = (killed ? 1 : KILLED);
		if ((! (n = incoming->receiver)) ? IER(1342) : _cru_member (n, seen))
		  goto c;
	  ...
	  // further changes are detected.
{
  ...
  if (*err ? 1 : (! g) ? IER(1343) : (! *g))
	 goto a;
  b = (*g)->base_node;
  if ((! r) ? IER(1344) : (r->valid != ROUTER_MAGIC) ? IER(1345) : (r->tag != POS) ? IER(1346) : 0)
	 goto b;
  if (_cru_half_duplex (*g, err))
	 goto c;
  if ((z = _cru_razing_router (&(r->ro_sig.destructors), (task) _cru_half_duplexing_task, r->lanes, err)) ? 0 : IER(1347))
	 goto b;
