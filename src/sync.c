	  ...
	  // signal a transition indicating quiescence.
{
  ...
  if (--(r->running) ? 0 : (r->quiescent = 1))
	 if ((pthread_cond_signal (&(r->transition)) ? IER(1684) : 0) ? (r->valid = MUGGLE(64)) : 0)
		{
		  pthread_mutex_unlock (&(r->lock));
		  return 0;
		}
  if ((pthread_mutex_unlock (&(r->lock)) ? IER(1685) : 0) ? (r->valid = MUGGLE(65)) : 0)
	 return 0;
  return 1;
}
...
#define NEW_ASSIGNMENT(x) (x->assigned ? 0 : ((x->assigned = x->deferred)) ? (!(x->deferred = NULL)) : 0)
  ...
  if ((! r) ? IER(1686) : (r->valid != ROUTER_MAGIC) ? IER(1687) : r->ports ? 0 : IER(1688))
	 return;
  for (i = 0; i < r->lanes; i++)
	 {
		if ((!(p = r->ports[i])) ? IER(1689) : (p->gruntled != PORT_MAGIC) ? IER(1690) : 0)
		  continue;
		if ((pthread_mutex_lock (&(p->suspension)) ? IER(1691) : 0) ? (p->gruntled = MUGGLE(66)) : 0)
		  continue;
		if (NEW_ASSIGNMENT(p) ? p->waiting : 0)
		  {
			 _cru_swell (r, err);
			 if (pthread_cond_signal (&(p->resumable)) ? IER(1692) : 0)
				p->gruntled = MUGGLE(67);
		  }
		if (pthread_mutex_unlock (&(p->suspension)) ? IER(1693) : 0)
	  ...
	  // Wait until all workers are suspended.
{
  if ((! r) ? IER(1694) : (r->valid != ROUTER_MAGIC) ? IER(1695) : 0)
	 return;
  if ((pthread_mutex_lock (&(r->lock)) ? IER(1696) : 0) ? (r->valid = MUGGLE(69)) : 0)
	 return;
  _cru_enable_killing (k, &(r->killed), err);
  if (r->quiescent ? 0 : pthread_cond_wait (&(r->transition), &(r->lock)) ? IER(1697) : 0)
	 r->valid = MUGGLE(70);
  else if (pthread_mutex_unlock (&(r->lock)) ? IER(1698) : 0)
	 r->valid = MUGGLE(71);
	  ...
	  // should be quiescent at this point.
{
  ...
  if ((! r) ? IER(1699) : (r->valid != ROUTER_MAGIC) ? IER(1700) : 0)
	 return;
  if ((! (r->lanes)) ? IER(1701) : (! (r->ports)) ? IER(1702) : 0)
	 return;
  for (i = 0; i < r->lanes; i++)
	 if (! ((! (source = r->ports[i])) ? IER(1703) : (source->gruntled == PORT_MAGIC) ? 0 : IER(1704)))
		{
		  if ((pthread_mutex_lock (&(source->suspension)) ? IER(1705) : 0) ? (source->gruntled = MUGGLE(72)) : 0)
			 continue;
		  if (source->assigned ? IER(1706) : 0)
			 _cru_nack (source->assigned, err);
		  source->assigned = NULL;
		  if (source->waiting ? (pthread_cond_signal (&(source->resumable)) ? IER(1707) : 0) : ! (source->dismissed = 1))
			 source->gruntled = MUGGLE(73);
		  if (pthread_mutex_unlock (&(source->suspension)) ? IER(1708) : 0)
