...
{
  if ((! extant_class) ? IER(1135) : (! incoming) ? IER(1136) : (! (incoming->receiver)) ? IER(1137) : 0)
	 return 0;
  if ((incoming == extant_class) ? IER(1138) : extant_class->receiver ? 0 : IER(1139))
	 return 0;
	  ...
	  // error recovery.
{
  ...
  if ((! c) ? IER(1140) : (! s) ? IER(1141) : 0)
	 return;
  for (; incoming; incoming = incoming->next_packet)
	 if (incoming->receiver ? 1 : ! IER(1142))
		{
		  if (*err ? 1 : (c->me_kernel.v_op.vertex.map != _cru_undefined_top) ? 1 : incoming->receiver->next_node ? IER(1143) : 0)
			 {
...
{
#define UNEQUAL(a,b) \
(*err ? 0 : (! b) ? (! IER(1144)) : (a->vertex_property == b->vertex_property) ? 0 : \
...
#define RECORDED(x) (_cru_member (x, seen) ? 1 : (*err ? 0 : ! killed) ? 0 : _cru_listed (x, s->deletions))
  ...
  if ((! s) ? IER(1145) : (s->gruntled != PORT_MAGIC) ? IER(1146) : 0)
	 goto a;
  if ((!(r = s->local)) ? IER(1147) : (r->valid != ROUTER_MAGIC) ? IER(1148) : (killed = 0))
	 goto a;
  if ((! (d = s->peers)) ? IER(1149) : (r->tag != CLU) ? IER(1150) : 0)
	 goto b;
  if (r->merger.me_classifier.cl_order.equal ? 0 : IER(1151))
	 goto b;
  ...
  for (incoming = NULL; incoming ? incoming : (incoming = _cru_exchanged (s, d, err));)
	 {
		...
		if ((! (n = incoming->receiver)) ? IER(1152) : RECORDED(n))
		  goto c;
		...
		if ((*c ? (! unequal) : 0) ? (assimilated (incoming, *c, err) ? 1 : IER(1153)) : 0)
		  goto c;
		p = (*c ? &((*c)->next_packet) : c);
		if ((((*p = _cru_popped_packet (&incoming, err))) ? (n = _cru_half_severed ((*p)->receiver)) : NULL) ? 1 : ! IER(1154))
	  ...
	  // destructor in the vertex fold of the merger's property prop.
{
  ...
  if ((! s) ? IER(1155) : (s->gruntled != PORT_MAGIC) ? IER(1156) : (! c) ? IER(1157) : (! r) ? IER(1158) : 0)
	 return;
  memset (&z, 0, sizeof (z));
  z.e_free = r->e_free;
	  ...
	  // particular Petri net reachability graph optimization.
{
  ...
  q = NULL;
  if ((! (l = edges)) ? IER(1159) : 0)
	 goto a;
  if ((! f) ? 1 : (! s--) ? IER(1160) : (! s) ? 0 : (! o) ? IER(1161) : (h = o->hash) ? 0 : IER(1162))
	 goto a;
  while (*l)
	 {
	  ...
	  // a class, and consume the members' edges.
{
  ...
  t = NULL;
  e = NULL;
  if ((! c) ? IER(1163) : (! o) ? IER(1164) : (! n) ? IER(1165) : 0)
	 return NULL;
  v = n->vertex;
		...
		for (q = NULL; n->edges_out;)
		  {
			 ...
			 if (*err ? 0 : (! (c->pruner)) ? 0 : ! ((! e) ? IER(1166) : (! (o->hash)) ? IER(1167) : 0))
	  ...
	  // them.
{
  ...
  killed = 0;
  sample = 0;
  if ((! s) ? IER(1168) : (s->gruntled != PORT_MAGIC) ? IER(1169) : ! _cru_pingback (s, err))
	 return NULL;
  if ((! (r = s->local)) ? IER(1170) : (r->valid != ROUTER_MAGIC) ? IER(1171) : (r->tag != CLU) ? IER(1172) : 0)
	 return NULL;
  z = &(r->ro_sig);
  d = &(z->destructors);
	  ...
	  // graph.
{
  ...
  if ((! s) ? IER(1173) : (s->gruntled != PORT_MAGIC) ? IER(1174) : (! c) ? IER(1175) : 0)
	 return;
  memset (&z, 0, sizeof (z));
  z.v_free = c->me_kernel.v_op.vertex.r_free;
	  ...
	  // concurrently with other workers in the event of an error.
{
  ...
  if ((! s) ? IER(1176) : (s->gruntled != PORT_MAGIC) ? IER(1177) : ! _cru_pingback (s, err))
	 return NULL;
  if ((! (r = s->local)) ? IER(1178) : (r->valid != ROUTER_MAGIC) ? IER(1179) : (r->tag != CLU) ? IER(1180) : 0)
	 return NULL;
	  ...
	  // third pass.
{
  ...
  if ((! r) ? IER(1181) : (r->valid != ROUTER_MAGIC) ? IER(1182) : (! g) ? IER(1183) : (! (g->base_node)) ? IER(1184) : *err)
	 return;
  if (((! (r->ports)) ? IER(1185) : (! (r->lanes)) ? IER(1186) : 0) ? (r->valid = MUGGLE(33)) : s ? 0 : IER(1187))
	 return;
  if ((r->tag != CLU) ? IER(1188) : (! (h = r->merger.me_classifier.cl_order.hash)) ? IER(1189) : 0)
	 return;
	  ...
	  // original.
{
  ...
  if (*err ? 1 : ! g)
	 goto a;
  if ((! r) ? IER(1190) : (r->valid != ROUTER_MAGIC) ? IER(1191) : (r->tag != CLU) ? IER(1192) : (! (r->ports)) ? IER(1193) : 0)
	 goto a;
