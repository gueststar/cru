...
free_edge (e, err)
	  ...
	  // b.
{
  if ((! e) ? FAIL(4903) : (e->e_magic != EDGE_MAGIC) ? FAIL(4904) : 0)
	 return;
  e->e_magic = MUGGLE(202);
  if (pthread_mutex_lock (&edge_lock) ? FAIL(4905) : edge_count-- ? 0 : FAIL(4906))
	 return;
  if (pthread_mutex_unlock (&edge_lock) ? FAIL(4907) : 0)
	 return;
  free (e);
}
	  ...
	  // but is needed only for composing and not building.
{
  ...
  if ((! e) ? GLOBAL_FAIL(4908) : (e->e_magic != EDGE_MAGIC) ? GLOBAL_FAIL(4909) : 0)
	 return 0;
  u = e->e_value;
...
{
  if ((! a) ? FAIL(4910) : (a->e_magic != EDGE_MAGIC) ? FAIL(4911) : 0)
	 return 0;
  if ((! b) ? FAIL(4912) : (b->e_magic != EDGE_MAGIC) ? FAIL(4913) : 0)
	 return 0;
...
{
  if ((! a) ? FAIL(4923) : (a->v_magic != VERTEX_MAGIC) ? FAIL(4924) : 0)
	 return 0;
  if ((! b) ? FAIL(4925) : (b->v_magic != VERTEX_MAGIC) ? FAIL(4926) : 0)
	 return 0;
	  ...
	  // in the builder b.
{
  ...
  if ((! v) ? GLOBAL_FAIL(4927) : (v->v_magic != VERTEX_MAGIC) ? GLOBAL_FAIL(4928) : 0)
	 return 0;
  u = v->v_value;
  for (i = 0; i < sizeof (u); i++)  // ignore overflow
	 u += (u + 1) << 8;
	  ...
	  // the builder b.
{
  ...
  if ((! given_vertex) ? FAIL(4929) : (given_vertex->v_magic != VERTEX_MAGIC) ? FAIL(4930) : 0)
	 return;
	  ...
	  // numbered edges.
{
  ...
  if (*err ? 1 : (! connecting_edge) ? FAIL(4931) : (connecting_edge->e_magic != EDGE_MAGIC) ? FAIL(4932) : 0)
	 return NULL;
  if ((! local_vertex) ? FAIL(4933) : (local_vertex->v_magic != VERTEX_MAGIC) ? FAIL(4934) : 0)
	 return NULL;
  if ((! remote_vertex) ? FAIL(4935) : (remote_vertex->v_magic != VERTEX_MAGIC) ? FAIL(4936) : 0)
	 return NULL;
...
vertex_checker (edges_in, given_vertex, edges_out, err)
	  ...
	  // edges being valid.
{
  if (*err ? 1 : (! given_vertex) ? FAIL(4937) : (given_vertex->v_magic != VERTEX_MAGIC) ? FAIL(4938) : 0)
	 return NULL;
  if ((! edges_in) ? FAIL(4939) : (edges_in->r_magic != REDEX_MAGIC) ? FAIL(4940) : 0)
	 return NULL;
  if ((! edges_out) ? FAIL(4941) : (edges_out->r_magic != REDEX_MAGIC) ? FAIL(4942) : 0)
	 return NULL;
  if ((edges_in->r_value != DEGREE) ? FAIL(4943) : 0)
	 return NULL;
  if ((edges_out->r_value != DEGREE) ? FAIL(4944) : 0)
	 return NULL;
  return redex_of (1, err);
}
	  ...
	  // errors.
{
  ...
  if (*err)
	 return NULL;
  if ((! l) ? 0 : (l->r_magic != REDEX_MAGIC) ? FAIL(4945) : 0)
	 return NULL;
  if ((! r) ? 0 : (r->r_magic != REDEX_MAGIC) ? FAIL(4946) : 0)
	 return NULL;
  s = (l ? l->r_value : 0) + (r ? r->r_value : 0);
  if ((s < (l ? l->r_value : 0)) ? FAIL(4947) : (s < (r ? r->r_value : 0)) ? FAIL(4948) : 0)
	 return NULL;
	  ...
	  // the hard way by using redexes for numbers.
{
  ...
  if ((cru_vertex_count (g, LANES, err) == NUMBER_OF_VERTICES) ? 0 : FAIL(4949))
	 return 0;
  if ((cru_edge_count (g, LANES, err) == (DEGREE * NUMBER_OF_VERTICES)) ? 0 : FAIL(4950))
	 return 0;
  if ((r = (redex) cru_mapreduced (g, &m, UNKILLABLE, LANES, err)) ? 0 : FAIL(4951))
	 return 0;
  if ((r->r_magic == REDEX_MAGIC) ? 0 : FAIL(4952))
	 return 0;
	  ...
	  int *err;
{
  ...
  if (lock_created ? (*lock_created = 0) : 1)
	 FAIL(4955);
  if (*err ? 1 : pthread_mutexattr_init (&mutex_attribute) ? FAIL(4956) : 0)
	 return 0;
  if (!((pthread_mutexattr_settype (&mutex_attribute, PTHREAD_MUTEX_ERRORCHECK)) ? FAIL(4957) : 0))
	 *lock_created = ! (pthread_mutex_init (lock, &mutex_attribute) ? FAIL(4958) : 0);
  return ! ((pthread_mutexattr_destroy (&mutex_attribute)) ? FAIL(4959) : *err);
}
...
{
  if ((! l) ? FAIL(4960) : (l->e_magic != EDGE_MAGIC) ? FAIL(4961) : 0)
	 return 0;
...
{
  if ((! l) ? FAIL(4962) : (l->e_magic != EDGE_MAGIC) ? FAIL(4963) : 0)
	 return 0;
  if ((! r) ? FAIL(4964) : (r->e_magic != EDGE_MAGIC) ? FAIL(4965) : 0)
	 return 0;
...
{
  if ((! o) ? FAIL(4966) : (o->v_magic != VERTEX_MAGIC) ? FAIL(4967) : 0)
	 return 0;
  if ((! t) ? FAIL(4968) : (t->v_magic != VERTEX_MAGIC) ? FAIL(4969) : 0)
	 return 0;
  if ((! l) ? FAIL(4970) : (l->e_magic != EDGE_MAGIC) ? FAIL(4971) : 0)
	 return 0;
  return ! (l->e_value & 1);
}
	  ...
	  char **argv;
{
  ...
  nthm_sync (&err);
  if (! err)
	 err = (edge_count ? THE_FAIL(4972) : vertex_count ? THE_FAIL(4973) : redex_count ? THE_FAIL(4974) : global_err);
