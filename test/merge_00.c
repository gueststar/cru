	  ...
	  // Create a redex.
{
  ...
  if ((r = (redex) malloc (sizeof (*r))) ? 0 : FAIL(3586))
	 return NULL;
  memset (r, 0, sizeof (*r));
  r->r_magic = REDEX_MAGIC;
  r->r_value = n;
  if (pthread_mutex_lock (&redex_lock) ? FAIL(3587) : ++redex_count ? 0 : FAIL(3588))
	 goto a;
  if (! (pthread_mutex_unlock (&redex_lock) ? FAIL(3589) : 0))
	 return r;
 a: free (r);
  return NULL;
}
...
free_redex (r, err)
	  ...
	  // Reclaim a redex.
{
  if ((! r) ? FAIL(3590) : (r->r_magic != REDEX_MAGIC) ? FAIL(3591) : 0)
	 return;
...
{
  if ((! r) ? FAIL(3595) : (r->r_magic != PROPERTY_MAGIC) ? FAIL(3596) : 0)
	 return;
  r->r_magic = MUGGLE(146);
  if (pthread_mutex_lock (&redex_lock) ? FAIL(3597) : redex_count-- ? 0 : FAIL(3598))
	 return;
  if (pthread_mutex_unlock (&redex_lock) ? FAIL(3599) : 0)
	 return;
  free (r);
}
...
{
  if ((! a) ? FAIL(3600) : (a->r_magic != PROPERTY_MAGIC) ? FAIL(3601) : 0)
	 return 0;
  if ((! b) ? FAIL(3602) : (b->r_magic != PROPERTY_MAGIC) ? FAIL(3603) : 0)
	 return 0;
	  ...
	  // cru_hash is passed as c.me_classifier.cl_order.hash in the merger c.
{
  ...
  if ((! r) ? GLOBAL_FAIL(3604) : (r->r_magic != PROPERTY_MAGIC) ? GLOBAL_FAIL(3605) : 0)
	 return 0;
  u = r->r_value;
	  ...
	  // Create an edge.
{
  ...
  if ((e = (edge) malloc (sizeof (*e))) ? 0 : FAIL(3606))
	 return NULL;
  memset (e, 0, sizeof (*e));
...
free_edge (e, err)
	  ...
	  // b.
{
  if ((! e) ? FAIL(3610) : (e->e_magic != EDGE_MAGIC) ? FAIL(3611) : 0)
	 return;
	  ...
	  // but is needed only for merging and not building.
{
  ...
  if ((! e) ? GLOBAL_FAIL(3615) : (e->e_magic != EDGE_MAGIC) ? GLOBAL_FAIL(3616) : 0)
	 return 0;
  u = e->e_value;
  for (i = 0; i < sizeof (u); i++)  // ignore overflow
	 u += (u + 1) << 8;
...
{
  if ((! a) ? FAIL(3617) : (a->e_magic != EDGE_MAGIC) ? FAIL(3618) : 0)
	 return 0;
  if ((! b) ? FAIL(3619) : (b->e_magic != EDGE_MAGIC) ? FAIL(3620) : 0)
	 return 0;
	  ...
	  // Create a vertex.
{
  ...
  if ((v = (vertex) malloc (sizeof (*v))) ? 0 : FAIL(3621))
	 return NULL;
  memset (v, 0, sizeof (*v));
...
free_vertex (v, err)
	  ...
	  // b.
{
  if ((! v) ? FAIL(3625) : (v->v_magic != VERTEX_MAGIC) ? FAIL(3626) : 0)
	 return;
...
{
  if ((! v) ? FAIL(3630) : (v->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(3631) : 0)
	 return;
...
{
  if ((! a) ? FAIL(3635) : (a->v_magic != VERTEX_MAGIC) ? FAIL(3636) : 0)
	 return 0;
  if ((! b) ? FAIL(3637) : (b->v_magic != VERTEX_MAGIC) ? FAIL(3638) : 0)
	 return 0;
...
fused_equal_vertices (a, b, err)
	  ...
	  // equal.
{
  if ((! a) ? FAIL(3639) : (a->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(3640) : 0)
	 return 0;
  if ((! b) ? FAIL(3641) : (b->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(3642) : 0)
	 return 0;
  return a->v_value == b->v_value;
}
	  ...
	  // in the builder b.
{
  ...
  if ((! v) ? GLOBAL_FAIL(3643) : (v->v_magic != VERTEX_MAGIC) ? GLOBAL_FAIL(3644) : 0)
	 return 0;
  u = v->v_value;
  for (i = 0; i < sizeof (u); i++)  // ignore overflow
	 u += (u + 1) << 8;
	  ...
	  // Mix up low-entropy numeric values.
{
  ...
  if ((! v) ? GLOBAL_FAIL(3645) : (v->v_magic != FUSED_VERTEX_MAGIC) ? GLOBAL_FAIL(3646) : 0)
	 return 0;
  u = v->v_value;
  for (i = 0; i < sizeof (u); i++)  // ignore overflow
	 u += (u + 1) << 8;
	  ...
	  // the builder b.
{
  ...
  if ((! given_vertex) ? FAIL(3647) : (given_vertex->v_magic != VERTEX_MAGIC) ? FAIL(3648) : 0)
	 return;
...
{
  if (*err ? 1 : (! connecting_edge) ? FAIL(3649) : (connecting_edge->e_magic != EDGE_MAGIC) ? FAIL(3650) : 0)
	 return NULL;
  if ((! local_vertex) ? FAIL(3651) : (local_vertex->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(3652) : 0)
	 return NULL;
  if ((! remote_vertex) ? FAIL(3653) : (remote_vertex->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(3654) : 0)
	 return NULL;
  if ((local_vertex->v_value == remote_vertex->v_value) ? (! (connecting_edge->e_value)) : 0)
	 goto a;
  if ((((local_vertex->v_value ^ remote_vertex->v_value) << 1) != ( 1 << connecting_edge->e_value)) ? FAIL(3655) : 0)
	 return NULL;
...
vertex_checker (edges_in, given_vertex, edges_out, err)
	  ...
	  // edges being valid.
{
  if (*err ? 1 : (! given_vertex) ? FAIL(3656) : (given_vertex->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(3657) : 0)
	 return NULL;
  if ((! edges_in) ? FAIL(3658) : (edges_in->r_magic != REDEX_MAGIC) ? FAIL(3659) : 0)
	 return NULL;
  if ((! edges_out) ? FAIL(3660) : (edges_out->r_magic != REDEX_MAGIC) ? FAIL(3661) : 0)
	 return NULL;
  if ((edges_in->r_value != DIMENSION) ? FAIL(3662) : (edges_out->r_value != DIMENSION) ? FAIL(3663) : 0)
	 return NULL;
	  ...
	  // errors.
{
  ...
  if (*err)
	 return NULL;
  if ((! l) ? 0 : (l->r_magic != REDEX_MAGIC) ? FAIL(3664) : 0)
	 return NULL;
  if ((! r) ? 0 : (r->r_magic != REDEX_MAGIC) ? FAIL(3665) : 0)
	 return NULL;
  s = (l ? l->r_value : 0) + (r ? r->r_value : 0);
  if ((s < (l ? l->r_value : 0)) ? FAIL(3666) : (s < (r ? r->r_value : 0)) ? FAIL(3667) : 0)
	 return NULL;
	  ...
	  // the hard way by using redexes for numbers.
{
  ...
  if (*err)
	 return 0;
  if ((cru_vertex_count (g, LANES, err) == (NUMBER_OF_VERTICES >> 1)) ? 0 : FAIL(3668))
	 return 0;
  if ((cru_edge_count (g, LANES, err) == ((DIMENSION * NUMBER_OF_VERTICES) >> 1)) ? 0 : FAIL(3669))
	 return 0;
  if ((r = (redex) cru_mapreduced (g, &m, UNKILLABLE, LANES, err)) ? 0 : FAIL(3670))
	 return 0;
  if ((r->r_magic == REDEX_MAGIC) ? 0 : FAIL(3671))
	 return 0;
	  ...
	  // as c.me_kernel.v_op.vertex.map in the merger c.
{
  ...
  if (edges_in ? FAIL(3677) : edges_out ? FAIL(3678) : 0)
	 return NULL;
  if ((! given_vertex) ? FAIL(3679) : (given_vertex->v_magic != VERTEX_MAGIC) ? FAIL(3680) : 0)
	 return NULL;
	  ...
	  // merger c.
{
  ...
  if ((! a) ? FAIL(3681) : (a->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(3682) : 0)
	 return NULL;
  if ((! b) ? FAIL(3683) : (b->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(3684) : 0)
	 return NULL;
  if ((a->v_value != b->v_value) ? FAIL(3685) : 0)
	 return NULL;
	  ...
	  int *err;
{
  ...
  if (lock_created ? (*lock_created = 0) : 1)
	 FAIL(3687);
  if (*err ? 1 : pthread_mutexattr_init (&mutex_attribute) ? FAIL(3688) : 0)
	 return 0;
  if (!((pthread_mutexattr_settype (&mutex_attribute, PTHREAD_MUTEX_ERRORCHECK)) ? FAIL(3689) : 0))
	 *lock_created = ! (pthread_mutex_init (lock, &mutex_attribute) ? FAIL(3690) : 0);
  return ! ((pthread_mutexattr_destroy (&mutex_attribute)) ? FAIL(3691) : *err);
}
	  ...
	  char **argv;
{
  ...
  nthm_sync (&err);
  if (! err)
	 err = (edge_count ? THE_FAIL(3692) : vertex_count ? THE_FAIL(3693) : redex_count ? THE_FAIL(3694) : global_err);
