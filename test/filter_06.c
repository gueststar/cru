	  ...
	  // Create an edge.
{
  ...
  if ((e = (edge) malloc (sizeof (*e))) ? 0 : FAIL(3371))
	 return NULL;
  memset (e, 0, sizeof (*e));
  e->e_magic = EDGE_MAGIC;
  e->e_value = n;
  if (pthread_mutex_lock (&edge_lock) ? FAIL(3372) : ++edge_count ? 0 : FAIL(3373))
	 goto a;
  if (! (pthread_mutex_unlock (&edge_lock) ? FAIL(3374) : 0))
	 return e;
 a: free (e);
  return NULL;
}
...
free_edge (e, err)
	  ...
	  // b.
{
  if ((! e) ? FAIL(3375) : (e->e_magic != EDGE_MAGIC) ? FAIL(3376) : 0)
	 return;
  e->e_magic = MUGGLE(137);
  if (pthread_mutex_lock (&edge_lock) ? FAIL(3377) : edge_count-- ? 0 : FAIL(3378))
	 return;
  if (pthread_mutex_unlock (&edge_lock) ? FAIL(3379) : 0)
	 return;
  free (e);
}
...
{
  if ((! a) ? FAIL(3380) : (a->e_magic != EDGE_MAGIC) ? FAIL(3381) : 0)
	 return 0;
  if ((! b) ? FAIL(3382) : (b->e_magic != EDGE_MAGIC) ? FAIL(3383) : 0)
	 return 0;
	  ...
	  // in the builder b.
{
  ...
  if ((! e) ? GLOBAL_FAIL(3384) : (e->e_magic != EDGE_MAGIC) ? GLOBAL_FAIL(3385) : 0)
	 return 0;
  u = e->e_value;
	  ...
	  // Check for correct vertex counts, edge counts, and edge labels.
{
  ...
  if (cru_vertex_count (g, LANES, err) ? FAIL(3406) : 0)
	 return 0;
  if (cru_edge_count (g, LANES, err) ? FAIL(3407) : 0)
	 return 0;
  if (cru_mapreduced (g, &m, UNKILLABLE, LANES, err) ?  FAIL(3408) : 0)
	 return 0;
  return 1;
}
...
{
  if ((! given_vertex) ? FAIL(3409) : (given_vertex->v_magic != VERTEX_MAGIC) ? FAIL(3410) : 0)
	 return 0;
  return 0;
}
	  ...
	  int *err;
{
  ...
  if (lock_created ? (*lock_created = 0) : 1)
	 FAIL(3412);
  if (*err ? 1 : pthread_mutexattr_init (&mutex_attribute) ? FAIL(3413) : 0)
	 return 0;
  if (!((pthread_mutexattr_settype (&mutex_attribute, PTHREAD_MUTEX_ERRORCHECK)) ? FAIL(3414) : 0))
	 *lock_created = ! (pthread_mutex_init (lock, &mutex_attribute) ? FAIL(3415) : 0);
  return ! ((pthread_mutexattr_destroy (&mutex_attribute)) ? FAIL(3416) : *err);
}
	  ...
	  char **argv;
{
  ...
  nthm_sync (&err);
  if (! err)
	 err = (edge_count ? THE_FAIL(3417) : vertex_count ? THE_FAIL(3418) : global_err);
