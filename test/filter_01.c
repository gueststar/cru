	  ...
	  // Create an edge.
{
  ...
  if ((e = (edge) malloc (sizeof (*e))) ? 0 : FAIL(2893))
	 return NULL;
  memset (e, 0, sizeof (*e));
...
free_edge (e, err)
	  ...
	  // b.
{
  if ((! e) ? FAIL(2897) : (e->e_magic != EDGE_MAGIC) ? FAIL(2898) : 0)
	 return;
...
{
  if ((! a) ? FAIL(2911) : (a->v_magic != VERTEX_MAGIC) ? FAIL(2912) : 0)
	 return 0;
  if ((! b) ? FAIL(2913) : (b->v_magic != VERTEX_MAGIC) ? FAIL(2914) : 0)
	 return 0;
	  ...
	  // in the builder b.
{
  ...
  if ((! v) ? GLOBAL_FAIL(2915) : (v->v_magic != VERTEX_MAGIC) ? GLOBAL_FAIL(2916) : 0)
	 return 0;
  u = v->v_value;
	  ...
	  // the builder b.
{
  ...
  if ((! given_vertex) ? FAIL(2917) : (given_vertex->v_magic != VERTEX_MAGIC) ? FAIL(2918) : 0)
	 return;
...
{
  if (*err ? 1 : (! connecting_edge) ? FAIL(2919) : (connecting_edge->e_magic != EDGE_MAGIC) ? FAIL(2920) : 0)
	 return 0;
  if ((! local_vertex) ? FAIL(2921) : (local_vertex->v_magic != VERTEX_MAGIC) ? FAIL(2922) : 0)
	 return 0;
  if ((! remote_vertex) ? FAIL(2923) : (remote_vertex->v_magic != VERTEX_MAGIC) ? FAIL(2924) : 0)
	 return 0;
  return ! (((local_vertex->v_value ^ remote_vertex->v_value) != (1 << connecting_edge->e_value)) ? FAIL(2925) : 0);
}
	  ...
	  // edges being valid.
{
  ...
  if (*err ? 1 : (! given_vertex) ? FAIL(2926) : (given_vertex->v_magic != VERTEX_MAGIC) ? FAIL(2927) : 0)
	 return 0;
  v = (int) given_vertex->v_value;
  for (bits = 0; v; v >>= 1)
	 bits += (v & 1);
  if (bits < ((DIMENSION >> 1) - 1))
	 return ! ((edges_in != DIMENSION) ? FAIL(2928) : (edges_out != DIMENSION) ? FAIL(2929) : 0);
  return ! ((edges_in != bits) ? FAIL(2930) : (edges_out != bits) ? FAIL(2931) : 0);
}
	  ...
	  // errors.
{
  ...
  return ((*err ? 1 : ((s = l + r) < l) ? FAIL(2932) : (s < r) ? FAIL(2933) : 0) ? 0 : s);
}
...
#define REMAINING_EDGES ((REMAINING_VERTICES * DIMENSION) - DISCONNECTIONS)
  ...
  if ((cru_vertex_count (g, LANES, err) == REMAINING_VERTICES) ? 0 : FAIL(2934))
	 return 0;
  if ((cru_edge_count (g, LANES, err) == REMAINING_EDGES) ? 0 : FAIL(2935))
	 return 0;
  if ((((uintptr_t) cru_mapreduced (g, &m, UNKILLABLE, LANES, err)) == REMAINING_VERTICES) ? 0 : FAIL(2936))
	 return 0;
  return 1;
}
	  ...
	  // f.
{
  ...
  if ((! connecting_edge) ? FAIL(2937) : (connecting_edge->e_magic != EDGE_MAGIC) ? FAIL(2938) : 0)
	 return 0;
  if ((! local_vertex) ? FAIL(2939) : (local_vertex->v_magic != VERTEX_MAGIC) ? FAIL(2940) : 0)
	 return 0;
  if ((! adjacent_vertex) ? FAIL(2941) : (adjacent_vertex->v_magic != VERTEX_MAGIC) ? FAIL(2942) : 0)
	 return 0;
	  ...
	  char **argv;
{
  ...
  nthm_sync (&err);
  if (! err)
	 err = (edge_count ? THE_FAIL(2949) : vertex_count ? THE_FAIL(2950) : global_err);
