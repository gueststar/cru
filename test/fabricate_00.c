...
{
  return ! (*err ? 1 : ((local_vertex ^ remote_vertex) != (1 << connecting_edge)) ? FAIL(2652) : 0);
}
...
vertex_checker (edges_in, vertex, edges_out, err)
	  ...
	  // edges being valid.
{
  return ! (*err ? 1 : (edges_in != DIMENSION) ? FAIL(2653) : (edges_out != DIMENSION) ? FAIL(2654) : 0);
}
	  ...
	  // other errors.
{
  ...
  return ((*err ? 1 : ((s = l + r) < l) ? FAIL(2655) : (s < r) ? FAIL(2656) : 0) ? 0 : s);
}
	  ...
	  // Check for correct vertex counts, edge counts, and edge labels.
{
  ...
  if (((n = cru_vertex_count (g, LANES, err)) == NUMBER_OF_VERTICES) ? 0 : FAIL(2657))
	 return 0;
  if ((cru_edge_count (g, LANES, err) == DIMENSION * NUMBER_OF_VERTICES) ? 0 : FAIL(2658))
	 return 0;
  if ((((uintptr_t) cru_mapreduced (g, &m, UNKILLABLE, LANES, err)) == NUMBER_OF_VERTICES) ? 0 : FAIL(2659))
	 return 0;
  return 1;
}
