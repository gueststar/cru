	  ...
	  // Create a redex.
{
  ...
  if ((r = (redex) malloc (sizeof (*r))) ? 0 : FAIL(3844))
	 return NULL;
  memset (r, 0, sizeof (*r));
  r->r_magic = REDEX_MAGIC;
  r->r_value = n;
  if (pthread_mutex_lock (&redex_lock) ? FAIL(3845) : ++redex_count ? 0 : FAIL(3846))
	 goto a;
  if (! (pthread_mutex_unlock (&redex_lock) ? FAIL(3847) : 0))
	 return r;
 a: free (r);
  return NULL;
}
...
free_redex (r, err)
	  ...
	  // Reclaim a redex.
{
  if ((! r) ? FAIL(3848) : (r->r_magic != REDEX_MAGIC) ? FAIL(3849) : 0)
	 return;
  r->r_magic = MUGGLE(157);
  if (pthread_mutex_lock (&redex_lock) ? FAIL(3850) : redex_count-- ? 0 : FAIL(3851))
	 return;
  if (pthread_mutex_unlock (&redex_lock) ? FAIL(3852) : 0)
	 return;
  free (r);
}
...
free_mapex (r, err)
	  ...
	  // Reclaim a mapex.
{
  if ((! r) ? FAIL(3853) : (r->r_magic != MAPEX_MAGIC) ? FAIL(3854) : 0)
	 return;
  r->r_magic = REDEX_MAGIC;
  free_redex (r, err);
}
...
{
  if ((! r) ? FAIL(3855) : (r->r_magic != PROPERTY_MAGIC) ? FAIL(3856) : 0)
	 return;
  r->r_magic = MUGGLE(158);
  if (pthread_mutex_lock (&redex_lock) ? FAIL(3857) : redex_count-- ? 0 : FAIL(3858))
	 return;
  if (pthread_mutex_unlock (&redex_lock) ? FAIL(3859) : 0)
	 return;
  free (r);
}
...
{
  if ((! r) ? FAIL(3860) : (r->r_magic != INCIDENT_REDEX_MAGIC) ? FAIL(3861) : 0)
	 return;
  r->r_magic = MUGGLE(159);
  if (pthread_mutex_lock (&redex_lock) ? FAIL(3862) : redex_count-- ? 0 : FAIL(3863))
	 return;
  if (pthread_mutex_unlock (&redex_lock) ? FAIL(3864) : 0)
	 return;
  free (r);
}
...
{
  if ((! r) ? FAIL(3865) : (r->r_magic != INCIDENT_MAPEX_MAGIC) ? FAIL(3866) : 0)
	 return;
...
{
  if ((! r) ? FAIL(3867) : (r->r_magic != OUTGOING_REDEX_MAGIC) ? FAIL(3868) : 0)
	 return;
...
{
  if ((! a) ? FAIL(3872) : (a->r_magic != PROPERTY_MAGIC) ? FAIL(3873) : 0)
	 return 0;
  if ((! b) ? FAIL(3874) : (b->r_magic != PROPERTY_MAGIC) ? FAIL(3875) : 0)
	 return 0;
	  ...
	  // cru_hash is passed as c.me_classifier.cl_order.hash in the merger c.
{
  ...
  if ((! r) ? GLOBAL_FAIL(3876) : (r->r_magic != PROPERTY_MAGIC) ? GLOBAL_FAIL(3877) : 0)
	 return 0;
  u = r->r_value;
	  ...
	  // Create an edge.
{
  ...
  if ((e = (edge) malloc (sizeof (*e))) ? 0 : FAIL(3878))
	 return NULL;
  memset (e, 0, sizeof (*e));
  e->e_magic = EDGE_MAGIC;
  e->e_value = n;
  if (pthread_mutex_lock (&edge_lock) ? FAIL(3879) : ++edge_count ? 0 : FAIL(3880))
	 goto a;
  if (! (pthread_mutex_unlock (&edge_lock) ? FAIL(3881) : 0))
	 return e;
 a: free (e);
  return NULL;
}
...
free_edge (e, err)
	  ...
	  // b.
{
  if ((! e) ? FAIL(3882) : (e->e_magic != EDGE_MAGIC) ? FAIL(3883) : 0)
	 return;
  e->e_magic = MUGGLE(161);
  if (pthread_mutex_lock (&edge_lock) ? FAIL(3884) : edge_count-- ? 0 : FAIL(3885))
	 return;
  if (pthread_mutex_unlock (&edge_lock) ? FAIL(3886) : 0)
	 return;
  free (e);
}
	  ...
	  // but is needed only for merging and not building.
{
  ...
  if ((! e) ? GLOBAL_FAIL(3887) : (e->e_magic != EDGE_MAGIC) ? GLOBAL_FAIL(3888) : 0)
	 return 0;
  u = e->e_value;
  for (i = 0; i < sizeof (u); i++)  // ignore overflow
	 u += (u + 1) << 8;
...
{
  if ((! a) ? FAIL(3889) : (a->e_magic != EDGE_MAGIC) ? FAIL(3890) : 0)
	 return 0;
  if ((! b) ? FAIL(3891) : (b->e_magic != EDGE_MAGIC) ? FAIL(3892) : 0)
	 return 0;
	  ...
	  // Create a vertex.
{
  ...
  if ((v = (vertex) malloc (sizeof (*v))) ? 0 : FAIL(3893))
	 return NULL;
  memset (v, 0, sizeof (*v));
  v->v_magic = VERTEX_MAGIC;
  v->v_value = n;
  if (pthread_mutex_lock (&vertex_lock) ? FAIL(3894) : ++vertex_count ? 0 : FAIL(3895))
	 goto a;
  if (! (pthread_mutex_unlock (&vertex_lock) ? FAIL(3896) : 0))
	 return v;
 a: free (v);
  return NULL;
}
...
free_vertex (v, err)
	  ...
	  // b.
{
  if ((! v) ? FAIL(3897) : (v->v_magic != VERTEX_MAGIC) ? FAIL(3898) : 0)
	 return;
  v->v_magic = MUGGLE(162);
  if (pthread_mutex_lock (&vertex_lock) ? FAIL(3899) : vertex_count-- ? 0 : FAIL(3900))
	 return;
  if (pthread_mutex_unlock (&vertex_lock) ? FAIL(3901) : 0)
	 return;
  free (v);
}
...
{
  if ((! v) ? FAIL(3902) : (v->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(3903) : 0)
	 return;
  v->v_magic = MUGGLE(163);
  if (pthread_mutex_lock (&vertex_lock) ? FAIL(3904) : vertex_count-- ? 0 : FAIL(3905))
	 return;
  if (pthread_mutex_unlock (&vertex_lock) ? FAIL(3906) : 0)
	 return;
  free (v);
}
...
{
  if ((! a) ? FAIL(3907) : (a->v_magic != VERTEX_MAGIC) ? FAIL(3908) : 0)
	 return 0;
  if ((! b) ? FAIL(3909) : (b->v_magic != VERTEX_MAGIC) ? FAIL(3910) : 0)
	 return 0;
...
fused_equal_vertices (a, b, err)
	  ...
	  // equal.
{
  if ((! a) ? FAIL(3911) : (a->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(3912) : 0)
	 return 0;
  if ((! b) ? FAIL(3913) : (b->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(3914) : 0)
	 return 0;
  return a->v_value == b->v_value;
}
	  ...
	  // in the builder b.
{
  ...
  if ((! v) ? GLOBAL_FAIL(3915) : (v->v_magic != VERTEX_MAGIC) ? GLOBAL_FAIL(3916) : 0)
	 return 0;
  u = v->v_value;
  for (i = 0; i < sizeof (u); i++)  // ignore overflow
	 u += (u + 1) << 8;
	  ...
	  // Mix up low-entropy numeric values.
{
  ...
  if ((! v) ? GLOBAL_FAIL(3917) : (v->v_magic != FUSED_VERTEX_MAGIC) ? GLOBAL_FAIL(3918) : 0)
	 return 0;
  u = v->v_value;
  for (i = 0; i < sizeof (u); i++)  // ignore overflow
	 u += (u + 1) << 8;
	  ...
	  // the builder b.
{
  ...
  if ((! given_vertex) ? FAIL(3919) : (given_vertex->v_magic != VERTEX_MAGIC) ? FAIL(3920) : 0)
	 return;
...
{
  if (*err ? 1 : (! connecting_edge) ? FAIL(3921) : (connecting_edge->e_magic != EDGE_MAGIC) ? FAIL(3922) : 0)
	 return NULL;
  if ((! local_vertex) ? FAIL(3923) : (local_vertex->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(3924) : 0)
	 return NULL;
  if ((! remote_vertex) ? FAIL(3925) : (remote_vertex->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(3926) : 0)
	 return NULL;
  if ((((local_vertex->v_value ^ remote_vertex->v_value) << 1) != ( 1 << connecting_edge->e_value)) ? FAIL(3927) : 0)
	 return NULL;
...
vertex_checker (edges_in, given_vertex, edges_out, err)
	  ...
	  // edges being valid.
{
  if (*err ? 1 : (! given_vertex) ? FAIL(3928) : (given_vertex->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(3929) : 0)
	 return NULL;
  if ((! edges_in) ? FAIL(3930) : (edges_in->r_magic != REDEX_MAGIC) ? FAIL(3931) : 0)
	 return NULL;
  if ((! edges_out) ? FAIL(3932) : (edges_out->r_magic != REDEX_MAGIC) ? FAIL(3933) : 0)
	 return NULL;
  if ((edges_in->r_value != DIMENSION - 1) ? FAIL(3934) : (edges_out->r_value != DIMENSION - 1) ? FAIL(3935) : 0)
	 return NULL;
  return redex_of (1, err);
}
	  ...
	  // errors.
{
  ...
  if (*err)
	 return NULL;
  if ((! l) ? 0 : (l->r_magic != REDEX_MAGIC) ? FAIL(3936) : 0)
	 return NULL;
  if ((! r) ? 0 : (r->r_magic != REDEX_MAGIC) ? FAIL(3937) : 0)
	 return NULL;
  s = (l ? l->r_value : 0) + (r ? r->r_value : 0);
  if ((s < (l ? l->r_value : 0)) ? FAIL(3938) : (s < (r ? r->r_value : 0)) ? FAIL(3939) : 0)
	 return NULL;
	  ...
	  // errors.
{
  ...
  if (*err)
	 return NULL;
  if ((! l) ? 0 : (l->r_magic != INCIDENT_MAPEX_MAGIC) ? FAIL(3940) : 0)
	 return NULL;
  if ((! r) ? 0 : (r->r_magic != INCIDENT_REDEX_MAGIC) ? FAIL(3941) : 0)
	 return NULL;
  s = (l ? l->r_value : 0) + (r ? r->r_value : 0);
  if ((s < (l ? l->r_value : 0)) ? FAIL(3942) : (s < (r ? r->r_value : 0)) ? FAIL(3943) : 0)
	 return NULL;
  if ((t = redex_of (s, err)))
	  ...
	  // errors.
{
  ...
  if (*err)
	 return NULL;
  if ((! l) ? 0 : (l->r_magic != OUTGOING_REDEX_MAGIC) ? FAIL(3944) : 0)
	 return NULL;
  if ((! r) ? 0 : (r->r_magic != OUTGOING_REDEX_MAGIC) ? FAIL(3945) : 0)
	 return NULL;
  s = (l ? l->r_value : 0) + (r ? r->r_value : 0);
  if ((s < (l ? l->r_value : 0)) ? FAIL(3946) : (s < (r ? r->r_value : 0)) ? FAIL(3947) : 0)
	 return NULL;
  if ((t = redex_of (s, err)))
	  ...
	  // the hard way by using redexes for numbers.
{
  ...
  if (*err)
	 return 0;
  if ((cru_vertex_count (g, LANES, err) == (NUMBER_OF_VERTICES >> 1)) ? 0 : FAIL(3948))
	 return 0;
  if ((cru_edge_count (g, LANES, err) == (((DIMENSION - 1) * NUMBER_OF_VERTICES) >> 1)) ? 0 : FAIL(3949))
	 return 0;
  if ((r = (redex) cru_mapreduced (g, &m, UNKILLABLE, LANES, err)) ? 0 : FAIL(3950))
	 return 0;
  if ((r->r_magic == REDEX_MAGIC) ? 0 : FAIL(3951))
	 return 0;
...
#define D ((DIMENSION * (DIMENSION - 1)) >> 1)
  ...
  if (*err ? 1 : (! given_vertex) ? FAIL(3953) : (given_vertex->v_magic != VERTEX_MAGIC) ? FAIL(3954) : 0)
	 return NULL;
  if ((! edges_in) ? FAIL(3955) : (edges_in->r_magic != INCIDENT_REDEX_MAGIC) ? FAIL(3956) : 0)
	 return NULL;
  if ((! edges_out) ? FAIL(3957) : (edges_out->r_magic != OUTGOING_REDEX_MAGIC) ? FAIL(3958) : 0)
	 return NULL;
  if ((edges_in->r_value != D) ? FAIL(3959) : (edges_out->r_value != D) ? FAIL(3960) : 0)
	 return NULL;
	  ...
	  // Return an outgoing redex of the edge label.
{
  ...
  if (*err ? 1 : (! connecting_edge) ? FAIL(3967) : (connecting_edge->e_magic != EDGE_MAGIC) ? FAIL(3968) : 0)
	 return NULL;
  if ((! local_vertex) ? FAIL(3969) : (local_vertex->v_magic != VERTEX_MAGIC) ? FAIL(3970) : 0)
	 return NULL;
  if ((! remote_vertex) ? FAIL(3971) : (remote_vertex->v_magic != VERTEX_MAGIC) ? FAIL(3972) : 0)
	 return NULL;
  if ((r = redex_of (connecting_edge->e_value, err)))
	  ...
	  // as c.me_kernel.v_op.vertex.map in the merger c.
{
  ...
  if (edges_in ? FAIL(3973) : edges_out ? FAIL(3974) : 0)
	 return NULL;
  if ((! given_vertex) ? FAIL(3975) : (given_vertex->v_magic != VERTEX_MAGIC) ? FAIL(3976) : 0)
	 return NULL;
	  ...
	  // merger c.
{
  ...
  if ((! a) ? FAIL(3977) : (a->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(3978) : 0)
	 return NULL;
  if ((! b) ? FAIL(3979) : (b->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(3980) : 0)
	 return NULL;
  if ((a->v_value != b->v_value) ? FAIL(3981) : 0)
	 return NULL;
	  ...
	  int *err;
{
  ...
  if (lock_created ? (*lock_created = 0) : 1)
	 FAIL(3983);
  if (*err ? 1 : pthread_mutexattr_init (&mutex_attribute) ? FAIL(3984) : 0)
	 return 0;
  if (!((pthread_mutexattr_settype (&mutex_attribute, PTHREAD_MUTEX_ERRORCHECK)) ? FAIL(3985) : 0))
	 *lock_created = ! (pthread_mutex_init (lock, &mutex_attribute) ? FAIL(3986) : 0);
  return ! ((pthread_mutexattr_destroy (&mutex_attribute)) ? FAIL(3987) : *err);
}
...
{
  if ((! outgoing_edge) ? FAIL(3988) : (outgoing_edge->e_magic != EDGE_MAGIC) ? FAIL(3989) : (! everywhere) ? FAIL(3990) : 0)
	 return 0;
	  ...
	  char **argv;
{
  ...
  nthm_sync (&err);
  if (! err)
	 err = (edge_count ? THE_FAIL(3991) : vertex_count ? THE_FAIL(3992) : redex_count ? THE_FAIL(3993) : global_err);
