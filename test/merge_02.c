	  ...
	  // Create a redex.
{
  ...
  if ((r = (redex) malloc (sizeof (*r))) ? 0 : FAIL(3845))
	 return NULL;
  memset (r, 0, sizeof (*r));
  r->r_magic = REDEX_MAGIC;
  r->r_value = n;
  if (pthread_mutex_lock (&redex_lock) ? FAIL(3846) : ++redex_count ? 0 : FAIL(3847))
	 goto a;
  if (! (pthread_mutex_unlock (&redex_lock) ? FAIL(3848) : 0))
	 return r;
 a: free (r);
  return NULL;
}
...
free_redex (r, err)
	  ...
	  // Reclaim a redex.
{
  if ((! r) ? FAIL(3849) : (r->r_magic != REDEX_MAGIC) ? FAIL(3850) : 0)
	 return;
...
free_mapex (r, err)
	  ...
	  // Reclaim a mapex.
{
  if ((! r) ? FAIL(3854) : (r->r_magic != MAPEX_MAGIC) ? FAIL(3855) : 0)
	 return;
  r->r_magic = REDEX_MAGIC;
  free_redex (r, err);
}
...
{
  if ((! r) ? FAIL(3856) : (r->r_magic != PROPERTY_MAGIC) ? FAIL(3857) : 0)
	 return;
  r->r_magic = MUGGLE(158);
  if (pthread_mutex_lock (&redex_lock) ? FAIL(3858) : redex_count-- ? 0 : FAIL(3859))
	 return;
  if (pthread_mutex_unlock (&redex_lock) ? FAIL(3860) : 0)
	 return;
  free (r);
}
...
{
  if ((! r) ? FAIL(3861) : (r->r_magic != INCIDENT_REDEX_MAGIC) ? FAIL(3862) : 0)
	 return;
...
{
  if ((! r) ? FAIL(3866) : (r->r_magic != INCIDENT_MAPEX_MAGIC) ? FAIL(3867) : 0)
	 return;
...
{
  if ((! r) ? FAIL(3868) : (r->r_magic != OUTGOING_REDEX_MAGIC) ? FAIL(3869) : 0)
	 return;
  r->r_magic = MUGGLE(160);
  if (pthread_mutex_lock (&redex_lock) ? FAIL(3870) : redex_count-- ? 0 : FAIL(3871))
	 return;
  if (pthread_mutex_unlock (&redex_lock) ? FAIL(3872) : 0)
	 return;
  free (r);
}
...
{
  if ((! a) ? FAIL(3873) : (a->r_magic != PROPERTY_MAGIC) ? FAIL(3874) : 0)
	 return 0;
  if ((! b) ? FAIL(3875) : (b->r_magic != PROPERTY_MAGIC) ? FAIL(3876) : 0)
	 return 0;
	  ...
	  // cru_hash is passed as c.me_classifier.cl_order.hash in the merger c.
{
  ...
  if ((! r) ? GLOBAL_FAIL(3877) : (r->r_magic != PROPERTY_MAGIC) ? GLOBAL_FAIL(3878) : 0)
	 return 0;
  u = r->r_value;
	  ...
	  // Create an edge.
{
  ...
  if ((e = (edge) malloc (sizeof (*e))) ? 0 : FAIL(3879))
	 return NULL;
  memset (e, 0, sizeof (*e));
  e->e_magic = EDGE_MAGIC;
  e->e_value = n;
  if (pthread_mutex_lock (&edge_lock) ? FAIL(3880) : ++edge_count ? 0 : FAIL(3881))
	 goto a;
  if (! (pthread_mutex_unlock (&edge_lock) ? FAIL(3882) : 0))
	 return e;
 a: free (e);
  return NULL;
}
...
free_edge (e, err)
	  ...
	  // b.
{
  if ((! e) ? FAIL(3883) : (e->e_magic != EDGE_MAGIC) ? FAIL(3884) : 0)
	 return;
  e->e_magic = MUGGLE(161);
  if (pthread_mutex_lock (&edge_lock) ? FAIL(3885) : edge_count-- ? 0 : FAIL(3886))
	 return;
  if (pthread_mutex_unlock (&edge_lock) ? FAIL(3887) : 0)
	 return;
  free (e);
}
	  ...
	  // but is needed only for merging and not building.
{
  ...
  if ((! e) ? GLOBAL_FAIL(3888) : (e->e_magic != EDGE_MAGIC) ? GLOBAL_FAIL(3889) : 0)
	 return 0;
  u = e->e_value;
  for (i = 0; i < sizeof (u); i++)  // ignore overflow
	 u += (u + 1) << 8;
...
{
  if ((! a) ? FAIL(3890) : (a->e_magic != EDGE_MAGIC) ? FAIL(3891) : 0)
	 return 0;
  if ((! b) ? FAIL(3892) : (b->e_magic != EDGE_MAGIC) ? FAIL(3893) : 0)
	 return 0;
	  ...
	  // Create a vertex.
{
  ...
  if ((v = (vertex) malloc (sizeof (*v))) ? 0 : FAIL(3894))
	 return NULL;
  memset (v, 0, sizeof (*v));
  v->v_magic = VERTEX_MAGIC;
  v->v_value = n;
  if (pthread_mutex_lock (&vertex_lock) ? FAIL(3895) : ++vertex_count ? 0 : FAIL(3896))
	 goto a;
  if (! (pthread_mutex_unlock (&vertex_lock) ? FAIL(3897) : 0))
	 return v;
 a: free (v);
  return NULL;
}
...
free_vertex (v, err)
	  ...
	  // b.
{
  if ((! v) ? FAIL(3898) : (v->v_magic != VERTEX_MAGIC) ? FAIL(3899) : 0)
	 return;
  v->v_magic = MUGGLE(162);
  if (pthread_mutex_lock (&vertex_lock) ? FAIL(3900) : vertex_count-- ? 0 : FAIL(3901))
	 return;
  if (pthread_mutex_unlock (&vertex_lock) ? FAIL(3902) : 0)
	 return;
  free (v);
}
...
{
  if ((! v) ? FAIL(3903) : (v->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(3904) : 0)
	 return;
  v->v_magic = MUGGLE(163);
  if (pthread_mutex_lock (&vertex_lock) ? FAIL(3905) : vertex_count-- ? 0 : FAIL(3906))
	 return;
  if (pthread_mutex_unlock (&vertex_lock) ? FAIL(3907) : 0)
	 return;
  free (v);
}
...
{
  if ((! a) ? FAIL(3908) : (a->v_magic != VERTEX_MAGIC) ? FAIL(3909) : 0)
	 return 0;
  if ((! b) ? FAIL(3910) : (b->v_magic != VERTEX_MAGIC) ? FAIL(3911) : 0)
	 return 0;
...
fused_equal_vertices (a, b, err)
	  ...
	  // equal.
{
  if ((! a) ? FAIL(3912) : (a->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(3913) : 0)
	 return 0;
  if ((! b) ? FAIL(3914) : (b->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(3915) : 0)
	 return 0;
  return a->v_value == b->v_value;
}
	  ...
	  // in the builder b.
{
  ...
  if ((! v) ? GLOBAL_FAIL(3916) : (v->v_magic != VERTEX_MAGIC) ? GLOBAL_FAIL(3917) : 0)
	 return 0;
  u = v->v_value;
  for (i = 0; i < sizeof (u); i++)  // ignore overflow
	 u += (u + 1) << 8;
	  ...
	  // Mix up low-entropy numeric values.
{
  ...
  if ((! v) ? GLOBAL_FAIL(3918) : (v->v_magic != FUSED_VERTEX_MAGIC) ? GLOBAL_FAIL(3919) : 0)
	 return 0;
  u = v->v_value;
  for (i = 0; i < sizeof (u); i++)  // ignore overflow
	 u += (u + 1) << 8;
	  ...
	  // the builder b.
{
  ...
  if ((! given_vertex) ? FAIL(3920) : (given_vertex->v_magic != VERTEX_MAGIC) ? FAIL(3921) : 0)
	 return;
...
{
  if (*err ? 1 : (! connecting_edge) ? FAIL(3922) : (connecting_edge->e_magic != EDGE_MAGIC) ? FAIL(3923) : 0)
	 return NULL;
  if ((! local_vertex) ? FAIL(3924) : (local_vertex->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(3925) : 0)
	 return NULL;
  if ((! remote_vertex) ? FAIL(3926) : (remote_vertex->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(3927) : 0)
	 return NULL;
  if ((((local_vertex->v_value ^ remote_vertex->v_value) << 1) != ( 1 << connecting_edge->e_value)) ? FAIL(3928) : 0)
	 return NULL;
...
vertex_checker (edges_in, given_vertex, edges_out, err)
	  ...
	  // edges being valid.
{
  if (*err ? 1 : (! given_vertex) ? FAIL(3929) : (given_vertex->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(3930) : 0)
	 return NULL;
  if ((! edges_in) ? FAIL(3931) : (edges_in->r_magic != REDEX_MAGIC) ? FAIL(3932) : 0)
	 return NULL;
  if ((! edges_out) ? FAIL(3933) : (edges_out->r_magic != REDEX_MAGIC) ? FAIL(3934) : 0)
	 return NULL;
  if ((edges_in->r_value != DIMENSION - 1) ? FAIL(3935) : (edges_out->r_value != DIMENSION - 1) ? FAIL(3936) : 0)
	 return NULL;
  return redex_of (1, err);
}
	  ...
	  // errors.
{
  ...
  if (*err)
	 return NULL;
  if ((! l) ? 0 : (l->r_magic != REDEX_MAGIC) ? FAIL(3937) : 0)
	 return NULL;
  if ((! r) ? 0 : (r->r_magic != REDEX_MAGIC) ? FAIL(3938) : 0)
	 return NULL;
  s = (l ? l->r_value : 0) + (r ? r->r_value : 0);
  if ((s < (l ? l->r_value : 0)) ? FAIL(3939) : (s < (r ? r->r_value : 0)) ? FAIL(3940) : 0)
	 return NULL;
	  ...
	  // errors.
{
  ...
  if (*err)
	 return NULL;
  if ((! l) ? 0 : (l->r_magic != INCIDENT_MAPEX_MAGIC) ? FAIL(3941) : 0)
	 return NULL;
  if ((! r) ? 0 : (r->r_magic != INCIDENT_REDEX_MAGIC) ? FAIL(3942) : 0)
	 return NULL;
  s = (l ? l->r_value : 0) + (r ? r->r_value : 0);
  if ((s < (l ? l->r_value : 0)) ? FAIL(3943) : (s < (r ? r->r_value : 0)) ? FAIL(3944) : 0)
	 return NULL;
  if ((t = redex_of (s, err)))
	  ...
	  // errors.
{
  ...
  if (*err)
	 return NULL;
  if ((! l) ? 0 : (l->r_magic != OUTGOING_REDEX_MAGIC) ? FAIL(3945) : 0)
	 return NULL;
  if ((! r) ? 0 : (r->r_magic != OUTGOING_REDEX_MAGIC) ? FAIL(3946) : 0)
	 return NULL;
  s = (l ? l->r_value : 0) + (r ? r->r_value : 0);
  if ((s < (l ? l->r_value : 0)) ? FAIL(3947) : (s < (r ? r->r_value : 0)) ? FAIL(3948) : 0)
	 return NULL;
  if ((t = redex_of (s, err)))
	  ...
	  // the hard way by using redexes for numbers.
{
  ...
  if (*err)
	 return 0;
  if ((cru_vertex_count (g, LANES, err) == (NUMBER_OF_VERTICES >> 1)) ? 0 : FAIL(3949))
	 return 0;
  if ((cru_edge_count (g, LANES, err) == (((DIMENSION - 1) * NUMBER_OF_VERTICES) >> 1)) ? 0 : FAIL(3950))
	 return 0;
  if ((r = (redex) cru_mapreduced (g, &m, UNKILLABLE, LANES, err)) ? 0 : FAIL(3951))
	 return 0;
  if ((r->r_magic == REDEX_MAGIC) ? 0 : FAIL(3952))
	 return 0;
...
#define D ((DIMENSION * (DIMENSION - 1)) >> 1)
  ...
  if (*err ? 1 : (! given_vertex) ? FAIL(3954) : (given_vertex->v_magic != VERTEX_MAGIC) ? FAIL(3955) : 0)
	 return NULL;
  if ((! edges_in) ? FAIL(3956) : (edges_in->r_magic != INCIDENT_REDEX_MAGIC) ? FAIL(3957) : 0)
	 return NULL;
  if ((! edges_out) ? FAIL(3958) : (edges_out->r_magic != OUTGOING_REDEX_MAGIC) ? FAIL(3959) : 0)
	 return NULL;
  if ((edges_in->r_value != D) ? FAIL(3960) : (edges_out->r_value != D) ? FAIL(3961) : 0)
	 return NULL;
	  ...
	  // Return an incident redex of the edge label.
{
  ...
  if (*err ? 1 : (! connecting_edge) ? FAIL(3962) : (connecting_edge->e_magic != EDGE_MAGIC) ? FAIL(3963) : 0)
	 return NULL;
  if ((! local_vertex) ? FAIL(3964) : (local_vertex->v_magic != VERTEX_MAGIC) ? FAIL(3965) : 0)
	 return NULL;
  if ((! remote_vertex) ? FAIL(3966) : (remote_vertex->v_magic != VERTEX_MAGIC) ? FAIL(3967) : 0)
	 return NULL;
	  ...
	  // Return an outgoing redex of the edge label.
{
  ...
  if (*err ? 1 : (! connecting_edge) ? FAIL(3968) : (connecting_edge->e_magic != EDGE_MAGIC) ? FAIL(3969) : 0)
	 return NULL;
  if ((! local_vertex) ? FAIL(3970) : (local_vertex->v_magic != VERTEX_MAGIC) ? FAIL(3971) : 0)
	 return NULL;
  if ((! remote_vertex) ? FAIL(3972) : (remote_vertex->v_magic != VERTEX_MAGIC) ? FAIL(3973) : 0)
	 return NULL;
  if ((r = redex_of (connecting_edge->e_value, err)))
	  ...
	  // as c.me_kernel.v_op.vertex.map in the merger c.
{
  ...
  if (edges_in ? FAIL(3974) : edges_out ? FAIL(3975) : 0)
	 return NULL;
  if ((! given_vertex) ? FAIL(3976) : (given_vertex->v_magic != VERTEX_MAGIC) ? FAIL(3977) : 0)
	 return NULL;
	  ...
	  // merger c.
{
  ...
  if ((! a) ? FAIL(3978) : (a->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(3979) : 0)
	 return NULL;
  if ((! b) ? FAIL(3980) : (b->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(3981) : 0)
	 return NULL;
  if ((a->v_value != b->v_value) ? FAIL(3982) : 0)
	 return NULL;
	  ...
	  int *err;
{
  ...
  if (lock_created ? (*lock_created = 0) : 1)
	 FAIL(3984);
  if (*err ? 1 : pthread_mutexattr_init (&mutex_attribute) ? FAIL(3985) : 0)
	 return 0;
  if (!((pthread_mutexattr_settype (&mutex_attribute, PTHREAD_MUTEX_ERRORCHECK)) ? FAIL(3986) : 0))
	 *lock_created = ! (pthread_mutex_init (lock, &mutex_attribute) ? FAIL(3987) : 0);
  return ! ((pthread_mutexattr_destroy (&mutex_attribute)) ? FAIL(3988) : *err);
}
...
{
  if ((! outgoing_edge) ? FAIL(3989) : (outgoing_edge->e_magic != EDGE_MAGIC) ? FAIL(3990) : (! everywhere) ? FAIL(3991) : 0)
	 return 0;
	  ...
	  char **argv;
{
  ...
  nthm_sync (&err);
  if (! err)
	 err = (edge_count ? THE_FAIL(3992) : vertex_count ? THE_FAIL(3993) : redex_count ? THE_FAIL(3994) : global_err);
