	  ...
	  // Create a redex.
{
  ...
  if ((r = (redex) malloc (sizeof (*r))) ? 0 : FAIL(5069))
	 return NULL;
  memset (r, 0, sizeof (*r));
...
free_redex (r, err)
	  ...
	  // Reclaim a redex.
{
  if ((! r) ? FAIL(5073) : (r->r_magic != REDEX_MAGIC) ? FAIL(5074) : 0)
	 return;
  r->r_magic = MUGGLE(213);
  if (pthread_mutex_lock (&redex_lock) ? FAIL(5075) : redex_count-- ? 0 : FAIL(5076))
	 return;
  if (pthread_mutex_unlock (&redex_lock) ? FAIL(5077) : 0)
	 return;
  free (r);
}
...
free_mapex (r, err)
	  ...
	  // Reclaim a mapex.
{
  if ((! r) ? FAIL(5078) : (r->r_magic != MAPEX_MAGIC) ? FAIL(5079) : 0)
	 return;
  r->r_magic = REDEX_MAGIC;
  free_redex (r, err);
}
	  ...
	  // Create an edge.
{
  ...
  if ((e = (edge) malloc (sizeof (*e))) ? 0 : FAIL(5080))
	 return NULL;
  memset (e, 0, sizeof (*e));
...
free_edge (e, err)
	  ...
	  // b.
{
  if ((! e) ? FAIL(5084) : (e->e_magic != EDGE_MAGIC) ? FAIL(5085) : 0)
	 return;
  e->e_magic = MUGGLE(214);
  if (pthread_mutex_lock (&edge_lock) ? FAIL(5086) : edge_count-- ? 0 : FAIL(5087))
	 return;
  if (pthread_mutex_unlock (&edge_lock) ? FAIL(5088) : 0)
	 return;
  free (e);
}
	  ...
	  // but is needed only for merging and not building.
{
  ...
  if ((! e) ? GLOBAL_FAIL(5089) : (e->e_magic != EDGE_MAGIC) ? GLOBAL_FAIL(5090) : 0)
	 return 0;
  u = e->e_value;
	  ...
	  // Create a vertex.
{
  ...
  if ((v = (vertex) malloc (sizeof (*v))) ? 0 : FAIL(5095))
	 return NULL;
  memset (v, 0, sizeof (*v));
...
{
  if ((! v) ? FAIL(5104) : (v->v_magic != VERTEX_MAGIC) ? FAIL(5105) : 0)
	 return;
  v->v_magic = MUGGLE(216);
  if (pthread_mutex_lock (&vertex_lock) ? FAIL(5106) : vertex_count-- ? 0 : FAIL(5107))
	 return;
  if (pthread_mutex_unlock (&vertex_lock) ? FAIL(5108) : 0)
	 return;
  free (v);
}
	  ...
	  // in the builder b.
{
  ...
  if ((! v) ? GLOBAL_FAIL(5113) : (v->v_magic != VERTEX_MAGIC) ? GLOBAL_FAIL(5114) : 0)
	 return 0;
  u = v->v_value;
  for (i = 0; i < sizeof (u); i++)  // ignore overflow
	 u += (u + 1) << 8;
	  ...
	  // the builder b.
{
  ...
  if ((! given_vertex) ? FAIL(5115) : (given_vertex->v_magic != VERTEX_MAGIC) ? FAIL(5116) : 0)
	 return;
...
{
  if (*err ? 1 : (! connecting_edge) ? FAIL(5117) : (connecting_edge->e_magic != EDGE_MAGIC) ? FAIL(5118) : 0)
	 return NULL;
  if ((! local_vertex) ? FAIL(5119) : (local_vertex->v_magic != VERTEX_MAGIC) ? FAIL(5120) : 0)
	 return NULL;
  if ((! remote_vertex) ? FAIL(5121) : (remote_vertex->v_magic != VERTEX_MAGIC) ? FAIL(5122) : 0)
	 return NULL;
  if (((local_vertex->v_value ^ remote_vertex->v_value) != (1 << connecting_edge->e_value)) ? FAIL(5123) : 0)
	 return NULL;
	  ...
	  // Simulate a different result type.
{
  ...
  if (*err ? 1 : (! connecting_edge) ? FAIL(5124) : (connecting_edge->e_magic != EDGE_MAGIC) ? FAIL(5125) : 0)
	 return NULL;
  if ((! local_vertex) ? FAIL(5126) : (local_vertex->v_magic != VERTEX_MAGIC) ? FAIL(5127) : 0)
	 return NULL;
  if ((! remote_vertex) ? FAIL(5128) : (remote_vertex->v_magic != VERTEX_MAGIC) ? FAIL(5129) : 0)
	 return NULL;
...
vertex_checker (edges_in, given_vertex, edges_out, err)
	  ...
	  // edges being valid.
{
  if (*err ? 1 : (! given_vertex) ? FAIL(5131) : (given_vertex->v_magic != VERTEX_MAGIC) ? FAIL(5132) : 0)
	 return NULL;
  ...
  if ((! edges_out) ? FAIL(5135) : (edges_out->r_magic != REDEX_MAGIC) ? FAIL(5136) : 0)
	 return NULL;
  if ((edges_in->r_value != (DIMENSION << 1)) ? FAIL(5137) : (edges_out->r_value != (DIMENSION << 1)) ? FAIL(5138) : 0)
	 return NULL;
  return redex_of (1, err);
}
	  ...
	  // as above but sequential
{
  ...
  if (*err)
	 return NULL;
  if ((! l) ? 0 : (l->r_magic != MAPEX_MAGIC) ? FAIL(5143) : 0)
	 return NULL;
  if ((! r) ? 0 : (r->r_magic != REDEX_MAGIC) ? FAIL(5144) : 0)
	 return NULL;
  s = (l ? l->r_value : 0) + (r ? r->r_value : 0);
  if ((s < (l ? l->r_value : 0)) ? FAIL(5145) : (s < (r ? r->r_value : 0)) ? FAIL(5146) : 0)
	 return NULL;
  return redex_of (s, err);
}
	  ...
	  // the hard way by using redexes for numbers.
{
  ...
  if (*err ? 1 : (cru_vertex_count (g, LANES, err) == (NUMBER_OF_VERTICES << 1)) ? 0 : FAIL(5147))
	 return 0;
  if ((cru_edge_count (g, LANES, err) == ((DIMENSION << 1) * (NUMBER_OF_VERTICES << 1))) ? 0 : FAIL(5148))
	 return 0;
  if ((r = (redex) cru_mapreduced (g, &m, UNKILLABLE, LANES, err)) ? 0 : FAIL(5149))
	 return 0;
  if ((r->r_magic == REDEX_MAGIC) ? 0 : FAIL(5150))
	 return 0;
...
{
  if ((! edges_in) ? FAIL(5152) : (edges_in->r_magic != REDEX_MAGIC) ? FAIL(5153) : 0)
	 return NULL;
  if ((! edges_out) ? FAIL(5154) : (edges_out->r_magic != REDEX_MAGIC) ? FAIL(5155) : 0)
	 return NULL;
  if ((! (edges_in->r_value)) ? 0 : (edges_in->r_value != DIMENSION) ? FAIL(5156) : 0)
	 return NULL;
  if ((edges_out->r_value != DIMENSION) ? FAIL(5157) : 0)
	 return NULL;
  if ((! v) ? FAIL(5158) : (v->v_magic != VERTEX_MAGIC) ? FAIL(5159) : 0)
	 return NULL;
	  ...
	  // create a vertex numbered the same as the given redex.
{
  if ((! v) ? FAIL(5160) : (v->v_magic != VERTEX_MAGIC) ? FAIL(5161) : 0)
	 return NULL;
  if ((! r) ? FAIL(5162) : (r->r_magic != REDEX_MAGIC) ? FAIL(5163) : 0)
	 return NULL;
	  ...
	  // create a vertex numbered the same as the given redex.
{
  if ((! v) ? FAIL(5164) : (v->v_magic != VERTEX_MAGIC) ? FAIL(5165) : 0)
	 return NULL;
  if ((! r) ? FAIL(5166) : (r->r_magic != REDEX_MAGIC) ? FAIL(5167) : 0)
	 return NULL;
  return vertex_of (r->r_value, err);
}
...
{
  if ((! source) ? FAIL(5168) : (source->r_magic != REDEX_MAGIC) ? FAIL(5169) : 0)
	 return NULL;
  if ((! target) ? FAIL(5170) : (target->r_magic != REDEX_MAGIC) ? FAIL(5171) : 0)
	 return NULL;
  if ((! label) ? FAIL(5172) : (label->e_magic != EDGE_MAGIC) ? FAIL(5173) : 0)
	 return NULL;
	  ...
	  int *err;
{
  ...
  if (lock_created ? (*lock_created = 0) : 1)
	 FAIL(5175);
  if (*err ? 1 : pthread_mutexattr_init (&mutex_attribute) ? FAIL(5176) : 0)
	 return 0;
  if (!((pthread_mutexattr_settype (&mutex_attribute, PTHREAD_MUTEX_ERRORCHECK)) ? FAIL(5177) : 0))
	 *lock_created = ! (pthread_mutex_init (lock, &mutex_attribute) ? FAIL(5178) : 0);
  return ! ((pthread_mutexattr_destroy (&mutex_attribute)) ? FAIL(5179) : *err);
}
	  ...
	  char **argv;
{
  ...
  nthm_sync (&err);
  if (! err)
	 err = (edge_count ? THE_FAIL(5180) : vertex_count ? THE_FAIL(5181) : redex_count ? THE_FAIL(5182) : global_err);
