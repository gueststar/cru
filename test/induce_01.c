	  ...
	  // Create a redex.
{
  ...
  if ((r = (redex) malloc (sizeof (*r))) ? 0 : FAIL(3492))
	 return NULL;
  memset (r, 0, sizeof (*r));
  r->r_magic = REDEX_MAGIC;
  r->r_value = n;
  if (pthread_mutex_lock (&redex_lock) ? FAIL(3493) : ++redex_count ? 0 : FAIL(3494))
	 goto a;
  if (! (pthread_mutex_unlock (&redex_lock) ? FAIL(3495) : 0))
	 return r;
 a: free (r);
  return NULL;
}
...
free_redex (r, err)
	  ...
	  // i.
{
  if ((! r) ? FAIL(3496) : (r->r_magic != REDEX_MAGIC) ? FAIL(3497) : 0)
	 return;
  r->r_magic = MUGGLE(142);
  if (pthread_mutex_lock (&redex_lock) ? FAIL(3498) : redex_count-- ? 0 : FAIL(3499))
	 return;
  if (pthread_mutex_unlock (&redex_lock) ? FAIL(3500) : 0)
	 return;
  free (r);
}
	  ...
	  // Create an edge.
{
  ...
  if ((e = (edge) malloc (sizeof (*e))) ? 0 : FAIL(3501))
	 return NULL;
  memset (e, 0, sizeof (*e));
  e->e_magic = EDGE_MAGIC;
  e->e_value = n;
  if (pthread_mutex_lock (&edge_lock) ? FAIL(3502) : ++edge_count ? 0 : FAIL(3503))
	 goto a;
  if (! (pthread_mutex_unlock (&edge_lock) ? FAIL(3504) : 0))
	 return e;
 a: free (e);
  return NULL;
}
...
free_edge (e, err)
	  ...
	  // b.
{
  if ((! e) ? FAIL(3505) : (e->e_magic != EDGE_MAGIC) ? FAIL(3506) : 0)
	 return;
  e->e_magic = MUGGLE(143);
  if (pthread_mutex_lock (&edge_lock) ? FAIL(3507) : edge_count-- ? 0 : FAIL(3508))
	 return;
  if (pthread_mutex_unlock (&edge_lock) ? FAIL(3509) : 0)
	 return;
  free (e);
}
...
{
  if ((! a) ? FAIL(3510) : (a->e_magic != EDGE_MAGIC) ? FAIL(3511) : 0)
	 return 0;
  if ((! b) ? FAIL(3512) : (b->e_magic != EDGE_MAGIC) ? FAIL(3513) : 0)
	 return 0;
	  ...
	  // in the builder b.
{
  ...
  if ((! e) ? GLOBAL_FAIL(3514) : (e->e_magic != EDGE_MAGIC) ? GLOBAL_FAIL(3515) : 0)
	 return 0;
  u = e->e_value;
	  ...
	  // Create a vertex.
{
  ...
  if ((v = (vertex) malloc (sizeof (*v))) ? 0 : FAIL(3516))
	 return NULL;
  memset (v, 0, sizeof (*v));
  v->v_magic = VERTEX_MAGIC;
  v->v_value = n;
  if (pthread_mutex_lock (&vertex_lock) ? FAIL(3517) : ++vertex_count ? 0 : FAIL(3518))
	 goto a;
  if (! (pthread_mutex_unlock (&vertex_lock) ? FAIL(3519) : 0))
	 return v;
 a: free (v);
  return NULL;
}
...
free_vertex (v, err)
	  ...
	  // b.
{
  if ((! v) ? FAIL(3520) : (v->v_magic != VERTEX_MAGIC) ? FAIL(3521) : 0)
	 return;
  v->v_magic = MUGGLE(144);
  if (pthread_mutex_lock (&vertex_lock) ? FAIL(3522) : vertex_count-- ? 0 : FAIL(3523))
	 return;
  if (pthread_mutex_unlock (&vertex_lock) ? FAIL(3524) : 0)
	 return;
  free (v);
}
...
{
  if ((! a) ? FAIL(3525) : (a->v_magic != VERTEX_MAGIC) ? FAIL(3526) : 0)
	 return 0;
  if ((! b) ? FAIL(3527) : (b->v_magic != VERTEX_MAGIC) ? FAIL(3528) : 0)
	 return 0;
	  ...
	  // in the builder b.
{
  ...
  if ((! v) ? GLOBAL_FAIL(3529) : (v->v_magic != VERTEX_MAGIC) ? GLOBAL_FAIL(3530) : 0)
	 return 0;
  u = v->v_value;
  for (i = 0; i < sizeof (u); i++)  // ignore overflow
	 u += (u + 1) << 8;
	  ...
	  // the builder b.
{
  ...
  if ((! given_vertex) ? FAIL(3531) : (given_vertex->v_magic != VERTEX_MAGIC) ? FAIL(3532) : 0)
	 return;
...
{
  if (*err ? 1 : (! connecting_edge) ? FAIL(3533) : (connecting_edge->e_magic != EDGE_MAGIC) ? FAIL(3534) : 0)
	 return NULL;
  if ((! local_vertex) ? FAIL(3535) : (local_vertex->v_magic != VERTEX_MAGIC) ? FAIL(3536) : 0)
	 return NULL;
  if ((! remote_vertex) ? FAIL(3537) : (remote_vertex->v_magic != VERTEX_MAGIC) ? FAIL(3538) : 0)
	 return NULL;
  if (((local_vertex->v_value ^ remote_vertex->v_value) != (1 << connecting_edge->e_value)) ? FAIL(3539) : 0)
	 return NULL;
	  ...
	  // edges being valid.
{
  ...
  if (*err ? 1 : (! given_vertex) ? FAIL(3540) : (given_vertex->v_magic != VERTEX_MAGIC) ? FAIL(3541) : 0)
	 return NULL;
  if ((! edges_in) ? FAIL(3542) : (edges_in->r_magic != REDEX_MAGIC) ? FAIL(3543) : 0)
	 return NULL;
  if ((! edges_out) ? FAIL(3544) : (edges_out->r_magic != REDEX_MAGIC) ? FAIL(3545) : 0)
	 return NULL;
  v = given_vertex->v_value;
  for (bits = 0; v; v >>= 1)
	 bits += (v & 1);
  if ((edges_in->r_value != bits) ? FAIL(3546) : (edges_out->r_value != (DIMENSION - bits)) ? FAIL(3547) : 0)
	 return NULL;
  return redex_of (1, err);
}
	  ...
	  // errors.
{
  ...
  if (*err)
	 return NULL;
  if ((! l) ? 0 : (l->r_magic != REDEX_MAGIC) ? FAIL(3548) : 0)
	 return NULL;
  if ((! r) ? 0 : (r->r_magic != REDEX_MAGIC) ? FAIL(3549) : 0)
	 return NULL;
  s = (l ? l->r_value : 0) + (r ? r->r_value : 0);
  if ((s < (l ? l->r_value : 0)) ? FAIL(3550) : (s < (r ? r->r_value : 0)) ? FAIL(3551) : 0)
	 return NULL;
	  ...
	  // the hard way by using redexes for numbers.
{
  ...
  v = 0;
  if ((cru_vertex_count (g, LANES, err) == NUMBER_OF_VERTICES) ? 0 : FAIL(3552))
	 goto a;
  if ((cru_edge_count (g, LANES, err) == (DIMENSION * NUMBER_OF_VERTICES) >> 1) ? 0 : FAIL(3553))
	 goto a;
  if ((r = (redex) cru_mapreduced (g, &m, UNKILLABLE, LANES, err)) ? 0 : FAIL(3554))
	 goto a;
  if ((r->r_magic == REDEX_MAGIC) ? 0 : FAIL(3555))
	 goto a;
  if ((r->r_value == NUMBER_OF_VERTICES) ? 0 : FAIL(3556))
	 goto b;
  if ((! path_count) ? FAIL(3557) : (path_count->r_magic == REDEX_MAGIC) ? 0 : FAIL(3558))
	 goto b;
  v = ((path_count->r_value == (1 + variations (DIMENSION - 1))) ? 1 : ! FAIL(3559));
 b: free_redex (r, err); 
...
{
  if (*err ? 1 : (! local_vertex) ? FAIL(3560) : (local_vertex->v_magic != VERTEX_MAGIC) ? FAIL(3561) : 0)
	 return NULL;
  if ((! connecting_edge) ? FAIL(3562) : (connecting_edge->e_magic != EDGE_MAGIC) ? FAIL(3563) : 0)
	 return NULL;
  if ((! remote_value) ? FAIL(3564) : (remote_value->r_magic != REDEX_MAGIC) ? FAIL(3565) : 0)
	 return NULL;
