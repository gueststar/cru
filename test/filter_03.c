	  ...
	  // Create an edge.
{
  ...
  if ((e = (edge) malloc (sizeof (*e))) ? 0 : FAIL(3015))
	 return NULL;
  memset (e, 0, sizeof (*e));
...
free_edge (e, err)
	  ...
	  // b.
{
  if ((! e) ? FAIL(3019) : (e->e_magic != EDGE_MAGIC) ? FAIL(3020) : 0)
	 return;
  e->e_magic = MUGGLE(129);
  if (pthread_mutex_lock (&edge_lock) ? FAIL(3021) : edge_count-- ? 0 : FAIL(3022))
	 return;
  if (pthread_mutex_unlock (&edge_lock) ? FAIL(3023) : 0)
	 return;
  free (e);
}
	  ...
	  // Create a vertex.
{
  ...
  if ((v = (vertex) malloc (sizeof (*v))) ? 0 : FAIL(3024))
	 return NULL;
  memset (v, 0, sizeof (*v));
...
free_vertex (v, err)
	  ...
	  // b.
{
  if ((! v) ? FAIL(3028) : (v->v_magic != VERTEX_MAGIC) ? FAIL(3029) : 0)
	 return;
  v->v_magic = MUGGLE(130);
  if (pthread_mutex_lock (&vertex_lock) ? FAIL(3030) : vertex_count-- ? 0 : FAIL(3031))
	 return;
  if (pthread_mutex_unlock (&vertex_lock) ? FAIL(3032) : 0)
	 return;
  free (v);
}
	  ...
	  // Create a redex.
{
  ...
  if ((r = (redex) malloc (sizeof (*r))) ? 0 : FAIL(3033))
	 return NULL;
  memset (r, 0, sizeof (*r));
...
free_redex (r, err)
	  ...
	  // Reclaim a redex.
{
  if ((! r) ? FAIL(3037) : (r->r_magic != REDEX_MAGIC) ? FAIL(3038) : 0)
	 return;
  r->r_magic = MUGGLE(131);
  if (pthread_mutex_lock (&redex_lock) ? FAIL(3039) : redex_count-- ? 0 : FAIL(3040))
	 return;
  if (pthread_mutex_unlock (&redex_lock) ? FAIL(3041) : 0)
	 return;
  free (r);
}
...
free_mapex (r, err)
	  ...
	  // Reclaim a mapex.
{
  if ((! r) ? FAIL(3042) : (r->r_magic != MAPEX_MAGIC) ? FAIL(3043) : 0)
	 return;
  r->r_magic = REDEX_MAGIC;
  free_redex (r, err);
}
...
{
  if ((! a) ? FAIL(3050) : (a->v_magic != VERTEX_MAGIC) ? FAIL(3051) : 0)
	 return 0;
  if ((! b) ? FAIL(3052) : (b->v_magic != VERTEX_MAGIC) ? FAIL(3053) : 0)
	 return 0;
	  ...
	  // in the builder b.
{
  ...
  if ((! v) ? GLOBAL_FAIL(3054) : (v->v_magic != VERTEX_MAGIC) ? GLOBAL_FAIL(3055) : 0)
	 return 0;
  u = v->v_value;
  for (i = 0; i < sizeof (u); i++)  // ignore overflow
	 u += (u + 1) << 8;
	  ...
	  // the builder b.
{
  ...
  if ((! given_vertex) ? FAIL(3056) : (given_vertex->v_magic != VERTEX_MAGIC) ? FAIL(3057) : 0)
	 return;
...
{
  if (*err ? 1 : (! connecting_edge) ? FAIL(3058) : (connecting_edge->e_magic != EDGE_MAGIC) ? FAIL(3059) : 0)
	 return 0;
  if ((! local_vertex) ? FAIL(3060) : (local_vertex->v_magic != VERTEX_MAGIC) ? FAIL(3061) : 0)
	 return 0;
  if ((! remote_vertex) ? FAIL(3062) : (remote_vertex->v_magic != VERTEX_MAGIC) ? FAIL(3063) : 0)
	 return 0;
  return ! (((local_vertex->v_value ^ remote_vertex->v_value) != (1 << connecting_edge->e_value)) ? FAIL(3064) : 0);
}
	  ...
	  // edges being valid.
{
  ...
  if (*err ? 1 : (! given_vertex) ? FAIL(3065) : (given_vertex->v_magic != VERTEX_MAGIC) ? FAIL(3066) : 0)
	 return 0;
  v = given_vertex->v_value;
  for (bits = 0; v; v >>= 1)
	 bits += (v & 1);
  if (bits < ((DIMENSION >> 1) - 1))
	 return ! ((edges_in != DIMENSION) ? FAIL(3067) : (edges_out != DIMENSION) ? FAIL(3068) : 0);
  if (bits == ((DIMENSION >> 1) - 1))
	 return ! ((edges_in != bits) ? FAIL(3069) : (edges_out != DIMENSION) ? FAIL(3070) : 0);
  return ! ((edges_in != bits) ? FAIL(3071) : edges_out ? FAIL(3072) : 0);
}
	  ...
	  // errors.
{
  ...
  return ((*err ? 1 : ((s = l + r) < l) ? FAIL(3073) : (s < r) ? FAIL(3074) : 0) ? 0 : s);
}
...
#define REMAINING_EDGES ((REMAINING_VERTICES - choose (DIMENSION, DIMENSION >> 1)) * DIMENSION)
  ...
  if ((cru_vertex_count (g, LANES, err) == REMAINING_VERTICES) ? 0 : FAIL(3075))
	 return 0;
  if ((cru_edge_count (g, LANES, err) == REMAINING_EDGES) ? 0 : FAIL(3076))
	 return 0;
  if ((((uintptr_t) cru_mapreduced (g, &m, UNKILLABLE, LANES, err)) == REMAINING_VERTICES) ? 0 : FAIL(3077))
	 return 0;
  return 1;
}
	  ...
	  // f.fi_kernel.e_op.map field of the filter f.
{
  ...
  if ((! connecting_edge) ? FAIL(3078) : (connecting_edge->e_magic != EDGE_MAGIC) ? FAIL(3079) : 0)
	 return 0;
  if ((! local_vertex) ? FAIL(3080) : (local_vertex->v_magic != VERTEX_MAGIC) ? FAIL(3081) : 0)
	 return 0;
  if ((! adjacent_vertex) ? FAIL(3082) : (adjacent_vertex->v_magic != VERTEX_MAGIC) ? FAIL(3083) : 0)
	 return 0;
...
edge_reduction (a, b, err)
	  ...
	  // filter f.
{
  if ((! a) ? FAIL(3084) : (a->r_magic != MAPEX_MAGIC) ? FAIL(3085) : 0)
	 return NULL;
  if ((! b) ? FAIL(3086) : (b->r_magic != REDEX_MAGIC) ? FAIL(3087) : 0)
	 return NULL;
	  ...
	  int *err;
{
  ...
  if (lock_created ? (*lock_created = 0) : 1)
	 FAIL(3089);
  if (*err ? 1 : pthread_mutexattr_init (&mutex_attribute) ? FAIL(3090) : 0)
	 return 0;
  if (!((pthread_mutexattr_settype (&mutex_attribute, PTHREAD_MUTEX_ERRORCHECK)) ? FAIL(3091) : 0))
	 *lock_created = ! (pthread_mutex_init (lock, &mutex_attribute) ? FAIL(3092) : 0);
  return ! ((pthread_mutexattr_destroy (&mutex_attribute)) ? FAIL(3093) : *err);
}
	  ...
	  char **argv;
{
  ...
  nthm_sync (&err);
  if (! err)
	 err = (edge_count ? THE_FAIL(3094) : vertex_count ? THE_FAIL(3095) : redex_count ? THE_FAIL(3096) : global_err);
