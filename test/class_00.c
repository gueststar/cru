	  ...
	  // Create a redex.
{
  ...
  if ((r = (redex) malloc (sizeof (*r))) ? 0 : FAIL(1918))
	 return NULL;
  memset (r, 0, sizeof (*r));
...
{
  if ((! r) ? FAIL(1922) : (r->r_magic != OUTGOING_REDEX_MAGIC) ? FAIL(1923) : 0)
	 return;
  r->r_magic = MUGGLE(91);
...
free_redex (r, err)
	  ...
	  // Reclaim a redex.
{
  if ((! r) ? FAIL(1927) : (r->r_magic != REDEX_MAGIC) ? FAIL(1928) : 0)
	 return;
  r->r_magic = MUGGLE(92);
  if (pthread_mutex_lock (&redex_lock) ? FAIL(1929) : redex_count-- ? 0 : FAIL(1930))
	 return;
  if (pthread_mutex_unlock (&redex_lock) ? FAIL(1931) : 0)
	 return;
  free (r);
}
...
free_property (r, err)
	  ...
	  // classifier c.
{
  if ((! r) ? FAIL(1932) : (r->r_magic != PROPERTY_MAGIC) ? FAIL(1933) : 0)
	 return;
  r->r_magic = MUGGLE(93);
  if (pthread_mutex_lock (&redex_lock) ? FAIL(1934) : redex_count-- ? 0 : FAIL(1935))
	 return;
  if (pthread_mutex_unlock (&redex_lock) ? FAIL(1936) : 0)
	 return;
  free (r);
}
...
equal_properties (a, b, err)
	  ...
	  // equal.
{
  if ((! a) ? FAIL(1937) : (a->r_magic != PROPERTY_MAGIC) ? FAIL(1938) : 0)
	 return 0;
  if ((! b) ? FAIL(1939) : (b->r_magic != PROPERTY_MAGIC) ? FAIL(1940) : 0)
	 return 0;
	  ...
	  // Mix up low-entropy numeric values.
{
  ...
  if ((! r) ? GLOBAL_FAIL(1941) : (r->r_magic != PROPERTY_MAGIC) ? GLOBAL_FAIL(1942) : 0)
	 return 0;
  u = r->r_value;
	  ...
	  // Create an edge.
{
  ...
  if ((e = (edge) malloc (sizeof (*e))) ? 0 : FAIL(1943))
	 return NULL;
  memset (e, 0, sizeof (*e));
  e->e_magic = EDGE_MAGIC;
  e->e_value = n;
  if (pthread_mutex_lock (&edge_lock) ? FAIL(1944) : ++edge_count ? 0 : FAIL(1945))
	 goto a;
  if (! (pthread_mutex_unlock (&edge_lock) ? FAIL(1946) : 0))
	 return e;
 a: free (e);
  return NULL;
}
...
free_edge (e, err)
	  ...
	  // b.
{
  if ((! e) ? FAIL(1947) : (e->e_magic != EDGE_MAGIC) ? FAIL(1948) : 0)
	 return;
  e->e_magic = MUGGLE(94);
  if (pthread_mutex_lock (&edge_lock) ? FAIL(1949) : edge_count-- ? 0 : FAIL(1950))
	 return;
  if (pthread_mutex_unlock (&edge_lock) ? FAIL(1951) : 0)
	 return;
  free (e);
}
	  ...
	  // but is needed only for merging and not building.
{
  ...
  if ((! e) ? GLOBAL_FAIL(1952) : (e->e_magic != EDGE_MAGIC) ? GLOBAL_FAIL(1953) : 0)
	 return 0;
  u = e->e_value;
  for (i = 0; i < sizeof (u); i++)  // ignore overflow
	 u += (u + 1) << 8;
...
{
  if ((! a) ? FAIL(1954) : (a->e_magic != EDGE_MAGIC) ? FAIL(1955) : 0)
	 return 0;
  if ((! b) ? FAIL(1956) : (b->e_magic != EDGE_MAGIC) ? FAIL(1957) : 0)
	 return 0;
	  ...
	  // Create a vertex.
{
  ...
  if ((v = (vertex) malloc (sizeof (*v))) ? 0 : FAIL(1958))
	 return NULL;
  memset (v, 0, sizeof (*v));
  v->v_magic = VERTEX_MAGIC;
  v->v_value = n;
  if (pthread_mutex_lock (&vertex_lock) ? FAIL(1959) : ++vertex_count ? 0 : FAIL(1960))
	 goto a;
  if (! (pthread_mutex_unlock (&vertex_lock) ? FAIL(1961) : 0))
	 return v;
 a: free (v);
  return NULL;
}
...
free_vertex (v, err)
	  ...
	  // b.
{
  if ((! v) ? FAIL(1962) : (v->v_magic != VERTEX_MAGIC) ? FAIL(1963) : 0)
	 return;
  v->v_magic = MUGGLE(95);
  if (pthread_mutex_lock (&vertex_lock) ? FAIL(1964) : vertex_count-- ? 0 : FAIL(1965))
	 return;
  if (pthread_mutex_unlock (&vertex_lock) ? FAIL(1966) : 0)
	 return;
  free (v);
}
...
{
  if ((! a) ? FAIL(1967) : (a->v_magic != VERTEX_MAGIC) ? FAIL(1968) : 0)
	 return 0;
  if ((! b) ? FAIL(1969) : (b->v_magic != VERTEX_MAGIC) ? FAIL(1970) : 0)
	 return 0;
	  ...
	  // in the builder b.
{
  ...
  if ((! v) ? GLOBAL_FAIL(1971) : (v->v_magic != VERTEX_MAGIC) ? GLOBAL_FAIL(1972) : 0)
	 return 0;
  u = v->v_value;
  for (i = 0; i < sizeof (u); i++)  // ignore overflow
	 u += (u + 1) << 8;
	  ...
	  // the builder b.
{
  ...
  if ((! given_vertex) ? FAIL(1973) : (given_vertex->v_magic != VERTEX_MAGIC) ? FAIL(1974) : 0)
	 return;
...
{
  if (*err ? 1 : (! connecting_edge) ? FAIL(1975) : (connecting_edge->e_magic != EDGE_MAGIC) ? FAIL(1976) : 0)
	 return NULL;
  if ((! local_vertex) ? FAIL(1977) : (local_vertex->v_magic != VERTEX_MAGIC) ? FAIL(1978) : 0)
	 return NULL;
  if ((! remote_vertex) ? FAIL(1979) : (remote_vertex->v_magic != VERTEX_MAGIC) ? FAIL(1980) : 0)
	 return NULL;
  if ((((local_vertex->v_value ^ remote_vertex->v_value)) != (1 << connecting_edge->e_value)) ? FAIL(1981) : 0)
	 return NULL;
...
{
  if (*err ? 1 : (! connecting_edge) ? FAIL(1982) : (connecting_edge->e_magic != EDGE_MAGIC) ? FAIL(1983) : 0)
	 return NULL;
  if ((! local_vertex) ? FAIL(1984) : (local_vertex->v_magic != VERTEX_MAGIC) ? FAIL(1985) : 0)
	 return NULL;
  if ((! remote_vertex) ? FAIL(1986) : (remote_vertex->v_magic != VERTEX_MAGIC) ? FAIL(1987) : 0)
	 return NULL;
  if ((((local_vertex->v_value ^ remote_vertex->v_value)) != (1 << connecting_edge->e_value)) ? FAIL(1988) : 0)
	 return NULL;
  return ((remote_vertex->v_value > local_vertex->v_value) ? cru_class_of (h, remote_vertex, err) : NULL);
}
	  ...
	  // higher numbered adjacent vertices.
{
  ...
  if (*err ? 1 : (! given_vertex) ? FAIL(1990) : (given_vertex->v_magic != VERTEX_MAGIC) ? FAIL(1991) : 0)
	 return NULL;
  if ((edges_in == edges_out) ? FAIL(1992) : 0)
	 return NULL;
  if (((c = cru_class_of (h, given_vertex, err)) == edges_in) ? FAIL(1993) : (c == edges_out) ? FAIL(1994) : 0)
	 return NULL;
  v = given_vertex->v_value;
  for (bits = 0; v; v >>= 1)
	 bits += (v & 1);
  if (*err != ENOMEM)
	 if ((cru_class_size (c, err) != choose (DIMENSION, bits)) ? FAIL(1995) : 0)
		return NULL;
	  ...
	  // errors.
{
  ...
  if (*err)
	 return NULL;
  if ((! l) ? 0 : (l->r_magic != REDEX_MAGIC) ? FAIL(1996) : 0)
	 return NULL;
  if ((! r) ? 0 : (r->r_magic != REDEX_MAGIC) ? FAIL(1997) : 0)
	 return NULL;
  s = (l ? l->r_value : 0) + (r ? r->r_value : 0);
  if ((s < (l ? l->r_value : 0)) ? FAIL(1998) : (s < (r ? r->r_value : 0)) ? FAIL(1999) : 0)
	 return NULL;
	  ...
	  // errors.
{
  ...
  if (*err)
	 return NULL;
  if ((! l) ? 0 : (l->r_magic != OUTGOING_REDEX_MAGIC) ? FAIL(2000) : 0)
	 return NULL;
  if ((! r) ? 0 : (r->r_magic != OUTGOING_REDEX_MAGIC) ? FAIL(2001) : 0)
	 return NULL;
  s = (l ? l->r_value : 0) + (r ? r->r_value : 0);
  if ((s < (l ? l->r_value : 0)) ? FAIL(2002) : (s < (r ? r->r_value : 0)) ? FAIL(2003) : 0)
	 return NULL;
  if ((t = redex_of (s, err)))
	  ...
	  // the hard way by using redexes for numbers.
{
  ...
  if ((cru_vertex_count (g, LANES, err) == NUMBER_OF_VERTICES) ? 0 : FAIL(2004))
	 return 0;
  if ((cru_edge_count (g, LANES, err) == (DIMENSION * NUMBER_OF_VERTICES)) ? 0 : FAIL(2005))
	 return 0;
  if ((r = (redex) cru_mapreduced (g, &m, UNKILLABLE, LANES, err)) ? 0 : FAIL(2006))
	 return 0;
  if ((r->r_magic == REDEX_MAGIC) ? 0 : FAIL(2007))
	 return 0;
...
#define D ((DIMENSION * (DIMENSION - 1)) >> 1)
  ...
  if (*err ? 1 : (! given_vertex) ? FAIL(2009) : (given_vertex->v_magic != VERTEX_MAGIC) ? FAIL(2010) : 0)
	 return NULL;
  if (edges_in ? FAIL(2011) : 0)
	 return NULL;
  if ((! edges_out) ? FAIL(2012) : (edges_out->r_magic != OUTGOING_REDEX_MAGIC) ? FAIL(2013) : 0)
	 return NULL;
  if ((edges_out->r_value != D) ? FAIL(2014) : 0)
	 return NULL;
  v = given_vertex->v_value;
  for (bits = 0; v; v >>= 1)
	 bits += (v & 1);
	  ...
	  int *err;
{
  ...
  if (lock_created ? (*lock_created = 0) : 1)
	 FAIL(2022);
  if (*err ? 1 : pthread_mutexattr_init (&mutex_attribute) ? FAIL(2023) : 0)
	 return 0;
  if (!((pthread_mutexattr_settype (&mutex_attribute, PTHREAD_MUTEX_ERRORCHECK)) ? FAIL(2024) : 0))
	 *lock_created = ! (pthread_mutex_init (lock, &mutex_attribute) ? FAIL(2025) : 0);
  return ! ((pthread_mutexattr_destroy (&mutex_attribute)) ? FAIL(2026) : *err);
}
	  ...
	  char **argv;
{
  ...
  nthm_sync (&err);
  if (! err)
	 err = (edge_count ? THE_FAIL(2027) : vertex_count ? THE_FAIL(2028) : redex_count ? THE_FAIL(2029) : global_err);
