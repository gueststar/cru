	  ...
	  // Create a redex.
{
  ...
  if ((r = (redex) malloc (sizeof (*r))) ? 0 : FAIL(1901))
	 return NULL;
  memset (r, 0, sizeof (*r));
  r->r_magic = REDEX_MAGIC;
  r->r_value = n;
  if (pthread_mutex_lock (&redex_lock) ? FAIL(1902) : ++redex_count ? 0 : FAIL(1903))
	 goto a;
  if (! (pthread_mutex_unlock (&redex_lock) ? FAIL(1904) : 0))
	 return r;
 a: free (r);
  return NULL;
}
...
{
  if ((! r) ? FAIL(1905) : (r->r_magic != OUTGOING_REDEX_MAGIC) ? FAIL(1906) : 0)
	 return;
  r->r_magic = MUGGLE(87);
...
free_redex (r, err)
	  ...
	  // Reclaim a redex.
{
  if ((! r) ? FAIL(1910) : (r->r_magic != REDEX_MAGIC) ? FAIL(1911) : 0)
	 return;
  r->r_magic = MUGGLE(88);
...
free_property (r, err)
	  ...
	  // classifier c.
{
  if ((! r) ? FAIL(1915) : (r->r_magic != PROPERTY_MAGIC) ? FAIL(1916) : 0)
	 return;
  r->r_magic = MUGGLE(89);
...
equal_properties (a, b, err)
	  ...
	  // equal.
{
  if ((! a) ? FAIL(1920) : (a->r_magic != PROPERTY_MAGIC) ? FAIL(1921) : 0)
	 return 0;
  if ((! b) ? FAIL(1922) : (b->r_magic != PROPERTY_MAGIC) ? FAIL(1923) : 0)
	 return 0;
	  ...
	  // Mix up low-entropy numeric values.
{
  ...
  if ((! r) ? GLOBAL_FAIL(1924) : (r->r_magic != PROPERTY_MAGIC) ? GLOBAL_FAIL(1925) : 0)
	 return 0;
  u = r->r_value;
	  ...
	  // Create an edge.
{
  ...
  if ((e = (edge) malloc (sizeof (*e))) ? 0 : FAIL(1926))
	 return NULL;
  memset (e, 0, sizeof (*e));
  e->e_magic = EDGE_MAGIC;
  e->e_value = n;
  if (pthread_mutex_lock (&edge_lock) ? FAIL(1927) : ++edge_count ? 0 : FAIL(1928))
	 goto a;
  if (! (pthread_mutex_unlock (&edge_lock) ? FAIL(1929) : 0))
	 return e;
 a: free (e);
  return NULL;
}
...
free_edge (e, err)
	  ...
	  // b.
{
  if ((! e) ? FAIL(1930) : (e->e_magic != EDGE_MAGIC) ? FAIL(1931) : 0)
	 return;
  e->e_magic = MUGGLE(90);
	  ...
	  // but is needed only for merging and not building.
{
  ...
  if ((! e) ? GLOBAL_FAIL(1935) : (e->e_magic != EDGE_MAGIC) ? GLOBAL_FAIL(1936) : 0)
	 return 0;
  u = e->e_value;
  for (i = 0; i < sizeof (u); i++)  // ignore overflow
	 u += (u + 1) << 8;
...
{
  if ((! a) ? FAIL(1937) : (a->e_magic != EDGE_MAGIC) ? FAIL(1938) : 0)
	 return 0;
  if ((! b) ? FAIL(1939) : (b->e_magic != EDGE_MAGIC) ? FAIL(1940) : 0)
	 return 0;
	  ...
	  // Create a vertex.
{
  ...
  if ((v = (vertex) malloc (sizeof (*v))) ? 0 : FAIL(1941))
	 return NULL;
  memset (v, 0, sizeof (*v));
  v->v_magic = VERTEX_MAGIC;
  v->v_value = n;
  if (pthread_mutex_lock (&vertex_lock) ? FAIL(1942) : ++vertex_count ? 0 : FAIL(1943))
	 goto a;
  if (! (pthread_mutex_unlock (&vertex_lock) ? FAIL(1944) : 0))
	 return v;
 a: free (v);
  return NULL;
}
...
free_vertex (v, err)
	  ...
	  // b.
{
  if ((! v) ? FAIL(1945) : (v->v_magic != VERTEX_MAGIC) ? FAIL(1946) : 0)
	 return;
  v->v_magic = MUGGLE(91);
...
{
  if ((! a) ? FAIL(1950) : (a->v_magic != VERTEX_MAGIC) ? FAIL(1951) : 0)
	 return 0;
  if ((! b) ? FAIL(1952) : (b->v_magic != VERTEX_MAGIC) ? FAIL(1953) : 0)
	 return 0;
	  ...
	  // in the builder b.
{
  ...
  if ((! v) ? GLOBAL_FAIL(1954) : (v->v_magic != VERTEX_MAGIC) ? GLOBAL_FAIL(1955) : 0)
	 return 0;
  u = v->v_value;
  for (i = 0; i < sizeof (u); i++)  // ignore overflow
	 u += (u + 1) << 8;
	  ...
	  // the builder b.
{
  ...
  if ((! given_vertex) ? FAIL(1956) : (given_vertex->v_magic != VERTEX_MAGIC) ? FAIL(1957) : 0)
	 return;
...
{
  if (*err ? 1 : (! connecting_edge) ? FAIL(1958) : (connecting_edge->e_magic != EDGE_MAGIC) ? FAIL(1959) : 0)
	 return NULL;
  if ((! local_vertex) ? FAIL(1960) : (local_vertex->v_magic != VERTEX_MAGIC) ? FAIL(1961) : 0)
	 return NULL;
  if ((! remote_vertex) ? FAIL(1962) : (remote_vertex->v_magic != VERTEX_MAGIC) ? FAIL(1963) : 0)
	 return NULL;
  if ((((local_vertex->v_value ^ remote_vertex->v_value)) != (1 << connecting_edge->e_value)) ? FAIL(1964) : 0)
	 return NULL;
	  ...
	  // higher numbered adjacent vertices.
{
  ...
  if (*err ? 1 : (! given_vertex) ? FAIL(1973) : (given_vertex->v_magic != VERTEX_MAGIC) ? FAIL(1974) : 0)
	 return NULL;
  if ((edges_in == edges_out) ? FAIL(1975) : 0)
	 return NULL;
  if (((c = cru_class_of (h, given_vertex, err)) == edges_in) ? FAIL(1976) : (c == edges_out) ? FAIL(1977) : 0)
	 return NULL;
  v = given_vertex->v_value;
  for (bits = 0; v; v >>= 1)
	 bits += (v & 1);
  if (*err != ENOMEM)
	 if ((cru_class_size (c, err) != choose (DIMENSION, bits)) ? FAIL(1978) : 0)
		return NULL;
	  ...
	  // errors.
{
  ...
  if (*err)
	 return NULL;
  if ((! l) ? 0 : (l->r_magic != REDEX_MAGIC) ? FAIL(1979) : 0)
	 return NULL;
  if ((! r) ? 0 : (r->r_magic != REDEX_MAGIC) ? FAIL(1980) : 0)
	 return NULL;
  s = (l ? l->r_value : 0) + (r ? r->r_value : 0);
  if ((s < (l ? l->r_value : 0)) ? FAIL(1981) : (s < (r ? r->r_value : 0)) ? FAIL(1982) : 0)
	 return NULL;
	  ...
	  // errors.
{
  ...
  if (*err)
	 return NULL;
  if ((! l) ? 0 : (l->r_magic != OUTGOING_REDEX_MAGIC) ? FAIL(1983) : 0)
	 return NULL;
  if ((! r) ? 0 : (r->r_magic != OUTGOING_REDEX_MAGIC) ? FAIL(1984) : 0)
	 return NULL;
  s = (l ? l->r_value : 0) + (r ? r->r_value : 0);
  if ((s < (l ? l->r_value : 0)) ? FAIL(1985) : (s < (r ? r->r_value : 0)) ? FAIL(1986) : 0)
	 return NULL;
  if ((t = redex_of (s, err)))
	  ...
	  // the hard way by using redexes for numbers.
{
  ...
  if ((cru_vertex_count (g, LANES, err) == NUMBER_OF_VERTICES) ? 0 : FAIL(1987))
	 return 0;
  if ((cru_edge_count (g, LANES, err) == (DIMENSION * NUMBER_OF_VERTICES)) ? 0 : FAIL(1988))
	 return 0;
  if ((r = (redex) cru_mapreduced (g, &m, UNKILLABLE, LANES, err)) ? 0 : FAIL(1989))
	 return 0;
  if ((r->r_magic == REDEX_MAGIC) ? 0 : FAIL(1990))
	 return 0;
...
#define D ((DIMENSION * (DIMENSION - 1)) >> 1)
  ...
  if (*err ? 1 : (! given_vertex) ? FAIL(1992) : (given_vertex->v_magic != VERTEX_MAGIC) ? FAIL(1993) : 0)
	 return NULL;
  if (edges_in ? FAIL(1994) : 0)
	 return NULL;
  if ((! edges_out) ? FAIL(1995) : (edges_out->r_magic != OUTGOING_REDEX_MAGIC) ? FAIL(1996) : 0)
	 return NULL;
  if ((edges_out->r_value != D) ? FAIL(1997) : 0)
	 return NULL;
  v = given_vertex->v_value;
  for (bits = 0; v; v >>= 1)
	 bits += (v & 1);
	  ...
	  // Return an outgoing redex of the edge label.
{
  ...
  if (*err ? 1 : (! connecting_edge) ? FAIL(1998) : (connecting_edge->e_magic != EDGE_MAGIC) ? FAIL(1999) : 0)
	 return NULL;
  if ((! local_vertex) ? FAIL(2000) : (local_vertex->v_magic != VERTEX_MAGIC) ? FAIL(2001) : 0)
	 return NULL;
  if ((! remote_vertex) ? FAIL(2002) : (remote_vertex->v_magic != VERTEX_MAGIC) ? FAIL(2003) : 0)
	 return NULL;
	  ...
	  int *err;
{
  ...
  if (lock_created ? (*lock_created = 0) : 1)
	 FAIL(2005);
  if (*err ? 1 : pthread_mutexattr_init (&mutex_attribute) ? FAIL(2006) : 0)
	 return 0;
  if (!((pthread_mutexattr_settype (&mutex_attribute, PTHREAD_MUTEX_ERRORCHECK)) ? FAIL(2007) : 0))
	 *lock_created = ! (pthread_mutex_init (lock, &mutex_attribute) ? FAIL(2008) : 0);
  return ! ((pthread_mutexattr_destroy (&mutex_attribute)) ? FAIL(2009) : *err);
}
	  ...
	  char **argv;
{
  ...
  nthm_sync (&err);
  if (! err)
	 err = (edge_count ? THE_FAIL(2010) : vertex_count ? THE_FAIL(2011) : redex_count ? THE_FAIL(2012) : global_err);
