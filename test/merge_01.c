	  ...
	  // Create a redex.
{
  ...
  if ((r = (redex) malloc (sizeof (*r))) ? 0 : FAIL(3677))
	 return NULL;
  memset (r, 0, sizeof (*r));
  r->r_magic = REDEX_MAGIC;
  r->r_value = n;
  if (pthread_mutex_lock (&redex_lock) ? FAIL(3678) : ++redex_count ? 0 : FAIL(3679))
	 goto a;
  if (! (pthread_mutex_unlock (&redex_lock) ? FAIL(3680) : 0))
	 return r;
 a: free (r);
  return NULL;
}
...
free_redex (r, err)
	  ...
	  // Reclaim a redex.
{
  if ((! r) ? FAIL(3681) : (r->r_magic != REDEX_MAGIC) ? FAIL(3682) : 0)
	 return;
  r->r_magic = MUGGLE(150);
  if (pthread_mutex_lock (&redex_lock) ? FAIL(3683) : redex_count-- ? 0 : FAIL(3684))
	 return;
  if (pthread_mutex_unlock (&redex_lock) ? FAIL(3685) : 0)
	 return;
  free (r);
}
...
free_mapex (r, err)
	  ...
	  // Reclaim a mapex.
{
  if ((! r) ? FAIL(3686) : (r->r_magic != MAPEX_MAGIC) ? FAIL(3687) : 0)
	 return;
  r->r_magic = REDEX_MAGIC;
  free_redex (r, err);
}
...
free_property (r, err)
	  ...
	  // merger c.
{
  if ((! r) ? FAIL(3688) : (r->r_magic != PROPERTY_MAGIC) ? FAIL(3689) : 0)
	 return;
...
{
  if ((! r) ? FAIL(3693) : (r->r_magic != INCIDENT_REDEX_MAGIC) ? FAIL(3694) : 0)
	 return;
...
{
  if ((! r) ? FAIL(3698) : (r->r_magic != INCIDENT_MAPEX_MAGIC) ? FAIL(3699) : 0)
	 return;
...
{
  if ((! r) ? FAIL(3700) : (r->r_magic != OUTGOING_REDEX_MAGIC) ? FAIL(3701) : 0)
	 return;
...
{
  if ((! a) ? FAIL(3705) : (a->r_magic != PROPERTY_MAGIC) ? FAIL(3706) : 0)
	 return 0;
  if ((! b) ? FAIL(3707) : (b->r_magic != PROPERTY_MAGIC) ? FAIL(3708) : 0)
	 return 0;
	  ...
	  // cru_hash is passed as c.me_classifier.cl_order.hash in the merger c.
{
  ...
  if ((! r) ? GLOBAL_FAIL(3709) : (r->r_magic != PROPERTY_MAGIC) ? GLOBAL_FAIL(3710) : 0)
	 return 0;
  u = r->r_value;
	  ...
	  // Create an edge.
{
  ...
  if ((e = (edge) malloc (sizeof (*e))) ? 0 : FAIL(3711))
	 return NULL;
  memset (e, 0, sizeof (*e));
  e->e_magic = EDGE_MAGIC;
  e->e_value = n;
  if (pthread_mutex_lock (&edge_lock) ? FAIL(3712) : ++edge_count ? 0 : FAIL(3713))
	 goto a;
  if (! (pthread_mutex_unlock (&edge_lock) ? FAIL(3714) : 0))
	 return e;
 a: free (e);
  return NULL;
}
...
free_edge (e, err)
	  ...
	  // b.
{
  if ((! e) ? FAIL(3715) : (e->e_magic != EDGE_MAGIC) ? FAIL(3716) : 0)
	 return;
  e->e_magic = MUGGLE(154);
  if (pthread_mutex_lock (&edge_lock) ? FAIL(3717) : edge_count-- ? 0 : FAIL(3718))
	 return;
  if (pthread_mutex_unlock (&edge_lock) ? FAIL(3719) : 0)
	 return;
  free (e);
}
	  ...
	  // but is needed only for merging and not building.
{
  ...
  if ((! e) ? GLOBAL_FAIL(3720) : (e->e_magic != EDGE_MAGIC) ? GLOBAL_FAIL(3721) : 0)
	 return 0;
  u = e->e_value;
  for (i = 0; i < sizeof (u); i++)  // ignore overflow
	 u += (u + 1) << 8;
...
{
  if ((! a) ? FAIL(3722) : (a->e_magic != EDGE_MAGIC) ? FAIL(3723) : 0)
	 return 0;
  if ((! b) ? FAIL(3724) : (b->e_magic != EDGE_MAGIC) ? FAIL(3725) : 0)
	 return 0;
	  ...
	  // Create a vertex.
{
  ...
  if ((v = (vertex) malloc (sizeof (*v))) ? 0 : FAIL(3726))
	 return NULL;
  memset (v, 0, sizeof (*v));
  v->v_magic = VERTEX_MAGIC;
  v->v_value = n;
  if (pthread_mutex_lock (&vertex_lock) ? FAIL(3727) : ++vertex_count ? 0 : FAIL(3728))
	 goto a;
  if (! (pthread_mutex_unlock (&vertex_lock) ? FAIL(3729) : 0))
	 return v;
 a: free (v);
  return NULL;
}
...
free_vertex (v, err)
	  ...
	  // b.
{
  if ((! v) ? FAIL(3730) : (v->v_magic != VERTEX_MAGIC) ? FAIL(3731) : 0)
	 return;
  v->v_magic = MUGGLE(155);
  if (pthread_mutex_lock (&vertex_lock) ? FAIL(3732) : vertex_count-- ? 0 : FAIL(3733))
	 return;
  if (pthread_mutex_unlock (&vertex_lock) ? FAIL(3734) : 0)
	 return;
  free (v);
}
...
{
  if ((! v) ? FAIL(3735) : (v->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(3736) : 0)
	 return;
  v->v_magic = MUGGLE(156);
  if (pthread_mutex_lock (&vertex_lock) ? FAIL(3737) : vertex_count-- ? 0 : FAIL(3738))
	 return;
  if (pthread_mutex_unlock (&vertex_lock) ? FAIL(3739) : 0)
	 return;
  free (v);
}
...
{
  if ((! a) ? FAIL(3740) : (a->v_magic != VERTEX_MAGIC) ? FAIL(3741) : 0)
	 return 0;
  if ((! b) ? FAIL(3742) : (b->v_magic != VERTEX_MAGIC) ? FAIL(3743) : 0)
	 return 0;
...
fused_equal_vertices (a, b, err)
	  ...
	  // equal.
{
  if ((! a) ? FAIL(3744) : (a->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(3745) : 0)
	 return 0;
  if ((! b) ? FAIL(3746) : (b->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(3747) : 0)
	 return 0;
  return a->v_value == b->v_value;
}
	  ...
	  // in the builder b.
{
  ...
  if ((! v) ? GLOBAL_FAIL(3748) : (v->v_magic != VERTEX_MAGIC) ? GLOBAL_FAIL(3749) : 0)
	 return 0;
  u = v->v_value;
  for (i = 0; i < sizeof (u); i++)  // ignore overflow
	 u += (u + 1) << 8;
	  ...
	  // Mix up low-entropy numeric values.
{
  ...
  if ((! v) ? GLOBAL_FAIL(3750) : (v->v_magic != FUSED_VERTEX_MAGIC) ? GLOBAL_FAIL(3751) : 0)
	 return 0;
  u = v->v_value;
  for (i = 0; i < sizeof (u); i++)  // ignore overflow
	 u += (u + 1) << 8;
	  ...
	  // the builder b.
{
  ...
  if ((! given_vertex) ? FAIL(3752) : (given_vertex->v_magic != VERTEX_MAGIC) ? FAIL(3753) : 0)
	 return;
...
{
  if (*err ? 1 : (! connecting_edge) ? FAIL(3754) : (connecting_edge->e_magic != EDGE_MAGIC) ? FAIL(3755) : 0)
	 return NULL;
  if ((! local_vertex) ? FAIL(3756) : (local_vertex->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(3757) : 0)
	 return NULL;
  if ((! remote_vertex) ? FAIL(3758) : (remote_vertex->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(3759) : 0)
	 return NULL;
  if ((local_vertex->v_value == remote_vertex->v_value) ? (! (connecting_edge->e_value)) : 0)
	 goto a;
  if ((((local_vertex->v_value ^ remote_vertex->v_value) << 1) != ( 1 << connecting_edge->e_value)) ? FAIL(3760) : 0)
	 return NULL;
...
vertex_checker (edges_in, given_vertex, edges_out, err)
	  ...
	  // edges being valid.
{
  if (*err ? 1 : (! given_vertex) ? FAIL(3761) : (given_vertex->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(3762) : 0)
	 return NULL;
  if ((! edges_in) ? FAIL(3763) : (edges_in->r_magic != REDEX_MAGIC) ? FAIL(3764) : 0)
	 return NULL;
  if ((! edges_out) ? FAIL(3765) : (edges_out->r_magic != REDEX_MAGIC) ? FAIL(3766) : 0)
	 return NULL;
  if ((edges_in->r_value != DIMENSION) ? FAIL(3767) : (edges_out->r_value != DIMENSION) ? FAIL(3768) : 0)
	 return NULL;
	  ...
	  // errors.
{
  ...
  if (*err)
	 return NULL;
  if ((! l) ? 0 : (l->r_magic != REDEX_MAGIC) ? FAIL(3769) : 0)
	 return NULL;
  if ((! r) ? 0 : (r->r_magic != REDEX_MAGIC) ? FAIL(3770) : 0)
	 return NULL;
  s = (l ? l->r_value : 0) + (r ? r->r_value : 0);
  if ((s < (l ? l->r_value : 0)) ? FAIL(3771) : (s < (r ? r->r_value : 0)) ? FAIL(3772) : 0)
	 return NULL;
	  ...
	  // errors.
{
  ...
  if (*err)
	 return NULL;
  if ((! l) ? 0 : (l->r_magic != INCIDENT_MAPEX_MAGIC) ? FAIL(3773) : 0)
	 return NULL;
  if ((! r) ? 0 : (r->r_magic != INCIDENT_REDEX_MAGIC) ? FAIL(3774) : 0)
	 return NULL;
  s = (l ? l->r_value : 0) + (r ? r->r_value : 0);
  if ((s < (l ? l->r_value : 0)) ? FAIL(3775) : (s < (r ? r->r_value : 0)) ? FAIL(3776) : 0)
	 return NULL;
  if ((t = redex_of (s, err)))
	  ...
	  // errors.
{
  ...
  if (*err)
	 return NULL;
  if ((! l) ? 0 : (l->r_magic != OUTGOING_REDEX_MAGIC) ? FAIL(3777) : 0)
	 return NULL;
  if ((! r) ? 0 : (r->r_magic != OUTGOING_REDEX_MAGIC) ? FAIL(3778) : 0)
	 return NULL;
  s = (l ? l->r_value : 0) + (r ? r->r_value : 0);
  if ((s < (l ? l->r_value : 0)) ? FAIL(3779) : (s < (r ? r->r_value : 0)) ? FAIL(3780) : 0)
	 return NULL;
  if ((t = redex_of (s, err)))
	  ...
	  // the hard way by using redexes for numbers.
{
  ...
  if (*err)
	 return 0;
  if ((cru_vertex_count (g, LANES, err) == (NUMBER_OF_VERTICES >> 1)) ? 0 : FAIL(3781))
	 return 0;
  if ((cru_edge_count (g, LANES, err) == ((DIMENSION * NUMBER_OF_VERTICES) >> 1)) ? 0 : FAIL(3782))
	 return 0;
  if ((r = (redex) cru_mapreduced (g, &m, UNKILLABLE, LANES, err)) ? 0 : FAIL(3783))
	 return 0;
  if ((r->r_magic == REDEX_MAGIC) ? 0 : FAIL(3784))
	 return 0;
...
#define D ((DIMENSION * (DIMENSION - 1)) >> 1)
  ...
  if (*err ? 1 : (! given_vertex) ? FAIL(3786) : (given_vertex->v_magic != VERTEX_MAGIC) ? FAIL(3787) : 0)
	 return NULL;
  if ((! edges_in) ? FAIL(3788) : (edges_in->r_magic != INCIDENT_REDEX_MAGIC) ? FAIL(3789) : 0)
	 return NULL;
  if ((! edges_out) ? FAIL(3790) : (edges_out->r_magic != OUTGOING_REDEX_MAGIC) ? FAIL(3791) : 0)
	 return NULL;
  if ((edges_in->r_value != D) ? FAIL(3792) : (edges_out->r_value != D) ? FAIL(3793) : 0)
	 return NULL;
	  ...
	  // Return an incident redex of the edge label.
{
  ...
  if (*err ? 1 : (! connecting_edge) ? FAIL(3794) : (connecting_edge->e_magic != EDGE_MAGIC) ? FAIL(3795) : 0)
	 return NULL;
  if ((! local_vertex) ? FAIL(3796) : (local_vertex->v_magic != VERTEX_MAGIC) ? FAIL(3797) : 0)
	 return NULL;
  if ((! remote_vertex) ? FAIL(3798) : (remote_vertex->v_magic != VERTEX_MAGIC) ? FAIL(3799) : 0)
	 return NULL;
	  ...
	  // Return an outgoing redex of the edge label.
{
  ...
  if (*err ? 1 : (! connecting_edge) ? FAIL(3800) : (connecting_edge->e_magic != EDGE_MAGIC) ? FAIL(3801) : 0)
	 return NULL;
  if ((! local_vertex) ? FAIL(3802) : (local_vertex->v_magic != VERTEX_MAGIC) ? FAIL(3803) : 0)
	 return NULL;
  if ((! remote_vertex) ? FAIL(3804) : (remote_vertex->v_magic != VERTEX_MAGIC) ? FAIL(3805) : 0)
	 return NULL;
  if ((r = redex_of (connecting_edge->e_value, err)))
	  ...
	  // as c.me_kernel.v_op.vertex.map in the merger c.
{
  ...
  if (edges_in ? FAIL(3806) : edges_out ? FAIL(3807) : 0)
	 return NULL;
  if ((! given_vertex) ? FAIL(3808) : (given_vertex->v_magic != VERTEX_MAGIC) ? FAIL(3809) : 0)
	 return NULL;
	  ...
	  // merger c.
{
  ...
  if ((! a) ? FAIL(3810) : (a->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(3811) : 0)
	 return NULL;
  if ((! b) ? FAIL(3812) : (b->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(3813) : 0)
	 return NULL;
  if ((a->v_value != b->v_value) ? FAIL(3814) : 0)
	 return NULL;
	  ...
	  int *err;
{
  ...
  if (lock_created ? (*lock_created = 0) : 1)
	 FAIL(3816);
  if (*err ? 1 : pthread_mutexattr_init (&mutex_attribute) ? FAIL(3817) : 0)
	 return 0;
  if (!((pthread_mutexattr_settype (&mutex_attribute, PTHREAD_MUTEX_ERRORCHECK)) ? FAIL(3818) : 0))
	 *lock_created = ! (pthread_mutex_init (lock, &mutex_attribute) ? FAIL(3819) : 0);
  return ! ((pthread_mutexattr_destroy (&mutex_attribute)) ? FAIL(3820) : *err);
}
	  ...
	  char **argv;
{
  ...
  nthm_sync (&err);
  if (! err)
	 err = (edge_count ? THE_FAIL(3821) : vertex_count ? THE_FAIL(3822) : redex_count ? THE_FAIL(3823) : global_err);
