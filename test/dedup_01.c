	  ...
	  // 2 + N2^(N+1) edges if N is odd.
{
  ...
  if (initial)
	 {
		if (given_vertex->v_value ? FAIL(2614) : 0)
		  return;
		if (incident ? FAIL(2615) : 0)
		  return;
		e = 0;            // the incident edge is ignored
	 }
  else
	 {
		if ((! incident) ? FAIL(2616) : (incident->e_magic != EDGE_MAGIC) ? FAIL(2617) : 0)
		  return;
		e = incident->e_value;
	 }
...
{
  if (*err ? 1 : (! connecting_edge) ? FAIL(2618) : (connecting_edge->e_magic != EDGE_MAGIC) ? FAIL(2619) : 0)
	 return 0;
  if ((! local_vertex) ? FAIL(2620) : (local_vertex->v_magic != VERTEX_MAGIC) ? FAIL(2621) : 0)
	 return 0;
  if ((! remote_vertex) ? FAIL(2622) : (remote_vertex->v_magic != VERTEX_MAGIC) ? FAIL(2623) : 0)
	 return 0;
  return ! (((local_vertex->v_value ^ remote_vertex->v_value) != (1 << connecting_edge->e_value)) ? FAIL(2624) : 0);
}
...
vertex_checker (edges_in, given_vertex, edges_out, err)
	  ...
	  // edges being valid.
{
  if (*err ? 1 : (! given_vertex) ? FAIL(2625) : (given_vertex->v_magic != VERTEX_MAGIC) ? FAIL(2626) : 0)
	 return 0;
  return ! ((edges_in != EDGES_PER_VERTEX) ? FAIL(2627) : (edges_out != EDGES_PER_VERTEX) ? FAIL(2628) : 0);
}
	  ...
	  // errors.
{
  ...
  return ((*err ? 1 : ((s = l + r) < l) ? FAIL(2629) : (s < r) ? FAIL(2630) : 0) ? 0 : s);
}
	  ...
	  // Check for correct vertex counts, edge counts, and edge labels.
{
  ...
  if ((cru_vertex_count (g, LANES, err) == NUMBER_OF_VERTICES) ? 0 : FAIL(2631))
	 return 0;
  if ((cru_edge_count (g, LANES, err) == (EDGES_PER_VERTEX * NUMBER_OF_VERTICES)) ? 0 : FAIL(2632))
	 return 0;
  if ((((uintptr_t) cru_mapreduced (g, &m, UNKILLABLE, LANES, err)) == NUMBER_OF_VERTICES) ? 0 : FAIL(2633))
	 return 0;
  return 1;
}
	  ...
	  int *err;
{
  ...
  if (lock_created ? (*lock_created = 0) : 1)
	 FAIL(2635);
  if (*err ? 1 : pthread_mutexattr_init (&mutex_attribute) ? FAIL(2636) : 0)
	 return 0;
  if (!((pthread_mutexattr_settype (&mutex_attribute, PTHREAD_MUTEX_ERRORCHECK)) ? FAIL(2637) : 0))
	 *lock_created = ! (pthread_mutex_init (lock, &mutex_attribute) ? FAIL(2638) : 0);
  return ! ((pthread_mutexattr_destroy (&mutex_attribute)) ? FAIL(2639) : *err);
}
	  ...
	  char **argv;
{
  ...
  nthm_sync (&err);
  if (! err)
	 err = (edge_count ? THE_FAIL(2640) : vertex_count ? THE_FAIL(2641) : global_err);
