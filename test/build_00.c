...
{
  return ! (*err ? 1 : ((local_vertex ^ remote_vertex) != (1 << connecting_edge)) ? FAIL(1776) : 0);
}
...
vertex_checker (edges_in, vertex, edges_out, err)
	  ...
	  // edges being valid.
{
  return ! (*err ? 1 : (edges_in != DIMENSION) ? FAIL(1777) : (edges_out != DIMENSION) ? FAIL(1778) : 0);
}
	  ...
	  // other errors.
{
  ...
  return ((*err ? 1 : ((s = l + r) < l) ? FAIL(1779) : (s < r) ? FAIL(1780) : 0) ? 0 : s);
}
	  ...
	  // Check for correct vertex counts, edge counts, and edge labels.
{
  ...
  if ((cru_vertex_count (g, LANES, err) == NUMBER_OF_VERTICES) ? 0 : FAIL(1781))
	 return 0;
  if ((cru_edge_count (g, LANES, err) == DIMENSION * NUMBER_OF_VERTICES) ? 0 : FAIL(1782))
	 return 0;
  if ((((uintptr_t) cru_mapreduced (g, &m, UNKILLABLE, LANES, err)) == NUMBER_OF_VERTICES) ? 0 : FAIL(1783))
	 return 0;
