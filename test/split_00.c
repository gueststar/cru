...
{
  return ! (*err ? 1 : ((local_vertex ^ remote_vertex) != (1 << connecting_edge)) ? FAIL(5061) : 0);
}
...
vertex_checker (edges_in, vertex, edges_out, err)
	  ...
	  // edges being valid.
{
  return ! (*err ? 1 : (edges_in != (DIMENSION << 1)) ? FAIL(5062) : (edges_out != (DIMENSION << 1)) ? FAIL(5063) : 0);
}
	  ...
	  // other errors.
{
  ...
  return ((*err ? 1 : ((s = l + r) < l) ? FAIL(5064) : (s < r) ? FAIL(5065) : 0) ? 0 : s);
}
	  ...
	  // Check for correct vertex counts, edge counts, and edge labels.
{
  ...
  if ((cru_vertex_count (g, LANES, err) == (NUMBER_OF_VERTICES << 1)) ? 0 : FAIL(5066))
	 return 0;
  if ((cru_edge_count (g, LANES, err) == ((DIMENSION << 1) * (NUMBER_OF_VERTICES << 1))) ? 0 : FAIL(5067))
	 return 0;
  if ((((uintptr_t) cru_mapreduced (g, &m, UNKILLABLE, LANES, err)) == (NUMBER_OF_VERTICES << 1)) ? 0 : FAIL(5068))
	 return 0;
  return 1;
}
