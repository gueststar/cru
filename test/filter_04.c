	  ...
	  // Create an edge.
{
  ...
  if ((e = (edge) malloc (sizeof (*e))) ? 0 : FAIL(3100))
	 return NULL;
  memset (e, 0, sizeof (*e));
...
free_edge (e, err)
	  ...
	  // b.
{
  if ((! e) ? FAIL(3104) : (e->e_magic != EDGE_MAGIC) ? FAIL(3105) : 0)
	 return;
  e->e_magic = MUGGLE(132);
  if (pthread_mutex_lock (&edge_lock) ? FAIL(3106) : edge_count-- ? 0 : FAIL(3107))
	 return;
  if (pthread_mutex_unlock (&edge_lock) ? FAIL(3108) : 0)
	 return;
  free (e);
}
	  ...
	  // Create a vertex.
{
  ...
  if ((v = (vertex) malloc (sizeof (*v))) ? 0 : FAIL(3109))
	 return NULL;
  memset (v, 0, sizeof (*v));
...
free_vertex (v, err)
	  ...
	  // b.
{
  if ((! v) ? FAIL(3113) : (v->v_magic != VERTEX_MAGIC) ? FAIL(3114) : 0)
	 return;
  v->v_magic = MUGGLE(133);
  if (pthread_mutex_lock (&vertex_lock) ? FAIL(3115) : vertex_count-- ? 0 : FAIL(3116))
	 return;
  if (pthread_mutex_unlock (&vertex_lock) ? FAIL(3117) : 0)
	 return;
  free (v);
}
	  ...
	  // Create a redex.
{
  ...
  if ((r = (redex) malloc (sizeof (*r))) ? 0 : FAIL(3118))
	 return NULL;
  memset (r, 0, sizeof (*r));
...
free_redex (r, err)
	  ...
	  // Reclaim a redex.
{
  if ((! r) ? FAIL(3122) : (r->r_magic != REDEX_MAGIC) ? FAIL(3123) : 0)
	 return;
  r->r_magic = MUGGLE(134);
  if (pthread_mutex_lock (&redex_lock) ? FAIL(3124) : redex_count-- ? 0 : FAIL(3125))
	 return;
  if (pthread_mutex_unlock (&redex_lock) ? FAIL(3126) : 0)
	 return;
  free (r);
}
...
free_mapex (r, err)
	  ...
	  // Reclaim a mapex.
{
  if ((! r) ? FAIL(3127) : (r->r_magic != MAPEX_MAGIC) ? FAIL(3128) : 0)
	 return;
  r->r_magic = REDEX_MAGIC;
  free_redex (r, err);
}
...
{
  if ((! a) ? FAIL(3129) : (a->e_magic != EDGE_MAGIC) ? FAIL(3130) : 0)
	 return 0;
  if ((! b) ? FAIL(3131) : (b->e_magic != EDGE_MAGIC) ? FAIL(3132) : 0)
	 return 0;
	  ...
	  // filter f.
{
  ...
  if ((! e) ? GLOBAL_FAIL(3133) : (e->e_magic != EDGE_MAGIC) ? GLOBAL_FAIL(3134) : 0)
	 return 0;
  u = e->e_value;
...
{
  if ((! a) ? FAIL(3135) : (a->v_magic != VERTEX_MAGIC) ? FAIL(3136) : 0)
	 return 0;
  if ((! b) ? FAIL(3137) : (b->v_magic != VERTEX_MAGIC) ? FAIL(3138) : 0)
	 return 0;
	  ...
	  // in the builder b.
{
  ...
  if ((! v) ? GLOBAL_FAIL(3139) : (v->v_magic != VERTEX_MAGIC) ? GLOBAL_FAIL(3140) : 0)
	 return 0;
  u = v->v_value;
  for (i = 0; i < sizeof (u); i++)  // ignore overflow
	 u += (u + 1) << 8;
	  ...
	  // the builder b.
{
  ...
  if ((! given_vertex) ? FAIL(3141) : (given_vertex->v_magic != VERTEX_MAGIC) ? FAIL(3142) : 0)
	 return;
...
{
  if (*err ? 1 : (! connecting_edge) ? FAIL(3143) : (connecting_edge->e_magic != EDGE_MAGIC) ? FAIL(3144) : 0)
	 return 0;
  if ((! local_vertex) ? FAIL(3145) : (local_vertex->v_magic != VERTEX_MAGIC) ? FAIL(3146) : 0)
	 return 0;
  if ((! remote_vertex) ? FAIL(3147) : (remote_vertex->v_magic != VERTEX_MAGIC) ? FAIL(3148) : 0)
	 return 0;
  return ! (((local_vertex->v_value ^ remote_vertex->v_value) != (1 << connecting_edge->e_value)) ? FAIL(3149) : 0);
}
	  ...
	  // edges being valid.
{
  ...
  if (*err ? 1 : (! given_vertex) ? FAIL(3150) : (given_vertex->v_magic != VERTEX_MAGIC) ? FAIL(3151) : 0)
	 return 0;
  v = given_vertex->v_value;
  for (bits = 0; v; v >>= 1)
	 bits += (v & 1);
  if (! bits)
	 return ! (edges_in ? FAIL(3152) : (edges_out != (DIMENSION - 1)) ? FAIL(3153) : 0);
  if (bits == 2)
	 return ! (((edges_in + (given_vertex->v_value >> (DIMENSION - 1))) != 2) ? FAIL(3154) : 0);
  if (bits == (DIMENSION >> 1))
	 return ! ((edges_in != DIMENSION >> 1) ? FAIL(3155) : edges_out ? FAIL(3156) : 0);
  if (((edges_out + bits) != DIMENSION) ? FAIL(3157) : 0)
	 return 0;
  return ! (((bits + edges_out) != DIMENSION) ? FAIL(3158) : 0);
}
	  ...
	  // errors.
{
  ...
  return ((*err ? 1 : ((s = l + r) < l) ? FAIL(3159) : (s < r) ? FAIL(3160) : 0) ? 0 : s);
}
...
#define REMAINING_EDGES ((((NUMBER_OF_VERTICES * DIMENSION) >> 2) - DIMENSION) - ((DIMENSION & 1) * CORRECTION))
  ...
  if ((cru_vertex_count (g, LANES, err) == REMAINING_VERTICES) ? 0 : FAIL(3161))
	 return 0;
  if ((cru_edge_count (g, LANES, err) == REMAINING_EDGES) ? 0 : FAIL(3162))
	 return 0;
  if (((uintptr_t) cru_mapreduced (g, &m, UNKILLABLE, LANES, err) == REMAINING_VERTICES) ? 0 : FAIL(3163))
	 return 0;
  return 1;
}
...
{
  if ((! given_vertex) ? FAIL(3164) : (given_vertex->v_magic != VERTEX_MAGIC) ? FAIL(3165) : 0)
	 return NULL;
...
free_pass (r, err)
	  ...
	  int *err;
{
  if ((! r) ? FAIL(3166) : (r->r_magic != REDEX_MAGIC) ? FAIL(3167) : (r->r_value != VERTEX_PASS) ? FAIL(3168) : 0)
	 return;
  free_redex (r, err);
}
	  ...
	  // f.fi_kernel.e_op.map field of the filter f.
{
  ...
  if ((! connecting_edge) ? FAIL(3169) : (connecting_edge->e_magic != EDGE_MAGIC) ? FAIL(3170) : 0)
	 return 0;
  if ((! local_vertex) ? FAIL(3171) : (local_vertex->v_magic != VERTEX_MAGIC) ? FAIL(3172) : 0)
	 return 0;
  if ((! adjacent_vertex) ? FAIL(3173) : (adjacent_vertex->v_magic != VERTEX_MAGIC) ? FAIL(3174) : 0)
	 return 0;
...
edge_reduction (a, b, err)
	  ...
	  // filter f.
{
  if ((! a) ? FAIL(3175) : (a->r_magic != MAPEX_MAGIC) ? FAIL(3176) : 0)
	 return NULL;
  if ((! b) ? FAIL(3177) : (b->r_magic != REDEX_MAGIC) ? FAIL(3178) : 0)
	 return NULL;
	  ...
	  int *err;
{
  ...
  if (lock_created ? (*lock_created = 0) : 1)
	 FAIL(3180);
  if (*err ? 1 : pthread_mutexattr_init (&mutex_attribute) ? FAIL(3181) : 0)
	 return 0;
  if (!((pthread_mutexattr_settype (&mutex_attribute, PTHREAD_MUTEX_ERRORCHECK)) ? FAIL(3182) : 0))
	 *lock_created = ! (pthread_mutex_init (lock, &mutex_attribute) ? FAIL(3183) : 0);
  return ! ((pthread_mutexattr_destroy (&mutex_attribute)) ? FAIL(3184) : *err);
}
	  ...
	  char **argv;
{
  ...
  nthm_sync (&err);
  if (! err)
	 err = (edge_count ? THE_FAIL(3185) : vertex_count ? THE_FAIL(3186) : redex_count ? THE_FAIL(3187) : global_err);
