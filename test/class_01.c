...
edge_checker (local_vertex, connecting_edge, remote_vertex, err)
	  ...
	  // NULL otherwise.
{
  if (((local_vertex ^ remote_vertex) == (1 << connecting_edge)) ? 1 : ! FAIL(2092))
...
vertex_checker (edges_in, vertex, edges_out, err)
	  ...
	  // edges being valid.
{
  return ! (*err ? 1 : (! edges_in) ? FAIL(2093) : (! edges_out) ? FAIL(2094) : (edges_in != edges_out) ? FAIL(2095) : 0);
}
	  ...
	  // other errors.
{
  ...
  return ((*err ? 1 : ((s = l + r) < l) ? FAIL(2096) : (s < r) ? FAIL(2097) : 0) ? 0 : s);
}
...
{
  if (*err ? 1 : (! l) ? FAIL(2098) : (! r) ? FAIL(2099) : (l != r) ? FAIL(2100) : 0)
	 return NULL;
  return l;
}
	  ...
	  // Check for correct vertex counts, edge counts, and edge labels.
{
  ...
  if ((cru_vertex_count (g, LANES, err) == NUMBER_OF_VERTICES) ? 0 : FAIL(2101))
	 return 0;
  if ((cru_edge_count (g, LANES, err) == DIMENSION * NUMBER_OF_VERTICES) ? 0 : FAIL(2102))
	 return 0;
  if ((((uintptr_t) cru_mapreduced (g, &m, UNKILLABLE, LANES, err)) == NUMBER_OF_VERTICES) ? 0 : FAIL(2103))
	 return 0;
  return 1;
}
	  ...
	  // c.
{
  ...
  if (*err)
	 return 0;
  if (edges_in ? FAIL(2104) : 0)
	 return 0;
  if ((edges_out != ((DIMENSION * (DIMENSION - 1)) >> 1)) ? FAIL(2105) : 0)
	 return 0;
  ...
  for (i = 0; *err ? 0 : (i < DIMENSION - 1); i++)
	 {
		...
		if (*err != ENOMEM)
		  if (cru_united (c, xh, yh, err) ? 0 : FAIL(2106))
			 break;
		if (((xh = cru_class_of (c, x, err)) == (yh = cru_class_of (c, y, err))) ? 0 : FAIL(2107))
		  break;
	 }
  return c;
}
