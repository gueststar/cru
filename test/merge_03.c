	  ...
	  // Create a redex.
{
  ...
  if ((r = (redex) malloc (sizeof (*r))) ? 0 : FAIL(3990))
	 return NULL;
  memset (r, 0, sizeof (*r));
...
free_redex (r, err)
	  ...
	  // Reclaim a redex.
{
  if ((! r) ? FAIL(3994) : (r->r_magic != REDEX_MAGIC) ? FAIL(3995) : 0)
	 return;
...
free_mapex (r, err)
	  ...
	  // Reclaim a mapex.
{
  if ((! r) ? FAIL(3999) : (r->r_magic != MAPEX_MAGIC) ? FAIL(4000) : 0)
	 return;
  r->r_magic = REDEX_MAGIC;
  free_redex (r, err);
}
...
{
  if ((! r) ? FAIL(4001) : (r->r_magic != PROPERTY_MAGIC) ? FAIL(4002) : 0)
	 return;
...
{
  if ((! r) ? FAIL(4006) : (r->r_magic != INCIDENT_REDEX_MAGIC) ? FAIL(4007) : 0)
	 return;
  r->r_magic = MUGGLE(166);
  if (pthread_mutex_lock (&redex_lock) ? FAIL(4008) : redex_count-- ? 0 : FAIL(4009))
	 return;
  if (pthread_mutex_unlock (&redex_lock) ? FAIL(4010) : 0)
	 return;
  free (r);
}
...
{
  if ((! r) ? FAIL(4011) : (r->r_magic != INCIDENT_MAPEX_MAGIC) ? FAIL(4012) : 0)
	 return;
...
{
  if ((! r) ? FAIL(4013) : (r->r_magic != OUTGOING_REDEX_MAGIC) ? FAIL(4014) : 0)
	 return;
...
{
  if ((! a) ? FAIL(4018) : (a->r_magic != PROPERTY_MAGIC) ? FAIL(4019) : 0)
	 return 0;
  if ((! b) ? FAIL(4020) : (b->r_magic != PROPERTY_MAGIC) ? FAIL(4021) : 0)
	 return 0;
	  ...
	  // cru_hash is passed as c.me_classifier.cl_order.hash in the merger c.
{
  ...
  if ((! r) ? GLOBAL_FAIL(4022) : (r->r_magic != PROPERTY_MAGIC) ? GLOBAL_FAIL(4023) : 0)
	 return 0;
  u = r->r_value;
	  ...
	  // Create an edge.
{
  ...
  if ((e = (edge) malloc (sizeof (*e))) ? 0 : FAIL(4024))
	 return NULL;
  memset (e, 0, sizeof (*e));
  e->e_magic = EDGE_MAGIC;
  e->e_value = n;
  if (pthread_mutex_lock (&edge_lock) ? FAIL(4025) : ++edge_count ? 0 : FAIL(4026))
	 goto a;
  if (! (pthread_mutex_unlock (&edge_lock) ? FAIL(4027) : 0))
	 return e;
 a: free (e);
  return NULL;
}
...
free_edge (e, err)
	  ...
	  // b.
{
  if ((! e) ? FAIL(4028) : (e->e_magic != EDGE_MAGIC) ? FAIL(4029) : 0)
	 return;
  e->e_magic = MUGGLE(168);
  if (pthread_mutex_lock (&edge_lock) ? FAIL(4030) : edge_count-- ? 0 : FAIL(4031))
	 return;
  if (pthread_mutex_unlock (&edge_lock) ? FAIL(4032) : 0)
	 return;
  free (e);
}
...
{
  if ((! e) ? FAIL(4033) : (e->e_magic != FUSED_EDGE_MAGIC) ? FAIL(4034) : 0)
	 return;
  e->e_magic = MUGGLE(169);
  if (pthread_mutex_lock (&edge_lock) ? FAIL(4035) : edge_count-- ? 0 : FAIL(4036))
	 return;
  if (pthread_mutex_unlock (&edge_lock) ? FAIL(4037) : 0)
	 return;
  free (e);
}
	  ...
	  // but is needed only for merging and not building.
{
  ...
  if ((! e) ? GLOBAL_FAIL(4038) : (e->e_magic != EDGE_MAGIC) ? GLOBAL_FAIL(4039) : 0)
	 return 0;
  u = e->e_value;
  for (i = 0; i < sizeof (u); i++)  // ignore overflow
	 u += (u + 1) << 8;
	  ...
	  // Mix up low-entropy numeric values.
{
  ...
  if ((! e) ? GLOBAL_FAIL(4040) : (e->e_magic != FUSED_EDGE_MAGIC) ? GLOBAL_FAIL(4041) : 0)
	 return 0;
  u = e->e_value;
  for (i = 0; i < sizeof (u); i++)  // ignore overflow
	 u += (u + 1) << 8;
...
{
  if ((! a) ? FAIL(4042) : (a->e_magic != EDGE_MAGIC) ? FAIL(4043) : 0)
	 return 0;
  if ((! b) ? FAIL(4044) : (b->e_magic != EDGE_MAGIC) ? FAIL(4045) : 0)
	 return 0;
...
fused_equal_edges (a, b, err)
	  ...
	  // equal.
{
  if ((! a) ? FAIL(4046) : (a->e_magic != FUSED_EDGE_MAGIC) ? FAIL(4047) : 0)
	 return 0;
  if ((! b) ? FAIL(4048) : (b->e_magic != FUSED_EDGE_MAGIC) ? FAIL(4049) : 0)
	 return 0;
  return a->e_value == b->e_value;
}
	  ...
	  // Create a vertex.
{
  ...
  if ((v = (vertex) malloc (sizeof (*v))) ? 0 : FAIL(4050))
	 return NULL;
  memset (v, 0, sizeof (*v));
  v->v_magic = VERTEX_MAGIC;
  v->v_value = n;
  if (pthread_mutex_lock (&vertex_lock) ? FAIL(4051) : ++vertex_count ? 0 : FAIL(4052))
	 goto a;
  if (! (pthread_mutex_unlock (&vertex_lock) ? FAIL(4053) : 0))
	 return v;
 a: free (v);
  return NULL;
}
...
free_vertex (v, err)
	  ...
	  // b.
{
  if ((! v) ? FAIL(4054) : (v->v_magic != VERTEX_MAGIC) ? FAIL(4055) : 0)
	 return;
  v->v_magic = MUGGLE(170);
  if (pthread_mutex_lock (&vertex_lock) ? FAIL(4056) : vertex_count-- ? 0 : FAIL(4057))
	 return;
  if (pthread_mutex_unlock (&vertex_lock) ? FAIL(4058) : 0)
	 return;
  free (v);
}
...
{
  if ((! v) ? FAIL(4059) : (v->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(4060) : 0)
	 return;
  v->v_magic = MUGGLE(171);
  if (pthread_mutex_lock (&vertex_lock) ? FAIL(4061) : vertex_count-- ? 0 : FAIL(4062))
	 return;
  if (pthread_mutex_unlock (&vertex_lock) ? FAIL(4063) : 0)
	 return;
  free (v);
}
...
{
  if ((! a) ? FAIL(4064) : (a->v_magic != VERTEX_MAGIC) ? FAIL(4065) : 0)
	 return 0;
  if ((! b) ? FAIL(4066) : (b->v_magic != VERTEX_MAGIC) ? FAIL(4067) : 0)
	 return 0;
...
fused_equal_vertices (a, b, err)
	  ...
	  // equal.
{
  if ((! a) ? FAIL(4068) : (a->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(4069) : 0)
	 return 0;
  if ((! b) ? FAIL(4070) : (b->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(4071) : 0)
	 return 0;
  return a->v_value == b->v_value;
}
	  ...
	  // in the builder b.
{
  ...
  if ((! v) ? GLOBAL_FAIL(4072) : (v->v_magic != VERTEX_MAGIC) ? GLOBAL_FAIL(4073) : 0)
	 return 0;
  u = v->v_value;
  for (i = 0; i < sizeof (u); i++)  // ignore overflow
	 u += (u + 1) << 8;
	  ...
	  // Mix up low-entropy numeric values.
{
  ...
  if ((! v) ? GLOBAL_FAIL(4074) : (v->v_magic != FUSED_VERTEX_MAGIC) ? GLOBAL_FAIL(4075) : 0)
	 return 0;
  u = v->v_value;
  for (i = 0; i < sizeof (u); i++)  // ignore overflow
	 u += (u + 1) << 8;
	  ...
	  // the builder b.
{
  ...
  if ((! given_vertex) ? FAIL(4076) : (given_vertex->v_magic != VERTEX_MAGIC) ? FAIL(4077) : 0)
	 return;
	  ...
	  // which its endpoints differ, and 0 otherwise.
{
  ...
  if (*err ? 1 : (! connecting_edge) ? FAIL(4078) : (connecting_edge->e_magic != FUSED_EDGE_MAGIC) ? FAIL(4079) : 0)
	 return NULL;
  if ((! local_vertex) ? FAIL(4080) : (local_vertex->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(4081) : 0)
	 return NULL;
  if ((! remote_vertex) ? FAIL(4082) : (remote_vertex->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(4083) : 0)
	 return NULL;
  if (((e = connecting_edge->e_value) & 1) ? FAIL(4084) : 0)
	 return NULL;
  if ((((local_vertex->v_value ^ remote_vertex->v_value) << 1) != (1 << (e >> 1))) ? FAIL(4085) : 0)
	 return NULL;
...
vertex_checker (edges_in, given_vertex, edges_out, err)
	  ...
	  // edges being valid.
{
  if (*err ? 1 : (! given_vertex) ? FAIL(4086) : (given_vertex->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(4087) : 0)
	 return NULL;
  if ((! edges_in) ? FAIL(4088) : (edges_in->r_magic != REDEX_MAGIC) ? FAIL(4089) : 0)
	 return NULL;
  if ((! edges_out) ? FAIL(4090) : (edges_out->r_magic != REDEX_MAGIC) ? FAIL(4091) : 0)
	 return NULL;
  if ((edges_in->r_value != DIMENSION - 1) ? FAIL(4092) : (edges_out->r_value != DIMENSION - 1) ? FAIL(4093) : 0)
	 return NULL;
  return redex_of (1, err);
}
	  ...
	  // errors.
{
  ...
  if (*err)
	 return NULL;
  if ((! l) ? 0 : (l->r_magic != REDEX_MAGIC) ? FAIL(4094) : 0)
	 return NULL;
  if ((! r) ? 0 : (r->r_magic != REDEX_MAGIC) ? FAIL(4095) : 0)
	 return NULL;
  s = (l ? l->r_value : 0) + (r ? r->r_value : 0);
  if ((s < (l ? l->r_value : 0)) ? FAIL(4096) : (s < (r ? r->r_value : 0)) ? FAIL(4097) : 0)
	 return NULL;
	  ...
	  // errors.
{
  ...
  if (*err)
	 return NULL;
  if ((! l) ? 0 : (l->r_magic != INCIDENT_MAPEX_MAGIC) ? FAIL(4098) : 0)
	 return NULL;
  if ((! r) ? 0 : (r->r_magic != INCIDENT_REDEX_MAGIC) ? FAIL(4099) : 0)
	 return NULL;
  s = (l ? l->r_value : 0) + (r ? r->r_value : 0);
  if ((s < (l ? l->r_value : 0)) ? FAIL(4100) : (s < (r ? r->r_value : 0)) ? FAIL(4101) : 0)
	 return NULL;
  if ((t = redex_of (s, err)))
	  ...
	  // errors.
{
  ...
  if (*err)
	 return NULL;
  if ((! l) ? 0 : (l->r_magic != OUTGOING_REDEX_MAGIC) ? FAIL(4102) : 0)
	 return NULL;
  if ((! r) ? 0 : (r->r_magic != OUTGOING_REDEX_MAGIC) ? FAIL(4103) : 0)
	 return NULL;
  s = (l ? l->r_value : 0) + (r ? r->r_value : 0);
	  ...
	  // the hard way by using redexes for numbers.
{
  ...
  if (*err)
	 return 0;
  if ((cru_vertex_count (g, LANES, err) == (NUMBER_OF_VERTICES >> 1)) ? 0 : FAIL(4106))
	 return 0;
  if ((cru_edge_count (g, LANES, err) == (((DIMENSION - 1) * NUMBER_OF_VERTICES) >> 1)) ? 0 : FAIL(4107))
	 return 0;
  if ((r = (redex) cru_mapreduced (g, &m, UNKILLABLE, LANES, err)) ? 0 : FAIL(4108))
	 return 0;
  if ((r->r_magic == REDEX_MAGIC) ? 0 : FAIL(4109))
	 return 0;
...
#define D ((DIMENSION * (DIMENSION - 1)) >> 1)
  ...
  if (*err ? 1 : (! given_vertex) ? FAIL(4111) : (given_vertex->v_magic != VERTEX_MAGIC) ? FAIL(4112) : 0)
	 return NULL;
  if ((! edges_in) ? FAIL(4113) : (edges_in->r_magic != INCIDENT_REDEX_MAGIC) ? FAIL(4114) : 0)
	 return NULL;
  if ((! edges_out) ? FAIL(4115) : (edges_out->r_magic != OUTGOING_REDEX_MAGIC) ? FAIL(4116) : 0)
	 return NULL;
  if ((edges_in->r_value != D) ? FAIL(4117) : (edges_out->r_value != D) ? FAIL(4118) : 0)
	 return NULL;
	  ...
	  // Return an incident redex of the edge label.
{
  ...
  if (*err ? 1 : (! connecting_edge) ? FAIL(4119) : (connecting_edge->e_magic != EDGE_MAGIC) ? FAIL(4120) : 0)
	 return NULL;
  if ((! local_vertex) ? FAIL(4121) : (local_vertex->v_magic != VERTEX_MAGIC) ? FAIL(4122) : 0)
	 return NULL;
  if ((! remote_vertex) ? FAIL(4123) : (remote_vertex->v_magic != VERTEX_MAGIC) ? FAIL(4124) : 0)
	 return NULL;
	  ...
	  // as c.me_kernel.v_op.vertex.map in the merger c.
{
  ...
  if (edges_in ? FAIL(4131) : edges_out ? FAIL(4132) : 0)
	 return NULL;
  if ((! given_vertex) ? FAIL(4133) : (given_vertex->v_magic != VERTEX_MAGIC) ? FAIL(4134) : 0)
	 return NULL;
	  ...
	  // Return an identical edge but change it to a fused edge type.
{
  ...
  if (*err ? 1 : (! connecting_edge) ? FAIL(4135) : (connecting_edge->e_magic != EDGE_MAGIC) ? FAIL(4136) : 0)
	 return NULL;
  if ((! local_vertex) ? FAIL(4137) : (local_vertex->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(4138) : 0)
	 return NULL;
  if ((! remote_vertex) ? FAIL(4139) : (remote_vertex->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(4140) : 0)
	 return NULL;
	  ...
	  // merger c.
{
  ...
  if ((! a) ? FAIL(4141) : (a->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(4142) : 0)
	 return NULL;
  if ((! b) ? FAIL(4143) : (b->v_magic != FUSED_VERTEX_MAGIC) ? FAIL(4144) : 0)
	 return NULL;
  if ((a->v_value != b->v_value) ? FAIL(4145) : 0)
	 return NULL;
	  ...
	  // them even more into a single vertex.
{
  ...
  if ((! a) ? FAIL(4146) : (a->e_magic != FUSED_EDGE_MAGIC) ? FAIL(4147) : 0)
	 return NULL;
  if ((! b) ? FAIL(4148) : (b->e_magic != FUSED_EDGE_MAGIC) ? FAIL(4149) : 0)
	 return NULL;
	  ...
	  int *err;
{
  ...
  if (lock_created ? (*lock_created = 0) : 1)
	 FAIL(4151);
  if (*err ? 1 : pthread_mutexattr_init (&mutex_attribute) ? FAIL(4152) : 0)
	 return 0;
  if (!((pthread_mutexattr_settype (&mutex_attribute, PTHREAD_MUTEX_ERRORCHECK)) ? FAIL(4153) : 0))
	 *lock_created = ! (pthread_mutex_init (lock, &mutex_attribute) ? FAIL(4154) : 0);
  return ! ((pthread_mutexattr_destroy (&mutex_attribute)) ? FAIL(4155) : *err);
}
...
{
  if ((! outgoing_edge) ? FAIL(4156) : (outgoing_edge->e_magic != EDGE_MAGIC) ? FAIL(4157) : (! everywhere) ? FAIL(4158) : 0)
	 return 0;
	  ...
	  char **argv;
{
  ...
  nthm_sync (&err);
  if (! err)
	 err = (edge_count ? THE_FAIL(4159) : vertex_count ? THE_FAIL(4160) : redex_count ? THE_FAIL(4161) : global_err);
